...
MAVLinkSystem::MAVLinkSystem(DroneCoreImpl &parent, uint8_t system_id, uint8_t comp_id)
    : _system_id(system_id), _parent(parent), _params(*this), _commands(*this),
      _timeout_handler(_time), _call_every_handler(_time), _thread_pool(3)
{
    _system_thread = new std::thread(system_thread, this);

    register_mavlink_message_handler(
        MAVLINK_MSG_ID_HEARTBEAT, std::bind(&MAVLinkSystem::process_heartbeat, this, _1), this);
    ...
    register_mavlink_message_handler(MAVLINK_MSG_ID_AUTOPILOT_VERSION,
                                     std::bind(&MAVLinkSystem::process_autopilot_version, this, _1),
                                     this);

    register_mavlink_message_handler(
        MAVLINK_MSG_ID_STATUSTEXT, std::bind(&MAVLinkSystem::process_statustext, this, _1), this);
...
bool
MAVLinkSystem::is_connected() const
{
    return _connected;
}

void
MAVLinkSystem::register_mavlink_message_handler(uint16_t msg_id,
...
void
MAVLinkSystem::unregister_all_mavlink_message_handlers(const void *cookie)
{
    std::lock_guard<std::mutex> lock(_mavlink_handler_table_mutex);
...
void
MAVLinkSystem::register_timeout_handler(std::function<void()> callback,
                                        double duration_s,
                                        void **cookie)
{
...
void
MAVLinkSystem::refresh_timeout_handler(const void *cookie)
{
    _timeout_handler.refresh(cookie);
}

void
MAVLinkSystem::unregister_timeout_handler(const void *cookie)
{
    _timeout_handler.remove(cookie);
}

void
MAVLinkSystem::process_mavlink_message(const mavlink_message_t &message)
{
...
void
MAVLinkSystem::add_call_every(std::function<void()> callback, float interval_s, void **cookie)
{
    _call_every_handler.add(callback, interval_s, cookie);
}

void
MAVLinkSystem::change_call_every(float interval_s, const void *cookie)
{
    _call_every_handler.change(interval_s, cookie);
}

void
MAVLinkSystem::reset_call_every(const void *cookie)
{
    _call_every_handler.reset(cookie);
}

void
MAVLinkSystem::remove_call_every(const void *cookie)
{
    _call_every_handler.remove(cookie);
}

void
MAVLinkSystem::process_heartbeat(const mavlink_message_t &message)
{
...
void
MAVLinkSystem::process_autopilot_version(const mavlink_message_t &message)
{
...
void
MAVLinkSystem::process_statustext(const mavlink_message_t &message)
{
...
void
MAVLinkSystem::heartbeats_timed_out()
{
...
void
MAVLinkSystem::system_thread(MAVLinkSystem *self)
{
    dl_time_t last_time{};
...
std::string
MAVLinkSystem::component_name(uint8_t component_id)
{
    switch (component_id) {
...
void
MAVLinkSystem::add_new_component(uint8_t component_id)
{
...
size_t
MAVLinkSystem::total_components() const
{
    return _components.size();
}

bool
MAVLinkSystem::is_standalone() const
{
    return !has_autopilot();
}

bool
MAVLinkSystem::has_autopilot() const
{
    return get_autopilot_id() != uint8_t(0);
}

bool
MAVLinkSystem::is_autopilot(uint8_t comp_id)
{
    return comp_id == MAV_COMP_ID_AUTOPILOT1;
}

bool
MAVLinkSystem::is_camera(uint8_t comp_id)
{
    return (comp_id >= MAV_COMP_ID_CAMERA) && (comp_id <= MAV_COMP_ID_CAMERA6);
}

bool
MAVLinkSystem::has_camera(int camera_id) const
{
...
bool
MAVLinkSystem::has_gimbal() const
{
    return get_gimbal_id() == MAV_COMP_ID_GIMBAL;
}

void
MAVLinkSystem::send_heartbeat(MAVLinkSystem &self)
{
...
bool
MAVLinkSystem::send_message(const mavlink_message_t &message)
{
    if (_communication_locked) {
        return false;
    }
...
void
MAVLinkSystem::request_autopilot_version()
{
...
void
MAVLinkSystem::set_disconnected()
{
    {
        std::lock_guard<std::mutex> lock(_connection_mutex);
...
uint8_t
MAVLinkSystem::get_system_id() const
{
    return _system_id;
}

void
MAVLinkSystem::set_system_id(uint8_t system_id)
{
    _system_id = system_id;
}

void
MAVLinkSystem::set_param_float_async(const std::string &name, float value, success_t callback)
{
...
void
MAVLinkSystem::set_param_int_async(const std::string &name, int32_t value, success_t callback)
{
    MAVLinkParameters::ParamValue param_value;
...
void
MAVLinkSystem::set_param_ext_float_async(const std::string &name, float value, success_t callback)
{
    MAVLinkParameters::ParamValue param_value;
    param_value.set_float(value);
...
void
MAVLinkSystem::set_param_ext_int_async(const std::string &name, int32_t value, success_t callback)
{
    MAVLinkParameters::ParamValue param_value;
    param_value.set_int32(value);
    _params.set_param_async(name, param_value, callback, true);
...
void
MAVLinkSystem::get_param_float_async(const std::string &name, get_param_float_callback_t callback)
{
    _params.get_param_async(name, std::bind(&MAVLinkSystem::receive_float_param, _1, _2, callback));
}

void
MAVLinkSystem::set_param_async(const std::string &name,
...
void
MAVLinkSystem::get_param_async(const std::string &name,
...
MAVLinkCommands::Result
MAVLinkSystem::set_flight_mode(FlightMode system_mode, uint8_t component_id)
{
    MAVLinkCommands::CommandLong command{};
...
void
MAVLinkSystem::set_flight_mode_async(FlightMode system_mode,
...
void
MAVLinkSystem::get_param_int_async(const std::string &name, get_param_int_callback_t callback)
{
    _params.get_param_async(name, std::bind(&MAVLinkSystem::receive_int_param, _1, _2, callback));
}

void
MAVLinkSystem::get_param_ext_float_async(const std::string &name,
                                         get_param_float_callback_t callback)
{
    _params.get_param_async(
        name, std::bind(&MAVLinkSystem::receive_float_param, _1, _2, callback), true);
}

void
MAVLinkSystem::get_param_ext_int_async(const std::string &name, get_param_int_callback_t callback)
{
    _params.get_param_async(
        name, std::bind(&MAVLinkSystem::receive_int_param, _1, _2, callback), true);
}

void
MAVLinkSystem::receive_float_param(bool success,
                                   MAVLinkParameters::ParamValue value,
                                   get_param_float_callback_t callback)
{
    if (callback) {
...
void
MAVLinkSystem::receive_int_param(bool success,
                                 MAVLinkParameters::ParamValue value,
...
uint8_t
MAVLinkSystem::get_autopilot_id() const
{
    for (auto compid : _components)
...
std::vector<uint8_t>
MAVLinkSystem::get_camera_ids() const
{
...
uint8_t
MAVLinkSystem::get_gimbal_id() const
{
    for (auto compid : _components)
...
void
MAVLinkSystem::send_command_async(MAVLinkCommands::CommandLong &command,
                                  command_result_callback_t callback)
{
    if (_system_id == 0 && _components.size() == 0) {
        if (callback) {
...
void
MAVLinkSystem::send_command_async(MAVLinkCommands::CommandInt &command,
                                  command_result_callback_t callback)
{
    if (_system_id == 0 && _components.size() == 0) {
        if (callback) {
...
MAVLinkCommands::Result
MAVLinkSystem::set_msg_rate(uint16_t message_id, double rate_hz, uint8_t component_id)
{
...
void
MAVLinkSystem::set_msg_rate_async(uint16_t message_id,
                                  double rate_hz,
                                  command_result_callback_t callback,
                                  uint8_t component_id)
{
...
void
MAVLinkSystem::register_plugin(PluginImplBase *plugin_impl)
{
    assert(plugin_impl);
...
void
MAVLinkSystem::unregister_plugin(PluginImplBase *plugin_impl)
{
    assert(plugin_impl);
...
void
MAVLinkSystem::call_user_callback(const std::function<void()> &func)
{
    _thread_pool.enqueue(func);
}

void
MAVLinkSystem::lock_communication()
{
    _communication_locked = true;
}

void
MAVLinkSystem::unlock_communication()
{
