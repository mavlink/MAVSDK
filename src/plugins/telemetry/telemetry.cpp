...
void Telemetry::subscribe_attitude_angular_velocity_body(AttitudeAngularVelocityBodyCallback callback)
{
...
{
    return
        ((std::isnan(rhs.min_distance) && std::isnan(lhs.min_distance)) || rhs.min_distance == lhs.min_distance) &&
        ((std::isnan(rhs.max_distance) && std::isnan(lhs.max_distance)) || rhs.max_distance == lhs.max_distance) &&
        ((std::isnan(rhs.current_distance) && std::isnan(lhs.current_distance)) || rhs.current_distance == lhs.current_distance);
}
...
std::ostream& operator<<(std::ostream& str, Telemetry::DistanceSensor const& distance_sensor)
{
    ...
    str << "distance_sensor:" << '\n'
        << "{\n";
    str << "    min_distance: " << distance_sensor.min_distance << '\n';
    str << "    max_distance: " << distance_sensor.max_distance << '\n';
    str << "    current_distance: " << distance_sensor.current_distance << '\n';
    str << '}';
    return str;
}
