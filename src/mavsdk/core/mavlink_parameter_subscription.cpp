    ...
    const std::string& name, const ParamChangedCallback<T>& callback, const void* cookie)
{
    ...
    if (callback != nullptr) {
        ...
        // This is just to let the upper level know of what probably is a bug, but we check again when actually calling the callback We also cannot assume here that the user called provide_param before subscribe_param_changed, so the only thing that makes sense is to log a warning, but then continue anyways.
        ...
        if (_all_params.find(name) != _all_params.end()) {
            ...
            if (!curr_value.template is_same_type_templated<T>()) {
                LogDebug()
                    << "You just registered a param changed callback where the type does not match the type already stored";
            }
        }*/
