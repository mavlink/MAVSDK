/*#include "plugins/camera_manager/camera_manager.h"
...
public:
    ...
    void provide_camera_information(const std::function<(mavlink_camera_information_t&
camera_information)>& callback) { callback(camera_information); camera_information_valid = true;
    }

    template <class T>
    void add_param(const std::string& name, T default_value, const typename std::function<void(T
t)>& callback) { _parent->provide_server_param_int("CAM_ISO", default_value);
        _parent->subscribe_param(
            name,
            overloaded {
                [&name](auto value) { std::cerr << "invalid parameter type for " << name <<
std::endl; }, callback
            },
            nullptr);
    }
