    ...
    const std::string& json_string, mav::Message& msg) const
{
    ...
    for (const auto& field_name : json.getMemberNames()) {
        ...
        } else if (field_value.isDouble() || field_value.isNull()) {
            if (field_value.isNull()) {
                // Handle null values by setting to NaN
                auto result = msg.set(field_name, std::numeric_limits<float>::quiet_NaN());
                if (result != ::mav::MessageResult::Success) {
                    // Try as double
                    if (msg.set(field_name, std::numeric_limits<double>::quiet_NaN()) !=
                        ::mav::MessageResult::Success) {
                        LogWarn() << "Failed to set null float/double field " << field_name
                                  << " to NaN";
                    }
                }
            } else {
                ...
                if (result != ::mav::MessageResult::Success) {
                    // Try as double
                    if (msg.set(field_name, field_value.asDouble()) !=
                        ::mav::MessageResult::Success) {
