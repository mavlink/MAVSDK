...
Mocap::Result MocapImpl::set_vision_speed_estimate(
    const Mocap::VisionSpeedEstimate& vision_speed_estimate)
{
    if (!_system_impl->is_connected()) {
        return Mocap::Result::NoSystem;
    }
...
Mocap::Result MocapImpl::send_vision_speed_estimate(
    const Mocap::VisionSpeedEstimate& vision_speed_estimate)
{
    const uint64_t autopilot_time_usec =
        (!vision_speed_estimate.time_usec) ?
            ...
            std::chrono::duration_cast<std::chrono::microseconds>(
                _system_impl->get_autopilot_time()
                    .time_in(SystemTimePoint(
                        std::chrono::microseconds(vision_speed_estimate.time_usec)))
                    .time_since_epoch())
                .count();
