// Example to connect multiple vehicles and make them follow their own separate plan file. Also saves the telemetry information to a csv file
//./fly_multipleDrones udp://:14540 udp://:14541 ../../../plugins/mission/test.plan ../../../plugins/mission/test2.plan
//
// Author: Julian Oes <julian@oes.ch>
// Author: Shayaan Haider (via Slack)
// By Shusil Shapkota (Air Matrix)
...
4. Run the executable by passing the connection urls (ex. udp://:14540) and path of QGC mission plan as arguments
    Example: If you have test.plan and test2.plan in "../../../plugins/mission/" and you are running two drones in 
    udp://:14540 and udp://:14541 then you run the example as: 
...
int main(int argc, char * argv[]) {
    ...
    if (argc % 2 == 0 || argc == 1) {
        std::cerr << ERROR_CONSOLE_TEXT << "Please make sure you have specified the connections and plan files for each drones" << NORMAL_CONSOLE_TEXT <<
            std::endl;
        return 1;
    }
    ...
    // Loop must start from 1 since we are ignoring argv[0] which would be the name of the executable
    ...
    int planFile_provided = total_ports + 1; // +1 because first plan is specified at argv[total_ports+1]
...
void complete_mission(std::string qgc_plan, System & system) {
    ...
    mission -> subscribe_progress([&](int current, int total) {
        std::cout << "Mission status update, VehicleID: " << system.get_uuid() % 100000 <<  " --> " << current << " / " << total << std::endl;
    });
