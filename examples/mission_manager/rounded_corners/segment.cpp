...
double LineSegment::length() const {
    return sqrt(
        (begin_.x - end_.x) * (begin_.x - end_.x) +
...
std::vector<Point> LineSegment::sample(double resolution) const {
    ...
    for (unsigned long step = 0; step < num_points; ++step) {
        auto point = Point{begin_ + (static_cast<double>(step) / static_cast<double>(num_points)) * dir};
...
std::vector<Point> CornerSegment::sample(double resolution) const {
    ...
    for (unsigned long step = 0; step < num_points; ++step) {
        ...
        const auto slerp =
            std::sin((1-t)*angle_) / std::sin(angle_) * (begin_-center_) +
