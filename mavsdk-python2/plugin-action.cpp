...
action.def(py::init<std::shared_ptr<mavsdk::System>>())
    .def("arm", &mavsdk::Action::arm)
...
py::enum_<mavsdk::Action::Result>(action, "Result")
    ...
    .value("CommandDeniedLandedStateUnknown", mavsdk::Action::Result::CommandDeniedLandedStateUnknown)
