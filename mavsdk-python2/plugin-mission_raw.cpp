...
void init_mission_raw(py::module_ &m) {
    py::class_<mavsdk::MissionRaw> mission_raw(m, "MissionRaw");


mission_raw.def(py::init<std::shared_ptr<mavsdk::System>>())
    .def("upload_mission", &mavsdk::MissionRaw::upload_mission)
...
py::class_<mavsdk::MissionRaw::MissionProgress> mission_progress(mission_raw, "MissionProgress");
mission_progress
    .def(py::init<>())
