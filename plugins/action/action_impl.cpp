...
void ActionImpl::transition_to_fixedwing_async(const Action::result_callback_t &callback)
{
    ...
    _parent->send_command_async(
        command, std::bind(&ActionImpl::command_result_callback, this, _1, callback));
}
...
Action::Result ActionImpl::set_takeoff_altitude(float relative_altitude_m)
{
    ...
    return success ? Action::Result::SUCCESS : Action::Result::PARAMETER_ERROR;
}
...
std::pair<Action::Result, float> ActionImpl::get_takeoff_altitude() const
{
    ...
    return std::make_pair<>(
        result.first ? Action::Result::SUCCESS : Action::Result::PARAMETER_ERROR, result.second);
}
                                         ...
                                         const Action::result_callback_t &callback)
{
    ...
    if (callback) {
        _parent->call_user_callback([callback, action_result]() { callback(action_result); });
    }
}
