...
void
FollowMeImpl::disable()
{
    stop_sending_target_location();
}

const FollowMe::Config &
FollowMeImpl::get_config() const
{
    return _config;
}

FollowMe::Result
FollowMeImpl::set_config(const FollowMe::Config &config)
{
...
void
FollowMeImpl::set_target_location(const FollowMe::TargetLocation &location)
{
    _mutex.lock();
...
FollowMeImpl::get_last_location() const
{
...
FollowMeImpl::start()
{
    FollowMe::Result result =
        to_follow_me_result(_parent->set_flight_mode(MAVLinkSystem::FlightMode::FOLLOW_ME));
...
FollowMeImpl::stop()
{
    ...
    return to_follow_me_result(_parent->set_flight_mode(MAVLinkSystem::FlightMode::HOLD));
}

// Applies default FollowMe configuration to the system
void
FollowMeImpl::set_default_config()
{
...
bool
FollowMeImpl::is_config_ok(const FollowMe::Config &config) const
{
...
void
FollowMeImpl::receive_param_min_height(bool success, float min_height_m)
{
    if (success) {
...
void
FollowMeImpl::receive_param_follow_distance(bool success, float follow_distance_m)
{
    if (success) {
...
void
FollowMeImpl::receive_param_follow_direction(bool success, int32_t direction)
{
...
void
FollowMeImpl::receive_param_responsiveness(bool success, float responsiveness)
{
    if (success) {
...
bool
FollowMeImpl::is_target_location_set() const
{
...
void
FollowMeImpl::send_target_location()
{
...
void
FollowMeImpl::stop_sending_target_location()
{
    // We assume that mutex was acquired by the caller
    if (_target_location_cookie) {
...
void
FollowMeImpl::process_heartbeat(const mavlink_message_t &message)
{
