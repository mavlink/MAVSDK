...
void
InfoImpl::process_heartbeat(const mavlink_message_t &message)
{
    UNUSED(message);
...
void
InfoImpl::process_autopilot_version(const mavlink_message_t &message)
{
...
void
InfoImpl::translate_binary_to_str(uint8_t *binary, unsigned binary_len, char *str, unsigned str_len)
{
...
uint64_t
InfoImpl::get_uuid() const
{
    return _parent->get_uuid();
}

bool
InfoImpl::is_complete() const
{
    {
...
void
InfoImpl::set_version(Info::Version version)
{
    std::lock_guard<std::mutex> lock(_version_mutex);
    _version = version;
}
...
void
InfoImpl::set_product(Info::Product product)
{
    std::lock_guard<std::mutex> lock(_product_mutex);
    _product = product;
}
...
const char *
InfoImpl::vendor_id_str(uint16_t vendor_id)
{
    switch (vendor_id) {
        case 0x26ac:
            return "Yuneec";
...
const char *
InfoImpl::product_id_str(uint16_t product_id)
{
    switch (product_id) {
        case 0x0010:
            return "H520";
