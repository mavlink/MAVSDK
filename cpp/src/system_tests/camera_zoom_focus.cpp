...
TEST(Camera, Zoom)
{
    ...
    auto zoom_in_start_handle =
        camera_server->subscribe_zoom_in_start([camera_server_weak](int32_t index) {
            ...
            server->respond_zoom_in_start(CameraServer::CameraFeedback::Ok);
        });

    auto zoom_out_start_handle =
        camera_server->subscribe_zoom_out_start([camera_server_weak](int32_t index) {
            auto server = camera_server_weak.lock();
            if (!server) {
                return;
            }
            ...
            server->respond_zoom_out_start(CameraServer::CameraFeedback::Ok);
        });

    auto zoom_stop_handle = camera_server->subscribe_zoom_stop([camera_server_weak](int32_t index) {
        auto server = camera_server_weak.lock();
        if (!server) {
            return;
        }
        ...
        server->respond_zoom_stop(CameraServer::CameraFeedback::Ok);
    });

    auto zoom_range_handle =
        camera_server->subscribe_zoom_range([camera_server_weak](float zoom_level) {
            auto server = camera_server_weak.lock();
            if (!server) {
                return;
            }
            ...
            server->respond_zoom_range(CameraServer::CameraFeedback::Ok);
        });
...
TEST(Camera, Focus)
{
    ...
    auto focus_in_step_handle =
        camera_server->subscribe_focus_in_step([camera_server_weak](int32_t index) {
            auto server = camera_server_weak.lock();
            if (!server) {
                return;
            }
            ...
            server->respond_focus_in_step(CameraServer::CameraFeedback::Ok);
        });

    auto focus_out_step_handle =
        camera_server->subscribe_focus_out_step([camera_server_weak](int32_t index) {
            auto server = camera_server_weak.lock();
            if (!server) {
                return;
            }
            ...
            server->respond_focus_out_step(CameraServer::CameraFeedback::Ok);
        });

    auto focus_in_start_handle =
        camera_server->subscribe_focus_in_start([camera_server_weak](int32_t index) {
            auto server = camera_server_weak.lock();
            if (!server) {
                return;
            }
            ...
            server->respond_focus_in_start(CameraServer::CameraFeedback::Ok);
        });

    auto focus_out_start_handle =
        camera_server->subscribe_focus_out_start([camera_server_weak](int32_t index) {
            auto server = camera_server_weak.lock();
            if (!server) {
                return;
            }
            ...
            server->respond_focus_out_start(CameraServer::CameraFeedback::Ok);
        });

    auto focus_stop_handle =
        camera_server->subscribe_focus_stop([camera_server_weak](int32_t index) {
            auto server = camera_server_weak.lock();
            if (!server) {
                return;
            }
            ...
            server->respond_focus_stop(CameraServer::CameraFeedback::Ok);
        });

    auto last_focus_level = std::make_shared<std::atomic<float>>(0.0f);
    auto focus_range_handle = camera_server->subscribe_focus_range(
        [camera_server_weak, last_focus_level](float focus_level) {
            auto server = camera_server_weak.lock();
            if (!server) {
                return;
            }
            ...
            *last_focus_level = focus_level;
            server->respond_focus_range(CameraServer::CameraFeedback::Ok);
        });

    auto last_focus_distance_m = std::make_shared<std::atomic<float>>(0.0f);
    auto focus_meters_handle = camera_server->subscribe_focus_meters(
        [camera_server_weak, last_focus_distance_m](float focus_distance_m) {
            auto server = camera_server_weak.lock();
            if (!server) {
                return;
            }
            ...
            *last_focus_distance_m = focus_distance_m;
            server->respond_focus_meters(CameraServer::CameraFeedback::Ok);
        });

    auto focus_auto_handle =
        camera_server->subscribe_focus_auto([camera_server_weak](int32_t index) {
            auto server = camera_server_weak.lock();
            if (!server) {
                return;
            }
            ...
            server->respond_focus_auto(CameraServer::CameraFeedback::Ok);
        });

    auto focus_auto_single_handle =
        camera_server->subscribe_focus_auto_single([camera_server_weak](int32_t index) {
            auto server = camera_server_weak.lock();
            if (!server) {
                return;
            }
            ...
            server->respond_focus_auto_single(CameraServer::CameraFeedback::Ok);
        });

    auto focus_auto_continuous_handle =
        camera_server->subscribe_focus_auto_continuous([camera_server_weak](int32_t index) {
            auto server = camera_server_weak.lock();
            if (!server) {
                return;
            }
            ...
            server->respond_focus_auto_continuous(CameraServer::CameraFeedback::Ok);
        });
