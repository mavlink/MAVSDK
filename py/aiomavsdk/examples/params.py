...
async def get_all(param: ParamAsync):
    all_params = await param.get_all_params()
...
async def get_param(param: ParamAsync, name: str):
    print("Try as float param...", end="")
    try:
        float_value = await param.get_param_float(name)
        print("Ok")
    ...
    try:
        int_value = await param.get_param_int(name)
        print("Ok")
...
async def set_param(param: ParamAsync, name: str, value: str):
    try:
        int_value = int(value)
        print(f"Setting {value} as integer...", end="")
        try:
            await param.set_param_int(name, int_value)
            print("Ok")
            return
        except Exception as e:
            if "WRONG_TYPE" not in str(e):
    ...
    try:
        ...
        try:
            await param.set_param_float(name, float_value)
            print("Ok")
            return
        except Exception as e:
            print(f"Failed: {e}")
...
async def main():
    ...
    await mavsdk.add_any_connection(args["connection"])
    ...
    param = ParamAsync(drone)

    if args["action"] == "get_all":
        await get_all(param)
    elif args["action"] == "get":
        await get_param(param, args["param_name"])
    elif args["action"] == "set":
        await set_param(param, args["param_name"], args["value"])
