...
#if !( \
    defined(_WINSOCKAPI_) || defined(_WINSOCK_H) || defined(__LWIP_OPT_H__) || \
    defined(LWIP_HDR_OPT_H))
...
/* HP-UX systems version 9, 10 and 11 lack sys/select.h and so does oldish
   ...
   require it! */
    ...
    defined(__SYMBIAN32__) || defined(__INTEGRITY) || defined(ANDROID) || defined(__ANDROID__) || \
...
typedef CURLcode (*curl_ssl_ctx_callback)(
...
curl_mime_headers(curl_mimepart* part, struct curl_slist* headers, int take_ownership);
...
curl_formadd(struct curl_httppost** httppost, struct curl_httppost** last_post, ...);
...
CURL_EXTERN CURLsslset
curl_global_sslset(curl_sslbackend id, const char* name, const curl_ssl_backend*** avail);
...
/* the typechecker doesn't work in C++ (yet) */
    ...
    ((__GNUC__ > 4) || (__GNUC__ == 4 && __GNUC_MINOR__ >= 3)) && !defined(__cplusplus) && \
    !defined(CURL_DISABLE_TYPECHECK)
