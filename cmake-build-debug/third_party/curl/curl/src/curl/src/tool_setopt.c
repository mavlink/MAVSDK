    ...
    long lval)
{
    ...
    if (config->libcurl && !skip && !ret) {
        ...
        const NameValue* nv = NULL;
        msnprintf(preamble, sizeof(preamble), "curl_easy_setopt(hnd, %s, ", name);
        for (nv = nvlist; nv->name; nv++) {
            if ((nv->value & ~rest) == 0) {
                /* all value flags contained in rest */
                rest &= ~nv->value; /* remove bits handled here */
                CODE3("%s(long)%s%s", preamble, nv->name, rest ? " |" : ");");
                if (!rest)
...
static CURLcode libcurl_generate_slist(struct curl_slist* slist, int* slistno)
{
    ...
    *slistno = ++easysrc_slist_count;
    ...
    for (; slist; slist = slist->next) {
        ...
        DATA3("slist%d = curl_slist_append(slist%d, \"%s\");", *slistno, *slistno, escaped);
    }
...
static CURLcode libcurl_generate_mime(
    CURL* curl, struct GlobalConfig* config, tool_mime* toolmime, int* mimeno); /* Forward. */

/* Wrapper to generate source code for a mime part. */
static CURLcode
libcurl_generate_mime_part(CURL* curl, struct GlobalConfig* config, tool_mime* part, int mimeno)
{
    CURLcode ret = CURLE_OK;
    int submimeno = 0;
    char* escaped = NULL;
    ...
    switch (part->kind) {
        ...
        case TOOLMIME_DATA:
            ...
            if (!ret) {
                ...
                NULL_CHECK(escaped);
                CODE2("curl_mime_data(part%d, \"%s\", CURL_ZERO_TERMINATED);", mimeno, escaped);
            }
            break;
        ...
        case TOOLMIME_STDINDATA:
            ...
            CODE1("curl_mime_data_cb(part%d, -1, (curl_read_callback) fread, \\", mimeno);
...
static CURLcode
libcurl_generate_mime(CURL* curl, struct GlobalConfig* config, tool_mime* toolmime, int* mimeno)
{
    CURLcode ret = CURLE_OK;
    ...
    *mimeno = ++easysrc_mime_count;
    ...
    if (toolmime->subparts) {
        ...
        ret = libcurl_generate_mime_part(curl, config, toolmime->subparts, *mimeno);
    }
...
CURLcode tool_setopt_mimepost(
    CURL* curl, struct GlobalConfig* config, const char* name, CURLoption tag, curl_mime* mimepost)
{
...
CURLcode tool_setopt(
    CURL* curl, bool str, struct GlobalConfig* config, const char* name, CURLoption tag, ...)
{
    va_list arg;
    char buf[256];
    ...
    if (tag < CURLOPTTYPE_OBJECTPOINT) {
        ...
        msnprintf(buf, sizeof(buf), "(curl_off_t)%" CURL_FORMAT_CURL_OFF_T, oval);
        value = buf;
