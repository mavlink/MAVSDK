...
#endif
    void
    parse_cert_parameter(const char* cert_parameter, char** certname, char** passphrase)
{
        ...
        switch (*param_place) {
            ...
            case ':':
                /* Since we live in a world of weirdness and confusion, the win32
                   ...
                   windows. */
#ifdef WIN32
                if (param_place && (param_place == &cert_parameter[1]) &&
 ...
 */
static ParameterError GetSizeParameter(
    struct GlobalConfig* global, const char* arg, const char* which, curl_off_t* value_out)
{
    char* unit;
    curl_off_t value;
...
ParameterError getparameter(
    const char* flag, /* f or -long-flag */
    ...
    do {
        ...
        switch (letter) {
            ...
            case 'I': /* --head */
                ...
                if (SetHTTPrequest(
                        config, (config->no_body) ? HTTPREQ_HEAD : HTTPREQ_GET, &config->httpreq))
                    return PARAM_BAD_USE;
                break;
...
ParameterError parse_args(struct GlobalConfig* global, int argc, argv_item_t argv[])
{
    ...
    for (i = 1, stillflags = TRUE; i < argc && !result; i++) {
        ...
        if (stillflags && ('-' == argv[i][0])) {
            ...
            result = getparameter((char*)"--url", argv[i], &used, global, config);
        }
    }

    if (result && result != PARAM_HELP_REQUESTED && result != PARAM_MANUAL_REQUESTED &&
        result != PARAM_VERSION_INFO_REQUESTED && result != PARAM_ENGINES_REQUESTED) {
