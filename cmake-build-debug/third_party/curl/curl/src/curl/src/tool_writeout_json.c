...
static int writeTime(FILE* str, CURL* curl, const char* key, CURLINFO ci)
{
    ...
    if (CURLE_OK == curl_easy_getinfo(curl, ci, &val)) {
        ...
        fprintf(str, "\"%s\":%" CURL_FORMAT_CURL_OFF_T ".%06" CURL_FORMAT_CURL_OFF_T, key, s, ms);
        return 1;
    }
    return 0;
}
...
{
    long version = 0;
    if (CURLE_OK == curl_easy_getinfo(curl, ci, &version) && (version >= 0) &&
...
void ourWriteOutJSON(
    const struct writeoutvar mappings[], CURL* curl, struct OutStruct* outs, FILE* stream)
{
    int i;
