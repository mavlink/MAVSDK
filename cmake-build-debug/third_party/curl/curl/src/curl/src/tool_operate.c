...
static CURLcode single_transfer(
...
static CURLcode post_per_transfer(
    struct GlobalConfig* global, struct per_transfer* per, CURLcode result, bool* retryp)
{
    ...
    *retryp = FALSE;
    ...
    if (is_vms_shell()) {
        ...
        fprintf(
            ...
            "curl: (%d) %s\n",
            result,
            (per->errorbuffer[0]) ? per->errorbuffer : curl_easy_strerror(result));
    ...
    /* if retry-max-time is non-zero, make sure we haven't exceeded the
       time */
    if (per->retry_numretries && (!config->retry_maxtime || (tvdiff(tvnow(), per->retrystart) <
        ...
        long response;
        if ((CURLE_OPERATION_TIMEDOUT == result) || (CURLE_COULDNT_RESOLVE_HOST == result) ||
            (CURLE_COULDNT_RESOLVE_PROXY == result) || (CURLE_FTP_ACCEPT_TIMEOUT == result))
            ...
            (CURLE_OK == result) ||
            (config->failonerror && (CURLE_HTTP_RETURNED_ERROR == result))) {
        ...
        if (retry) {
            ...
            if (outs->bytes && outs->filename && outs->stream) {
                ...
                if (rc) {
                    if (!global->mute)
                        fprintf(global->errors, "failed seeking to end of file, exiting\n");
                    return CURLE_WRITE_ERROR;
                }
        ...
        if (CURLE_OK == result) {
            ...
            if (effective_url && curl_strnequal(effective_url, "http", 4)) {
                /* This was HTTP(S) */
                curl_easy_getinfo(curl, CURLINFO_RESPONSE_CODE, &response);
    ...
    *added = FALSE; /* not yet */
    ...
    while (config->state.urlnode) {
        ...
        if (!config->globoff && infiles && !inglob) {
            ...
            result = glob_url(
                &inglob, infiles, &state->infilenum, global->showerror ? global->errors : NULL);
            if (result)
                break;
...

        {
            ...
            separator = ((!state->outfiles || !strcmp(state->outfiles, "-")) && urlnum > 1);
                ...
                *added = TRUE;
                ...
                if (config->etag_compare_file) {
                    ...
                    if (!header) {
                        if (file)
                            fclose(file);
                        errorf(
                            config->global, "Failed to allocate memory for custom etag header\n");
                        result = CURLE_OUT_OF_MEMORY;
                        break;
                    }
                ...
                if (((urlnode->flags & GETOUT_USEREMOTE) ||
                    ...
                    if (config->resume_from) {
                        ...
                        FILE* file =
                            fopen(outfile, "ab", "ctx=stm", "rfm=stmlf", "rat=cr", "mrs=0");
#else
                        /* open file for output: */
                ...
                if (curlinfo->features & CURL_VERSION_SSL) {
                    ...
                    my_setopt_enum(
                        curl, CURLOPT_SSLVERSION, config->ssl_version | config->ssl_version_max);
                ...
                if ((global->progressmode == CURL_PROGRESS_BAR) && !global->noprogress &&
                    !global->mute) {
                ...
                my_setopt(
                    ...
                    (long)(config->ftp_create_dirs ? CURLFTP_CREATE_DIR_RETRY : CURLFTP_CREATE_DIR_NONE));
                ...
                /* curl 7.15.5 */
                my_setopt_str(
                    curl, CURLOPT_FTP_ALTERNATIVE_TO_USER, config->ftp_alternative_to_user);
                ...
                /* curl 7.69.x */
                my_setopt(
                    curl, CURLOPT_MAIL_RCPT_ALLLOWFAILS, config->mail_rcpt_allowfails ? 1L : 0L);
                ...
                if (curlinfo->features & CURL_VERSION_TLSAUTH_SRP) {
                    ...
                    if (config->proxy_tls_username)
                        my_setopt_str(
                            curl, CURLOPT_PROXY_TLSAUTH_USERNAME, config->proxy_tls_username);
                    if (config->proxy_tls_password)
                        my_setopt_str(
                            curl, CURLOPT_PROXY_TLSAUTH_PASSWORD, config->proxy_tls_password);
                ...
                if (config->proxy_ssl_allow_beast)
                    my_setopt(curl, CURLOPT_PROXY_SSL_OPTIONS, (long)CURLSSLOPT_ALLOW_BEAST);
                ...
                if (config->happy_eyeballs_timeout_ms != CURL_HET_DEFAULT)
                    my_setopt(
                        curl, CURLOPT_HAPPY_EYEBALLS_TIMEOUT_MS, config->happy_eyeballs_timeout_ms);
                ...
                per->retry_sleep_default = (config->retry_delay) ? config->retry_delay * 1000L :
                                                                   RETRY_SLEEP_DEFAULT; /* ms */
 ...
 */
static CURLcode add_parallel_transfers(
    struct GlobalConfig* global, CURLM* multi, CURLSH* share, bool* morep, bool* addedp)
{
    struct per_transfer* per;
    CURLcode result = CURLE_OK;
    CURLMcode mcode;
    ...
    *morep = FALSE;
    ...
    if (result)
        return result;
    for (per = transfers; per && (all_added < global->parallel_max); per = per->next) {
        bool getadded = FALSE;
        if (per->added)
            /* already added */
            continue;
...
static CURLcode parallel_transfers(struct GlobalConfig* global, CURLSH* share)
{
    ...
    while (!mcode && (still_running || more_transfers)) {
        ...
        if (!mcode) {
            ...
            if (removed) {
                ...
                (void)add_parallel_transfers(
                    global, multi, share, &more_transfers, &added_transfers);
                if (added_transfers)
    ...
    if (mcode) {
        result = (mcode == CURLM_OUT_OF_MEMORY) ? CURLE_OUT_OF_MEMORY :
                                                  /* The other multi errors should never happen, so
                                                     return something suitably generic */
...
static CURLcode serial_transfers(struct GlobalConfig* global, CURLSH* share)
{
    ...
    for (per = transfers; per;) {
        ...
        if (result || is_fatal_error(returncode) || (returncode && global->fail_early))
            bailout = TRUE;
        else {
...
static CURLcode transfer_per_config(
    struct GlobalConfig* global, struct OperationConfig* config, CURLSH* share, bool* added)
{
    CURLcode result = CURLE_OK;
    bool capath_from_env;
    *added = FALSE;
    ...
    if (!config->cacert && !config->capath && !config->insecure_ok) {
        ...
        if (tls_backend_info->backend != CURLSSLBACKEND_SCHANNEL) {
            ...
            if (env)
                ...
                result = FindWin32CACert(config, tls_backend_info->backend, "curl-ca-bundle.crt");
            }
#endif
        }
...
CURLcode operate(struct GlobalConfig* global, int argc, argv_item_t argv[])
{
    ...
    if ((argc == 1) || (!curl_strequal(argv[1], "-q") && !curl_strequal(argv[1], "--disable"))) {
