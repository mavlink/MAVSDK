...
static void MD5_Update(MD5_CTX* ctx, const unsigned char* input, unsigned int inputLen)
{
...
static void SHA1_Update(SHA_CTX* ctx, const unsigned char* input, unsigned int inputLen)
{
...
static void SHA256_Update(SHA256_CTX* ctx, const unsigned char* input, unsigned int inputLen)
{
...
static void MD5_Update(MD5_CTX* pctx, const unsigned char* input, unsigned int input_len)
{
...
static void SHA1_Update(SHA_CTX* pctx, const unsigned char* input, unsigned int input_len)
{
    PK11_DigestOp(*pctx, input, input_len);
}
...
static void SHA256_Update(SHA256_CTX* pctx, const unsigned char* input, unsigned int input_len)
{
    PK11_DigestOp(*pctx, input, input_len);
}
...
static void
win32_crypto_final(struct win32_crypto_hash* ctx, unsigned char* digest, unsigned int digestLen)
{
...
static int MD5_Init(MD5_CTX* ctx)
{
    if (CryptAcquireContext(
            &ctx->hCryptProv, NULL, NULL, PROV_RSA_FULL, CRYPT_VERIFYCONTEXT | CRYPT_SILENT)) {
...
{
    if (CryptAcquireContext(
            &ctx->hCryptProv, NULL, NULL, PROV_RSA_AES, CRYPT_VERIFYCONTEXT | CRYPT_SILENT)) {
...
static const metalink_digest_def SHA256_DIGEST_DEF[] = {{"sha-256", SHA256_DIGEST_PARAMS}};

static const metalink_digest_def SHA1_DIGEST_DEF[] = {{"sha-1", SHA1_DIGEST_PARAMS}};

static const metalink_digest_def MD5_DIGEST_DEF[] = {{"md5", MD5_DIGEST_PARAMS}};
...
static int digest_update(digest_context* context, const unsigned char* data, unsigned int len)
{
    ...
    FILE* error)
{
    ...
    digest_final(dctx, result);
    check_ok = memcmp(result, digest, digest_def->dparams->digest_resultlen) == 0;
...
int metalink_check_hash(struct GlobalConfig* config, metalinkfile* mlfile, const char* filename)
{
    int rv;
    fprintf(config->errors, "Metalink: validating (%s)...\n", filename);
    if (mlfile->checksum == NULL) {
        fprintf(config->errors, "Metalink: validating (%s) FAILED (digest missing)\n", filename);
        return -2;
    }
    rv = check_hash(
        filename, mlfile->checksum->digest_def, mlfile->checksum->digest, config->errors);
    return rv;
}

static metalink_checksum*
checksum_from_hex_digest(const metalink_digest_def* digest_def, const char* hex_digest)
{
...
static int check_hex_digest(const char* hex_digest, const metalink_digest_def* digest_def)
{
    size_t i;
...
static metalinkfile* new_metalinkfile(metalink_file_t* fileinfo)
{
    ...
    if (fileinfo->checksums) {
        ...
        for (digest_alias = digest_aliases; digest_alias->alias_name; ++digest_alias) {
            ...
            for (p = fileinfo->checksums; *p; ++p) {
                if (curl_strequal(digest_alias->alias_name, (*p)->type) &&
                    ...
                    f->checksum = checksum_from_hex_digest(digest_alias->digest_def, (*p)->hash);
                    break;
                }
            }
            if (f->checksum) {
    ...
    if (fileinfo->resources) {
        ...
        for (p = fileinfo->resources; *p; ++p) {
            ...
            */
            if ((*p)->type == NULL || curl_strequal((*p)->type, "http") ||
                curl_strequal((*p)->type, "https") || curl_strequal((*p)->type, "ftp") ||
