   ...
   access to a 64-bit type we can bypass stat and get the times directly. */
    ...
    if (hfile != INVALID_HANDLE_VALUE) {
        ...
        if (GetFileTime(hfile, NULL, NULL, &ft)) {
            curl_off_t converted = (curl_off_t)ft.dwLowDateTime | ((curl_off_t)ft.dwHighDateTime)
                                                                      << 32;
    ...
    if (-1 != stat(filename, &statbuf)) {
        ...
        fprintf(error_stream, "Failed to get filetime: %s\n", strerror(errno));
    }
#endif
    return result;
}
...
void setfiletime(curl_off_t filetime, const char* filename, FILE* error_stream)
{
    if (filetime >= 0) {
/* Windows utime() may attempt to adjust the unix GMT file time by a daylight
   ...
   access to a 64-bit type we can bypass utime and set the times directly. */
        ...
        if (filetime > CURL_OFF_T_C(910670515199)) {
            fprintf(
                error_stream,
                "Failed to set filetime %" CURL_FORMAT_CURL_OFF_T " on outfile: overflow\n",
                filetime);
            return;
        }
        ...
        hfile = CreateFileA(
            ...
            NULL);
        if (hfile != INVALID_HANDLE_VALUE) {
            curl_off_t converted =
                ((curl_off_t)filetime * 10000000) + CURL_OFF_T_C(116444736000000000);
            FILETIME ft;
        ...
        if (utimes(filename, times)) {
            fprintf(
                error_stream,
                "Failed to set filetime %" CURL_FORMAT_CURL_OFF_T " on outfile: %s\n",
                filetime,
                strerror(errno));
        }
