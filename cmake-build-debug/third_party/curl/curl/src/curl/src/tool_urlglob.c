...
static CURLcode
glob_set(URLGlob* glob, char** patternp, size_t* posp, unsigned long* amount, int globindex)
{
    ...
    while (!done) {
        switch (*pattern) {
            ...
            case '}': /* set element completed */
                if (opattern == pattern)
                    return GLOBERROR("empty string within braces", *posp, CURLE_URL_MALFORMAT);
            ...
            case ',':
                ...
                *buf = '\0';
                if (pat->content.Set.elements) {
                    char** new_arr = realloc(
                        pat->content.Set.elements, (pat->content.Set.size + 1) * sizeof(char*));
                    if (!new_arr)
                        return GLOBERROR("out of memory", 0, CURLE_OUT_OF_MEMORY);
...
static CURLcode
glob_range(URLGlob* glob, char** patternp, size_t* posp, unsigned long* amount, int globindex)
{
    ...
    */
    ...
    if (ISALPHA(*pattern)) {
        ...
        *posp += (pattern - *patternp);
        ...
        if (rc != 3 || !step || step > (unsigned)INT_MAX || (min_c == max_c && step != 1) ||
            (min_c != max_c &&
             (min_c > max_c || step > (unsigned)(max_c - min_c) || (max_c - min_c) > ('z' - 'a'))))
        ...
        if (multiply(
                amount,
                ((pat->content.CharRange.max_c - pat->content.CharRange.min_c) /
    ...
    fail:
        *posp += (pattern - *patternp);
        ...
        if (!endp || !step_n || (min_n == max_n && step_n != 1) ||
        ...
        if (multiply(
                amount,
                ((pat->content.NumRange.max_n - pat->content.NumRange.min_n) /
    ...
    *amount = 1;
    ...
    while (*pattern && !res) {
        ...
        while (*pattern && *pattern != '{') {
            ...
            if (*pattern == '}' || *pattern == ']')
                return GLOBERROR("unmatched close brace/bracket", pos, CURLE_URL_MALFORMAT);
...
void glob_cleanup(URLGlob* glob)
{
    ...
    for (i = 0; i < glob->size; i++) {
        if ((glob->pattern[i].type == UPTSet) && (glob->pattern[i].content.Set.elements)) {
            for (elem = glob->pattern[i].content.Set.size - 1; elem >= 0; --elem) {
    ...
    *globbed = NULL;
    ...
    for (i = 0; i < glob->size; ++i) {
        ...
        switch (pat->type) {
            case UPTSet:
                if (pat->content.Set.elements) {
                    msnprintf(buf, buflen, "%s", pat->content.Set.elements[pat->content.Set.ptr_s]);
                    len = strlen(buf);
                    buf += len;
                    buflen -= len;
                }
...
CURLcode glob_match_url(char** result, char* filename, URLGlob* glob)
{
    ...
    *result = NULL;
        ...
        if (*filename == '#' && ISDIGIT(filename[1])) {
            ...
            if (pat) {
                switch (pat->type) {
                    ...
                    default:
                        fprintf(
                            stderr, "internal error: invalid pattern type (%d)\n", (int)pat->type);
...
#if defined(MSDOS) || defined(WIN32)
    {
        char* sanitized;
        SANITIZEcode sc =
            sanitize_file_name(&sanitized, target, (SANITIZE_ALLOW_PATH | SANITIZE_ALLOW_RESERVED));
        Curl_safefree(target);
        if (sc)
