...
#ifdef MSDOS
static SANITIZEcode msdosify(char** const sanitized, const char* file_name, int flags);
#endif
static SANITIZEcode
rename_if_reserved_dos_device_name(char** const sanitized, const char* file_name, int flags);
...
*/
SANITIZEcode sanitize_file_name(char** const sanitized, const char* file_name, int flags)
{
    char *p, *target;
    size_t len;
    SANITIZEcode sc;
...
SANITIZEcode msdosify(char** const sanitized, const char* file_name, int flags)
{
    ...
    *sanitized = NULL;
    ...
    if (strlen(file_name) > PATH_MAX - 1 &&
        (!(flags & SANITIZE_ALLOW_TRUNCATE) || truncate_dryrun(file_name, PATH_MAX - 1)))
        return SANITIZE_ERR_INVALID_PATH;
...
rename_if_reserved_dos_device_name(char** const sanitized, const char* file_name, int flags)
{
    ...
    *sanitized = NULL;
...
#ifndef MSDOS
    if ((flags & SANITIZE_ALLOW_PATH) && file_name[0] == '\\' && file_name[1] == '\\') {
    ...
    for (p = fname; p; p = (p == fname && fname != base ? base : NULL)) {
        size_t p_len;
        int x = (curl_strnequal(p, "CON", 3) || curl_strnequal(p, "PRN", 3) ||
                 curl_strnequal(p, "AUX", 3) || curl_strnequal(p, "NUL", 3)) ?
                    3 :
                    (curl_strnequal(p, "CLOCK$", 6)) ?
                    6 :
...
CURLcode
FindWin32CACert(struct OperationConfig* config, curl_sslbackend backend, const char* bundle_file)
{
     ...
     */
    if ((curlinfo->features & CURL_VERSION_SSL) && backend != CURLSSLBACKEND_SCHANNEL) {
        DWORD res_len;
        char buf[PATH_MAX];
        char* ptr = NULL;
...
struct curl_slist* GetLoadedModulePaths(void)
{
    ...
    do {
        ...
        if (!WideCharToMultiByte(CP_ACP, 0, mod.szExePath, -1, buffer, sizeof(buffer), NULL, NULL))
            goto error;
        path = buffer;
#else
        path = mod.szExePath;
