...
int tool_debug_cb(CURL* handle, curl_infotype type, char* data, size_t size, void* userdata)
{
    ...
    if ((type == CURLINFO_HEADER_OUT) && (size > 4)) {
        ...
        for (i = 0; i < size - 4; i++) {
            if (memcmp(&data[i], "\r\n\r\n", 4) == 0) {
                ...
                text = "=> Send header";
                dump(timebuf, text, output, (unsigned char*)data, i + 4, config->tracetype, type);
                data += i + 3;
                size -= i + 4;
    ...
    dump(timebuf, text, output, (unsigned char*)data, size, config->tracetype, type);
    return 0;
}
    ...
    curl_infotype infotype)
{
    ...
    for (i = 0; i < size; i += width) {
        ...
        for (c = 0; (c < width) && (i + c < size); c++) {
            ...
            if ((tracetype == TRACE_ASCII) && (i + c + 1 < size) && (ptr[i + c] == 0x0D) &&
                (ptr[i + c + 1] == 0x0A)) {
                i += (c + 2 - width);
                break;
            }
            ...
            if ((tracetype == TRACE_ASCII) && (i + c + 1 < size) && (ptr[i + c] == '\r') &&
                (ptr[i + c + 1] == '\n')) {
                i += (c + 2 - width);
                break;
            }
            ...
            if ((tracetype == TRACE_ASCII) && (i + c + 2 < size) && (ptr[i + c + 1] == 0x0D) &&
                (ptr[i + c + 2] == 0x0A)) {
                i += (c + 3 - width);
                break;
            }
