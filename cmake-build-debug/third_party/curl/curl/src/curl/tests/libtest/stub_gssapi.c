    ...
    OM_uint32* time_rec)
{
    ...
    *min = 0;
    ...
    /* Note: this is using the *real* snprintf() and not the curl provided
       one */
    used = snprintf(token, length, "%s:%s:%d:", creds, (char*)target_name, ctx->sent);
...
OM_uint32
gss_delete_sec_context(OM_uint32* min, gss_ctx_id_t* context_handle, gss_buffer_t output_token)
{
    (void)output_token;
