...
static void store_errmsg(const char* msg, int err)
{
    if (!err)
        ...
        msnprintf(msgbuff, sizeof(msgbuff), "%s, errno %d, %s", msg, err, strerror(err));
}
...
static int rlimit(int keep_open)
{
...
#ifdef OPEN_MAX
        if ((rl.rlim_cur > 0) && (rl.rlim_cur < OPEN_MAX)) {
    ...
    if ((rl.rlim_cur > 0) &&
        ...
        msnprintf(strbuff, sizeof(strbuff), "fds needed %s > system limit %s", strbuff1, strbuff2);
    ...
    for (num_open.rlim_cur = 1; num_open.rlim_cur < num_open.rlim_max; num_open.rlim_cur++) {
        ...
        if (fd[num_open.rlim_cur] < 0) {
            ...
            msnprintf(strbuff, sizeof(strbuff), "fds system limit seems close to %s", strbuff1);
            fprintf(stderr, "%s\n", strbuff);
            ...
            msnprintf(strbuff1, sizeof(strbuff1), fmt, num_open.rlim_cur);
            msnprintf(
                strbuff, sizeof(strbuff), "fds needed %s > system limit %s", strbuff2, strbuff1);
            store_errmsg(strbuff, 0);
            fprintf(stderr, "%s\n", msgbuff);

            for (num_open.rlim_cur = 0; fd[num_open.rlim_cur] >= 0; num_open.rlim_cur++)
                close(fd[num_open.rlim_cur]);
            free(fd);
            fd = NULL;
            free(memchunk);
    ...
    if (num_open.rlim_max > num_open.rlim_cur) {
        msnprintf(strbuff, sizeof(strbuff), "select limit is FD_SETSIZE %d", FD_SETSIZE);
        store_errmsg(strbuff, 0);
        fprintf(stderr, "%s\n", msgbuff);
    ...
    num_open.rlim_cur = FD_SETSIZE - SAFETY_MARGIN;
    for (rl.rlim_cur = 0; rl.rlim_cur < num_open.rlim_max; rl.rlim_cur++) {
        if ((fd[rl.rlim_cur] > 0) && ((unsigned int)fd[rl.rlim_cur] > num_open.rlim_cur)) {
            msnprintf(strbuff, sizeof(strbuff), "select limit is FD_SETSIZE %d", FD_SETSIZE);
            store_errmsg(strbuff, 0);
            fprintf(stderr, "%s\n", msgbuff);
            close_file_descriptors();
    ...
    if (!fopen_works()) {
        msnprintf(strbuff1, sizeof(strbuff1), fmt, num_open.rlim_max);
        msnprintf(strbuff, sizeof(strbuff), "fopen fails with %s fds open()", strbuff1);
        fprintf(stderr, "%s\n", msgbuff);
