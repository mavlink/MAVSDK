...
static int checkparts(CURLU* u, const char* in, const char* wanted, unsigned int getflags)
{
    int i;
    CURLUcode rc;
    char buf[256];
...
static struct testcase get_parts_list[] = {
    ...
    {"https://example.#com/color/?green#no-black",
     ...
     0,
     CURLUE_OK},
    {"http://[ab.be:1]/x", "", CURLU_DEFAULT_SCHEME, 0, CURLUE_MALFORMED_INPUT},
    {"http://[ab.be]/x", "", CURLU_DEFAULT_SCHEME, 0, CURLUE_MALFORMED_INPUT},
    /* URL without host name */
    {"http://a:b@/x", "", CURLU_DEFAULT_SCHEME, 0, CURLUE_NO_HOST},
    {"boing:80",
    ...
    {"http://HO0_-st%41/",
     ...
     0,
     CURLUE_OK},
    {"file://hello.html", "", 0, 0, CURLUE_MALFORMED_INPUT},
    {"http://HO0_-st/",
    ...
    {"https://127abc.com",
     ...
     0,
     CURLUE_OK},
    {"https:// example.com?check", "", CURLU_DEFAULT_SCHEME, 0, CURLUE_MALFORMED_INPUT},
    {"https://e x a m p l e.com?check", "", CURLU_DEFAULT_SCHEME, 0, CURLUE_MALFORMED_INPUT},
    {"https://example.com?check",
     "https | [11] | [12] | [13] | example.com | [15] | / | check | [17]",
     CURLU_DEFAULT_SCHEME,
     0,
     CURLUE_OK},
    {"https://example.com:65536", "", CURLU_DEFAULT_SCHEME, 0, CURLUE_BAD_PORT_NUMBER},
    {"https://example.com:0#moo", "", CURLU_DEFAULT_SCHEME, 0, CURLUE_BAD_PORT_NUMBER},
...
static struct urltestcase get_url_list[] = {
    ...
    {"AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA://hostname/path",
     ...
     0,
     CURLUE_MALFORMED_INPUT},
    {"https://[fe80::20c:29ff:fe9c:409b%]:1234", "", 0, 0, CURLUE_MALFORMED_INPUT},
    ...
    {"https://[fe80::20c:29ff:fe9c:409b%eth0]:1234",
     ...
     0,
     CURLUE_OK},
    {"https://[::%25fakeit]/moo", "https://[::%25fakeit]/moo", 0, 0, CURLUE_OK},
    ...
    {"ftp.example.com/path/html",
     ...
     0,
     CURLUE_OK},
    {"example.com/path/html", "http://example.com/path/html", CURLU_GUESS_SCHEME, 0, CURLUE_OK},
    ...
    {"htt\x02://host/file", "", CURLU_NON_SUPPORT_SCHEME, 0, CURLUE_MALFORMED_INPUT},
    {" http://host/file", "", 0, 0, CURLUE_MALFORMED_INPUT},
    /* here the password ends at the semicolon and options is 'word' */
    {"imap://user:pass;word@host/file", "imap://user:pass;word@host/file", 0, 0, CURLUE_OK},
    /* here the password has the semicolon */
    {"http://user:pass;word@host/file", "http://user:pass;word@host/file", 0, 0, CURLUE_OK},
    {"file:///file.txt#moo", "file:///file.txt#moo", 0, 0, CURLUE_OK},
    {"file:////file.txt", "file:////file.txt", 0, 0, CURLUE_OK},
    {"file:///file.txt", "file:///file.txt", 0, 0, CURLUE_OK},
    {"file:./", "file://", 0, 0, CURLUE_MALFORMED_INPUT},
    {"http://example.com/hello/../here",
     "http://example.com/hello/../here",
     CURLU_PATH_AS_IS,
     0,
     CURLUE_OK},
    {"http://example.com/hello/../here", "http://example.com/here", 0, 0, CURLUE_OK},
    {"http://example.com:80", "http://example.com/", 0, CURLU_NO_DEFAULT_PORT, CURLUE_OK},
    {"tp://example.com/path/html", "", 0, 0, CURLUE_UNSUPPORTED_SCHEME},
    {"http://hello:fool@example.com", "", CURLU_DISALLOW_USER, 0, CURLUE_USER_NOT_ALLOWED},
    {"http:/@example.com:123", "http://example.com:123/", 0, 0, CURLUE_OK},
    {"http:/:password@example.com", "http://:password@example.com/", 0, 0, CURLUE_OK},
    {"http://user@example.com?#", "http://user@example.com/", 0, 0, CURLUE_OK},
    {"http://user@example.com?", "http://user@example.com/", 0, 0, CURLUE_OK},
    {"http://user@example.com#anchor", "http://user@example.com/#anchor", 0, 0, CURLUE_OK},
    {"example.com/path/html", "https://example.com/path/html", CURLU_DEFAULT_SCHEME, 0, CURLUE_OK},
    {"example.com/path/html", "", 0, 0, CURLUE_MALFORMED_INPUT},
    {"http://user:password@example.com:1234/path/html?query=name#anchor",
     "http://user:password@example.com:1234/path/html?query=name#anchor",
     0,
     0,
    ...
    {"http://example.com/path/html?query=name",
     ...
     0,
     CURLUE_OK},
    {"http://example.com/path/html", "http://example.com/path/html", 0, 0, CURLUE_OK},
    ...
    {"custom-scheme://host?expected=test-good",
     ...
     0,
     CURLUE_OK},
    {"custom-scheme://?expected=test-bad", "", CURLU_NON_SUPPORT_SCHEME, 0, CURLUE_MALFORMED_INPUT},
...
static struct setcase set_parts_list[] = {
    ...
    {"https://[::1%25fake]:1234/",
     ...
     CURLUE_OK,
     CURLUE_OK},
    {"https://host:1234/", "port=NULL,", "https://host/", 0, 0, CURLUE_OK, CURLUE_OK},
    {"https://host:1234/",
     "port=\"\",",
     "https://host:1234/",
     0,
    ...
    {"https://host:1234/",
     ...
     CURLUE_OK,
     CURLUE_BAD_PORT_NUMBER},
    {"https://host:1234/", "port=65535,", "https://host:65535/", 0, 0, CURLUE_OK, CURLUE_OK},
    {"https://host:1234/",
     "port=65536,",
     "https://host:1234/",
     0,
     ...
     CURLUE_OK,
     CURLUE_BAD_PORT_NUMBER},
    {"https://host/", "path=%4A%4B%4C,", "https://host/%4a%4b%4c", 0, 0, CURLUE_OK, CURLUE_OK},
    {"https://host/mooo?q#f",
    ...
    {"http://example.com",
     ...
     CURLUE_OK,
     CURLUE_OK},
    {"http://example.com:80", "scheme=ftp,", "ftp://example.com:80/", 0, 0, CURLUE_OK, CURLUE_OK},
    {"custom-scheme://host",
     "host=\"\",",
     "custom-scheme://host/",
     CURLU_NON_SUPPORT_SCHEME,
...
static struct redircase set_url_list[] = {
    {"file://localhost/path?query#frag", "foo#another", "file:///foo#another", 0, 0, 0},
...
static int set_url(void)
{
    ...
    for (i = 0; set_url_list[i].in && !error; i++) {
        ...
        if (!urlp)
            break;
        rc = curl_url_set(urlp, CURLUPART_URL, set_url_list[i].in, set_url_list[i].urlflags);
        if (!rc) {
            rc = curl_url_set(urlp, CURLUPART_URL, set_url_list[i].set, set_url_list[i].setflags);
            if (rc) {
                fprintf(
                    stderr,
                    "%s:%d Set URL %s returned %d\n",
                    __FILE__,
                    __LINE__,
                    set_url_list[i].set,
                    (int)rc);
...
static int set_parts(void)
{
    ...
    for (i = 0; set_parts_list[i].set && !error; i++) {
        ...
        if (set_parts_list[i].in)
            rc =
                curl_url_set(urlp, CURLUPART_URL, set_parts_list[i].in, set_parts_list[i].urlflags);
        else
            rc = CURLUE_OK;
        if (!rc) {
            char* url = NULL;
            CURLUcode uc = updateurl(urlp, set_parts_list[i].set, set_parts_list[i].setflags);
...
static int get_url(void)
{
    ...
    for (i = 0; get_url_list[i].in && !error; i++) {
        ...
        rc = curl_url_set(urlp, CURLUPART_URL, get_url_list[i].in, get_url_list[i].urlflags);
        if (!rc) {
            char* url = NULL;
...
static int get_parts(void)
{
    ...
    for (i = 0; get_parts_list[i].in && !error; i++) {
        ...
        rc = curl_url_set(urlp, CURLUPART_URL, get_parts_list[i].in, get_parts_list[i].urlflags);
...
static struct querycase append_list[] = {
    {"HTTP://test/?s", "name=joe\x02", "http://test/?s&name=joe%02", 0, CURLU_URLENCODE, CURLUE_OK},
    {"HTTP://test/?size=2#f",
     "name=joe=",
    ...
    {"HTTP://test/?size=2", "name=joe", "http://test/?size=2&name=joe", 0, 0, CURLUE_OK},
    {"HTTP://test/?size=2&", "name=joe", "http://test/?size=2&name=joe", 0, 0, CURLUE_OK},
    {"HTTP://test/?size=2#f", "name=joe", "http://test/?size=2&name=joe#f", 0, 0, CURLUE_OK},
    {NULL, NULL, NULL, 0, 0, 0}};
...
static int append(void)
{
    ...
    for (i = 0; append_list[i].in && !error; i++) {
        ...
        rc = curl_url_set(urlp, CURLUPART_URL, append_list[i].in, append_list[i].urlflags);
        if (rc)
            error++;
        else
            rc = curl_url_set(
                urlp, CURLUPART_QUERY, append_list[i].q, append_list[i].qflags | CURLU_APPENDQUERY);
        if (error)
            ;
...
static int scopeid(void)
{
    ...
    if (rc != CURLUE_OK) {
        fprintf(
            stderr, "%s:%d curl_url_get CURLUPART_HOST returned %d\n", __FILE__, __LINE__, (int)rc);
        error++;
    } else {
        printf("we got %s\n", url);
        curl_free(url);
    ...
    if (rc != CURLUE_OK) {
        fprintf(
            stderr, "%s:%d curl_url_set CURLUPART_HOST returned %d\n", __FILE__, __LINE__, (int)rc);
        error++;
    }
    ...
    if (rc != CURLUE_OK) {
        fprintf(
            stderr, "%s:%d curl_url_get CURLUPART_URL returned %d\n", __FILE__, __LINE__, (int)rc);
        error++;
    } else {
        printf("we got %s\n", url);
        curl_free(url);
    ...
    if (rc != CURLUE_OK) {
        fprintf(
            stderr,
            "%s:%d curl_url_get CURLUPART_ZONEID returned %d\n",
            __FILE__,
            __LINE__,
            (int)rc);
        error++;
    ...
    if (rc != CURLUE_OK) {
        fprintf(
            stderr,
            "%s:%d curl_url_set CURLUPART_ZONEID returned %d\n",
            __FILE__,
            __LINE__,
            (int)rc);
        error++;
