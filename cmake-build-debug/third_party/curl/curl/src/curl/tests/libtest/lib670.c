...
static int
xferinfo(void* clientp, curl_off_t dltotal, curl_off_t dlnow, curl_off_t ultotal, curl_off_t ulnow)
{
...
int test(char* URL)
{
    ...
    if (!result)
        res = curl_mime_data_cb(part, (curl_off_t)2, read_callback, NULL, NULL, &pooh);

    if (result) {
        fprintf(stderr, "Something went wrong when building the mime structure: %d\n", (int)result);
        goto test_cleanup;
    }
