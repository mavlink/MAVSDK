...
static curl_socket_t tst_opensocket(void* clientp, curlsocktype purpose, struct curl_sockaddr* addr)
{
    (void)clientp;
    (void)purpose;
    ...
    if (!res) {
        ...
        if (libtest_arg2) {
            ...
            if (moo) {
                ...
                curl_easy_getinfo(curl, CURLINFO_PRETRANSFER_TIME_T, &time_pretransfer);
                curl_easy_getinfo(curl, CURLINFO_STARTTRANSFER_TIME_T, &time_starttransfer);
                ...
                if (time_namelookup > time_connect) {
                    fprintf(
                        moo,
                        "namelookup vs connect: %" CURL_FORMAT_CURL_OFF_T
                ...
                if (time_connect > time_pretransfer) {
                    fprintf(
                        moo,
                        "connect vs pretransfer: %" CURL_FORMAT_CURL_OFF_T
                        ".%06ld %" CURL_FORMAT_CURL_OFF_T ".%06ld\n",
                        (time_connect / 1000000),
                        (long)(time_connect % 1000000),
                ...
                if (time_pretransfer > time_starttransfer) {
                    fprintf(
                        moo,
                        "pretransfer vs starttransfer: %" CURL_FORMAT_CURL_OFF_T
                        ".%06ld %" CURL_FORMAT_CURL_OFF_T ".%06ld\n",
                        (time_pretransfer / 1000000),
                        (long)(time_pretransfer % 1000000),
                ...
                if (time_starttransfer > time_total) {
                    fprintf(
                        moo,
                        "starttransfer vs total: %" CURL_FORMAT_CURL_OFF_T
                        ".%06ld %" CURL_FORMAT_CURL_OFF_T ".%06ld\n",
                        (time_starttransfer / 1000000),
                        (long)(time_starttransfer % 1000000),
                        (time_total / 1000000),
