...
static void addFd(struct Sockets* sockets, curl_socket_t fd, const char* what)
{
    ...
    if (!sockets->sockets) {
        ...
        sockets->sockets = realloc(oldptr, sizeof(curl_socket_t) * (sockets->max_count + 20));
        if (!sockets->sockets) {
...
static int getMicroSecondTimeout(struct timeval* timeout)
{
    ...
    ssize_t result;
    now = tutil_tvnow();
    result = (ssize_t)((timeout->tv_sec - now.tv_sec) * 1000000 + timeout->tv_usec - now.tv_usec);
    if (result < 0)
        result = 0;
 ...
 */
static void
checkFdSet(CURLM* curl, struct Sockets* sockets, fd_set* fdset, int evBitmask, const char* name)
{
    int i;
    for (i = 0; i < sockets->count; ++i) {
