...
do {
    ...
    const char* toolong =
        ...
        "";
    const char* emptylabel = "this.is.an.otherwise-valid.hostname."
                             ".with.an.empty.label.";
    const char* outsizelabel = "this.is.an.otherwise-valid.hostname."
    ...
    for (i = 0; i < (int)(sizeof(playlist) / sizeof(*playlist)); i++) {
        ...
        fail_unless(d == playlist[i].expected_result, "result returned was not as expected");
        if (d == playlist[i].expected_result) {
            if (name == max) {
                fail_if(victim.canary1 == 87, "demo one-byte buffer overwrite did not happen");
            } else {
