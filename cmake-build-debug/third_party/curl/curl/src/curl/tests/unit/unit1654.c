...
UNITTEST_START
{
    ...
    result = Curl_altsvc_parse(curl, asi, "h2=\"example.com:8080\"", ALPN_h1, "example.org", 8080);
    if (result) {
    ...
    result = Curl_altsvc_parse(curl, asi, "h3=\":8080\"", ALPN_h1, "2.example.org", 8080);
    if (result) {
    ...
    result = Curl_altsvc_parse(
        curl, asi, "h2=\"example.com:8080\", h3=\"yesyes.com\"", ALPN_h1, "3.example.org", 8080);
    if (result) {
    ...
    result = Curl_altsvc_parse(
        curl, asi, "h2=\"example.com:443\"; ma = 120;", ALPN_h2, "example.org", 80);
    if (result) {
    ...
    /* quoted 'ma' value */
    result = Curl_altsvc_parse(
        curl, asi, "h2=\"example.net:443\"; ma=\"180\";", ALPN_h2, "example.net", 80);
    if (result) {
        fprintf(stderr, "Curl_altsvc_parse(4) failed!\n");
        unitfail++;
    }
    ...
    result = Curl_altsvc_parse(
        curl, asi, "h2=\":443\", h3=\":443\"; ma = 120; persist = 1", ALPN_h1, "curl.haxx.se", 80);
    if (result) {
    ...
    result = Curl_altsvc_parse(curl, asi, "clear;", ALPN_h1, "curl.haxx.se", 80);
    if (result) {
