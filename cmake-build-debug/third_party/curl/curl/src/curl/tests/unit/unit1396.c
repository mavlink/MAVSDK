...
UNITTEST_START
{
    ...
    for (i = 0; list1[i].in; i++) {
        int outlen;
        char* out = curl_easy_unescape(hnd, list1[i].in, list1[i].inlen, &outlen);

        abort_unless(out != NULL, "returned NULL!");
        fail_unless(outlen == list1[i].outlen, "wrong output length returned");
        fail_unless(!memcmp(out, list1[i].out, list1[i].outlen), "bad output data returned");
    ...
    for (i = 0; list2[i].in; i++) {
        ...
        fail_unless(!memcmp(out, list2[i].out, list2[i].outlen), "bad output data returned");
