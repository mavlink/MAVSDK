...
UNITTEST_START
{
    ...
    fail_unless(
        llist.dtor == test_curl_llist_dtor, "list dtor should initiate to test_curl_llist_dtor");
    ...
    fail_unless(
        llist.head->next->ptr == &unusedData_case3,
    ...
    fail_unless(
        llist.head->next->ptr == &unusedData_case2,
        "the node next to head is not getting set correctly");
    ...
    fail_unless(
        Curl_llist_count(&llist) == (llist_size - 1), "llist size not decremented as expected");
