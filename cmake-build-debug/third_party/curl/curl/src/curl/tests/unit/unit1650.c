...
static struct dohresp resp[] = {
    ...
    {"\x00\x01\x00\x01\x00\x01\x00\x01\x00\x01\x00\x01", 12, DNS_TYPE_A, DOH_DNS_BAD_ID, NULL},
    {"\x00\x00\x00\x01\x00\x01\x00\x01\x00\x01\x00\x01", 12, DNS_TYPE_A, DOH_DNS_BAD_RCODE, NULL},
    {"\x00\x00\x01\x00\x00\x01\x00\x01\x00\x00\x00\x00\x03\x66\x6f\x6f",
     16,
     DNS_TYPE_A,
    ...
    {"\x00\x00\x01\x00\x00\x01\x00\x01\x00\x01\x00\x00\x04\x61\x61\x61"
     "\x61\x07\x65\x78\x61\x6d\x70\x6c\x65\x03\x63\x6f\x6d\x00\x00\x1c"
     "\x00\x01\xc0\x0c\x00\x1c\x00\x01\x00\x00\x00\x37\x00\x10\x20\x20"
     "\x20\x20\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x20\x20" LABEL_TEST LABEL_HOST LABEL_NAME
         DNSAAAA_EPILOGUE "\x00\x00\x00\x01"
...
UNITTEST_START
{
    ...
    for (i = 0; i < sizeof(req) / sizeof(req[0]); i++) {
        int rc = doh_encode(req[i].name, req[i].type, buffer, sizeof(buffer), &size);
        if (rc != req[i].rc) {
    ...
    for (i = 0; i < sizeof(resp) / sizeof(resp[0]); i++) {
        ...
        int u;
        memset(&d, 0, sizeof(d));
        rc = doh_decode((const unsigned char*)resp[i].packet, resp[i].size, resp[i].type, &d);
        if (rc != resp[i].rc) {
        ...
        for (u = 0; u < d.numaddr; u++) {
            ...
            if (resp[i].type == DNS_TYPE_A) {
                ...
                for (j = 0; j < 16; j += 2) {
                    size_t l;
                    msnprintf(ptr, len, "%s%02x%02x", j ? ":" : "", a->ip.v6[j], a->ip.v6[j + 1]);
                    l = strlen(ptr);
                    len -= l;
                    ptr += l;
                }
...

    {
        ...
        for (i = 1; i < sizeof(full49); i++) {
            ...
            memset(&d, 0, sizeof(d));
            rc = doh_decode(
                (const unsigned char*)&full49[i], sizeof(full49) - i - 1, DNS_TYPE_A, &d);
            if (!rc) {
                /* none of them should work */
        ...
        }
        {
            ...
            struct dohaddr* a;
            memset(&d, 0, sizeof(d));
            rc = doh_decode((const unsigned char*)full49, sizeof(full49) - 1, DNS_TYPE_A, &d);
            fail_if(d.numaddr != 1, "missing address");
            a = &d.addr[0];
            p = &a->ip.v4[0];
            msnprintf((char*)buffer, sizeof(buffer), "%u.%u.%u.%u", p[0], p[1], p[2], p[3]);
