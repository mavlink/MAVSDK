...
UNITTEST_START
{
    ...
    for (i = 0; i < testnum; ++i) {
        ...
        for (j = 0; j < addressnum; ++j) {
            ...
            if (addr && !Curl_addr2string(addr->ai_addr, addr->ai_addrlen, ipaddress, &port)) {
                fprintf(
                    stderr,
                    "%s:%d tests[%d] failed. getaddressinfo failed.\n",
                    __FILE__,
                    __LINE__,
                    i);
                problem = true;
            ...
            if (addr && !tests[i].address[j]) {
                fprintf(
                    stderr,
                    "%s:%d tests[%d] failed. the retrieved addr "
            ...
            if (port != tests[i].port) {
                fprintf(
                    stderr,
                    "%s:%d tests[%d] failed. the retrieved port "
