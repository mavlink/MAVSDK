...
void win32_init(void)
{
    ...
    if (LOBYTE(wsaData.wVersion) != USE_WINSOCK || HIBYTE(wsaData.wVersion) != USE_WINSOCK) {
        WSACleanup();
        perror("Winsock init failed");
...
void set_advisor_read_lock(const char* filename)
{
    ...
    if (lockfile == NULL) {
        logmsg("Error creating lock file %s error: %d %s", filename, error, strerror(error));
        return;
    }
    ...
    if (res)
        logmsg("Error closing lock file %s error: %d %s", filename, error, strerror(error));
}
...
void clear_advisor_read_lock(const char* filename)
{
    ...
    */
    ...
    if (res)
        logmsg("Error removing lock file %s error: %d %s", filename, error, strerror(error));
}
...
static DWORD WINAPI main_window_loop(LPVOID lpParameter)
{
    ...
    hidden_main_window = CreateWindowEx(
        0,
        "MainWClass",
        "Recv WM_CLOSE msg",
        WS_OVERLAPPEDWINDOW,
...
void install_signal_handlers(bool keep_sigalrm)
{
    ...
    thread_main_window =
        CreateThread(NULL, 0, &main_window_loop, (LPVOID)GetModuleHandle(NULL), 0, &thread_main_id);
