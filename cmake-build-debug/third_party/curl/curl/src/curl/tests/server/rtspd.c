...
typedef enum { RPROT_NONE = 0, RPROT_RTSP = 1, RPROT_HTTP = 2 } reqprot_t;

#define SET_RTP_PKT_CHN(p, c) ((p)[1] = (unsigned char)((c)&0xFF))

#define SET_RTP_PKT_LEN(p, l) \
    (((p)[2] = (unsigned char)(((l) >> 8) & 0xFF)), ((p)[3] = (unsigned char)((l)&0xFF)))
...
static const char* doc404_RTSP = "RTSP/1.0 404 Not Found\r\n"
                                 "Server: " RTSPDVERSION END_OF_HEADERS;
...
static int ProcessRequest(struct httprequest* req)
{
                        ...
                        if (!strncmp(CMD_AUTH_REQUIRED, ptr, strlen(CMD_AUTH_REQUIRED))) {
                            ...
                            if (rtp_partno == req->partno) {
                                ...
                                if (req->rtp_buffer == NULL) {
                                    ...
                                    req->rtp_buffer = realloc(
                                        req->rtp_buffer, req->rtp_buffersize + rtp_size + 4);
                                    memcpy(
                                        req->rtp_buffer + req->rtp_buffersize,
                                        rtp_scratch,
                                        rtp_size + 4);
                                    req->rtp_buffersize += rtp_size + 4;
                                    free(rtp_scratch);
    ...
    if (!req->pipe && req->open && req->prot_version >= 11 && end &&
...
static void storerequest(char* reqbuf, size_t totalsize)
{
    ...
    if (dump == NULL) {
        logmsg("Error opening file %s error: %d %s", REQUEST_DUMP, error, strerror(error));
    ...
    if (writeleft == 0)
        ...
        logmsg("Error writing file %s error: %d %s", REQUEST_DUMP, error, strerror(error));
        logmsg(
...
storerequest_cleanup:
    ...
    if (res)
        logmsg("Error closing file %s error: %d %s", REQUEST_DUMP, error, strerror(error));
}
...
#define STREAMTHIS "a string to stream 01234567890\n"
    ...
    if (res)
        logmsg("Error closing file %s error: %d %s", RESPONSE_DUMP, error, strerror(error));
    ...
    if (sendfailure) {
        logmsg(
            "Sending response failed. Only (%zu bytes) of "
            "(%zu bytes) were sent",
            responsesize - count,
            responsesize);
        free(ptr);
    ...
    logmsg("Response sent (%zu bytes) and written to " RESPONSE_DUMP, responsesize);
    free(ptr);
...
int main(int argc, char* argv[])
{
    ...
    if (0 != rc) {
        error = SOCKERRNO;
        logmsg("Error binding socket on port %hu: (%d) %s", port, error, strerror(error));
        goto server_cleanup;
    }
    ...
    */
    ...
    for (;;) {
        ...
        */
         ...
         */
        flag = 1;
        if (setsockopt(msgsock, IPPROTO_TCP, TCP_NODELAY, (void*)&flag, sizeof(flag)) == -1) {
