...
static curl_socket_t socksconnect(unsigned short connectport, const char* connectaddr)
{
    int rc;
...
static curl_socket_t socks4(curl_socket_t fd, unsigned char* buffer, ssize_t rc)
{
    ...
    if (!config.port)
        s4port = (unsigned short)((buffer[SOCKS4_DSTPORT] << 8) | (buffer[SOCKS4_DSTPORT + 1]));
    else
        s4port = config.port;
...
static curl_socket_t sockit(curl_socket_t fd)
{
    ...
    if ((buffer[SOCKS5_NMETHODS] < config.nmethods_min) ||
        ...
        logmsg("NMETHODS byte not within %d - %d ", config.nmethods_min, config.nmethods_max);
        return CURL_SOCKET_BAD;
    }
    ...
    */
    ...
    /* port number */
    memcpy(&response[SOCKS5_BNDADDR + len], &buffer[SOCKS5_DSTADDR + len], sizeof(socksport));
    ...
    do {
        ...
        if ((clients < 2) && FD_ISSET(sockfd, &fds_read)) {
            ...
            if (CURL_SOCKET_BAD == newfd) {
                ...
                curl_socket_t remotefd;
                logmsg("====> Client connect, fd %d. Read config from %s", newfd, configfile);
        ...
        for (i = 0; i < 2; i++) {
            ...
            if (cp->used) {
                if (tunnel(cp, &fds_read)) {
                    logmsg(
                        "SOCKS transfer completed. Bytes: < %zu > %zu",
                        cp->fromremote,
                        cp->fromclient);
                    sclose(cp->clientfd);
                    sclose(cp->remotefd);
