...
static int parse_servercmd(struct httprequest* req)
{
    ...
    if (!stream) {
        ...
        while (cmd && cmdsize) {
            ...
            } else if (!strncmp(CMD_CONNECTIONMONITOR, cmd, strlen(CMD_CONNECTIONMONITOR))) {
...
static int ProcessRequest(struct httprequest* req)
{
    ...
    logmsg("Process %d bytes request%s", req->offset, req->callcount > 1 ? " [CONTINUED]" : "");

    /* try to figure out the request characteristics as soon as possible, but
       only once! */

    if (use_gopher && (req->testno == DOCNUMBER_NOTHING) && !strncmp("/verifiedserver", line, 15)) {
        ...
        if (req->testno == DOCNUMBER_NOTHING) {
            ...
            if (sscanf(
                    req->reqbuf,
                    "CONNECT %" MAXDOCNAMELEN_TXT "s HTTP/%d.%d",
                    doc,
                    &prot_major,
                    &prot_minor) == 3) {
                char* portp = NULL;
                ...
                logmsg("Port number: %d, test case number: %ld", req->connect_port, req->testno);
            }
        }
        ...
        logmsg(
            "** Unusual request. Starts with %02x %02x %02x (%c%c%c)",
            line[0],
            line[1],
            line[2],
            line[0],
    ...
    do {
        ...
        } else if (strncasecompare(
                       "Transfer-Encoding: chunked", line, strlen("Transfer-Encoding: chunked"))) {
            /* chunked data coming in */
            chunked = TRUE;
        } else if (
            req->noexpect &&
            strncasecompare("Expect: 100-continue", line, strlen("Expect: 100-continue"))) {
            if (req->cl)
                req->cl = 0;
            req->skipall = TRUE;
    ...
    if (strstr(req->reqbuf, "Authorization: Negotiate")) {
        ...
        logmsg("Negotiate: prev_testno: %d, prev_partno: %d", prev_testno, prev_partno);
    ...
    } else if (!req->ntlm && strstr(req->reqbuf, "Authorization: NTLM TlRMTVNTUAAD")) {
    ...
    } else if (!req->ntlm && strstr(req->reqbuf, "Authorization: NTLM TlRMTVNTUAAB")) {
    ...
    if (req->open && req->prot_version >= 11 && end &&
        req->reqbuf + req->offset > end + strlen(end_of_headers) && !req->cl &&
...
static void storerequest(const char* reqbuf, size_t totalsize)
{
    ...
    if (dump == NULL) {
        logmsg("[2] Error opening file %s error: %d %s", dumpfile, error, strerror(error));
    ...
    if (writeleft == 0)
        ...
        logmsg("Error writing file %s error: %d %s", dumpfile, error, strerror(error));
        logmsg(
...
storerequest_cleanup:
    ...
    if (res)
        logmsg("Error closing file %s error: %d %s", dumpfile, error, strerror(error));
}
...
#define STREAMTHIS "a string to stream 01234567890\n"
    ...
    if (res)
        logmsg("Error closing file %s error: %d %s", responsedump, error, strerror(error));
    ...
    if (sendfailure) {
        logmsg(
            "Sending response failed. Only (%zu bytes) of (%zu bytes) "
            "were sent",
            responsesize - count,
            responsesize);
    ...
    logmsg("Response sent (%zu bytes) and written to %s", responsesize, responsedump);
    free(ptr);
                ...
                if (!strcmp("wait", command)) {
                    ...
                    while ((quarters > 0) && !got_exit_signal) {
                        ...
                        if (res) {
                            /* should not happen */
                            error = errno;
                            logmsg("wait_ms() failed with error: (%d) %s", error, strerror(error));
                            break;
                        }
                    }
                    if (!quarters)
...
static curl_socket_t connect_to(const char* ipaddr, unsigned short port)
{
    ...
    if (CURL_SOCKET_BAD == serverfd) {
        error = SOCKERRNO;
        logmsg("Error creating socket for server connection: (%d) %s", error, strerror(error));
        return CURL_SOCKET_BAD;
    }
    ...
    if (socket_domain_is_ip()) {
        ...
        if (0 != setsockopt(serverfd, IPPROTO_TCP, TCP_NODELAY, (void*)&flag, sizeof(flag)))
    ...
    if (rc) {
        error = SOCKERRNO;
        logmsg("Error connecting to server port %hu: (%d) %s", port, error, strerror(error));
        sclose(serverfd);
        return CURL_SOCKET_BAD;
    }

    logmsg("connected fine to %s%s%s:%hu, now tunnel", op_br, ipaddr, cl_br, port);
...
static void
http_connect(curl_socket_t* infdp, curl_socket_t rootfd, const char* ipaddr, unsigned short ipport)
{
    ...
    while (!got_exit_signal) {
        ...
        if ((clientfd[DATA] == CURL_SOCKET_BAD) && (serverfd[DATA] == CURL_SOCKET_BAD) &&
            poll_client_rd[CTRL] && poll_client_wr[CTRL] && poll_server_rd[CTRL] &&
            poll_server_wr[CTRL]) {
            ...
            if ((clientfd[DATA] == CURL_SOCKET_BAD) && (serverfd[DATA] == CURL_SOCKET_BAD) &&
                ...
                if (datafd != CURL_SOCKET_BAD) {
                    ...
                    if (socket_domain_is_ip()) {
                        ...
                        curl_socklen_t flag = 1;
                        if (0 != setsockopt(
                                     datafd, IPPROTO_TCP, TCP_NODELAY, (void*)&flag, sizeof(flag)))
...
http_connect_cleanup:
    ...
    for (i = DATA; i >= CTRL; i--) {
        ...
        if ((serverfd[i] != CURL_SOCKET_BAD) || (clientfd[i] != CURL_SOCKET_BAD)) {
...
static curl_socket_t accept_connection(curl_socket_t sock)
{
    ...
    if (CURL_SOCKET_BAD == msgsock) {
        ...
        logmsg("MAJOR ERROR: accept() failed with error: (%d) %s", error, strerror(error));
        return CURL_SOCKET_BAD;
    }

    if (0 != curlx_nonblock(msgsock, TRUE)) {
        error = SOCKERRNO;
        logmsg("curlx_nonblock failed with error: (%d) %s", error, strerror(error));
        sclose(msgsock);
        return CURL_SOCKET_BAD;
    }

    if (0 != setsockopt(msgsock, SOL_SOCKET, SO_KEEPALIVE, (void*)&flag, sizeof(flag))) {
        error = SOCKERRNO;
        logmsg("setsockopt(SO_KEEPALIVE) failed with error: (%d) %s", error, strerror(error));
        sclose(msgsock);
        return CURL_SOCKET_BAD;
    }
    ...
    */
    ...
    if (socket_domain_is_ip()) {
         ...
         */
        if (0 != setsockopt(msgsock, IPPROTO_TCP, TCP_NODELAY, (void*)&flag, sizeof(flag)))
...
int main(int argc, char* argv[])
{
    ...
    flag = 1;
    if (0 != setsockopt(sock, SOL_SOCKET, SO_REUSEADDR, (void*)&flag, sizeof(flag))) {
        error = SOCKERRNO;
        logmsg("setsockopt(SO_REUSEADDR) failed with error: (%d) %s", error, strerror(error));
        goto sws_cleanup;
    }
    ...
    if (!port) {
        ...
        if (getsockname(sock, &localaddr.sa, &la_size) < 0) {
            error = SOCKERRNO;
            logmsg("getsockname() failed with error: (%d) %s", error, strerror(error));
            sclose(sock);
            goto sws_cleanup;
        }
    ...
    logmsg(
        "Running %s %s version on %s", use_gopher ? "GOPHER" : "HTTP", socket_type, location_str);

    /* start accepting connections */
    rc = listen(sock, 5);
    if (0 != rc) {
        error = SOCKERRNO;
        logmsg("listen() failed with error: (%d) %s", error, strerror(error));
        goto sws_cleanup;
    }
    ...
    */
    ...
    for (;;) {
        ...
        if (rc < 0) {
            error = SOCKERRNO;
            logmsg("select() failed with error: (%d) %s", error, strerror(error));
            goto sws_cleanup;
        }
        ...
        for (socket_idx = 1; socket_idx < num_sockets; ++socket_idx) {
            if (FD_ISSET(all_sockets[socket_idx], &input)) {
                ...
                do {
                    rc = service_connection(all_sockets[socket_idx], &req, sock, connecthost);
                    if (got_exit_signal)
                        goto sws_cleanup;
...
sws_cleanup:
    ...
    for (socket_idx = 1; socket_idx < num_sockets; ++socket_idx)
        if ((all_sockets[socket_idx] != sock) && (all_sockets[socket_idx] != CURL_SOCKET_BAD))
            sclose(all_sockets[socket_idx]);
