...
typedef enum { FROM_CLIENT, FROM_SERVER } mqttdir;

static void logprotocol(
    mqttdir dir,
    const char* prefix,
    size_t remlen,
    FILE* output,
...
{
    unsigned char packet[] = {MQTT_MSG_CONNACK, 0x02, 0x00, 0x00};
    ssize_t rc;
...
{
    unsigned char packet[] = {
        MQTT_MSG_PUBACK, 0x00, 0, 0 /* filled in below */
    };
    ssize_t rc;
    packet[2] = (unsigned char)(packetid >> 8);
    packet[3] = (unsigned char)(packetid & 0xff);
...
static size_t decode_length(unsigned char* buf, size_t buflen, size_t* lenbytes)
{
    size_t len = 0;
    size_t mult = 1;
    size_t i;
    ...
    size_t* remaining_length_bytesp)
{
    ...
    *remaining_lengthp = decode_length(&buffer[1], i, remaining_length_bytesp);
    logmsg("Remaining Length: %ld [%d bytes]", (long)*remaining_lengthp, *remaining_length_bytesp);
    return 0;
}
    ...
    do {
        ...
        if (byte == MQTT_MSG_CONNECT) {
            ...
            if ((ssize_t)payload_len != (rc - 12)) {
                logmsg("Payload length mismatch, expected %x got %x", rc - 12, payload_len);
                goto end;
            ...
            if (topic_len != (remaining_length - 5)) {
                logmsg("Wrong topic length, got %d expected %d", topic_len, remaining_length - 5);
                goto end;
            }
