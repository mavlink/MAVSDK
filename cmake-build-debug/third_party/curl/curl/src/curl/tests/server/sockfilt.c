...
static ssize_t fullread(int filedes, void* buffer, size_t nbytes)
{
    ...
    do {
        ...
        if (rc < 0) {
            ...
            logmsg("unrecoverable read() failure: (%d) %s", error, strerror(error));
            return -1;
        }
...
static ssize_t fullwrite(int filedes, const void* buffer, size_t nbytes)
{
    ...
    do {
        ssize_t wc = write(filedes, (const unsigned char*)buffer + nwrite, nbytes - nwrite);
        ...
        if (wc < 0) {
            ...
            logmsg("unrecoverable write() failure: (%d) %s", error, strerror(error));
            return -1;
        }
    ...
    for (i = 0; i < len; i++) {
        switch (ptr[i]) {
            ...
            case '\r':
                ...
                break;
            default:
                msnprintf(optr, left, "%c", (ISGRAPH(ptr[i]) || ptr[i] == 0x20) ? ptr[i] : '.');
                width++;
                optr++;
                left--;
                break;
...
static DWORD WINAPI select_ws_wait_thread(LPVOID lpParameter)
{
    ...
    switch (type) {
        case FILE_TYPE_DISK:
             ...
             */
            while (WaitForMultipleObjectsEx(1, handles, FALSE, 0, FALSE) == WAIT_TIMEOUT) {
        ...
        default:
            ...
            if (WaitForMultipleObjectsEx(2, handles, FALSE, INFINITE, FALSE) == WAIT_OBJECT_0 + 1) {
...
static HANDLE select_ws_wait(HANDLE handle, HANDLE signal, HANDLE abort, HANDLE mutex)
{
    struct select_ws_wait_data* data;
    HANDLE thread = NULL;
...
static int
select_ws(int nfds, fd_set* readfds, fd_set* writefds, fd_set* exceptfds, struct timeval* tv)
{
...
static bool juggle(curl_socket_t* sockfdp, curl_socket_t listenfd, enum sockmode* mode)
{
    ...
    if (FD_ISSET(fileno(stdin), &fds_read)) {
        ...
        */
        ...
        logmsg("Received %c%c%c%c (on stdin)", buffer[0], buffer[1], buffer[2], buffer[3]);
        ...
        if (!memcmp("PING", buffer, 4)) {
            ...
            if (buffer_len > (ssize_t)sizeof(buffer)) {
                logmsg(
                    "ERROR: Buffer size (%zu bytes) too small for data size "
                    "(%zd bytes)",
                    sizeof(buffer),
                    buffer_len);
                return FALSE;
...
static curl_socket_t sockdaemon(curl_socket_t sock, unsigned short* listenport)
{
    ...
    do {
        attempt++;
        flag = 1;
        rc = setsockopt(sock, SOL_SOCKET, SO_REUSEADDR, (void*)&flag, sizeof(flag));
        if (rc) {
            error = SOCKERRNO;
    ...
    if (rc) {
        error = SOCKERRNO;
        logmsg("Error binding socket on port %hu: (%d) %s", *listenport, error, strerror(error));
        sclose(sock);
        return CURL_SOCKET_BAD;
    }
    ...
    if (0 != rc) {
        error = SOCKERRNO;
        logmsg("listen(%d, 5) failed with error: (%d) %s", sock, error, strerror(error));
        sclose(sock);
        return CURL_SOCKET_BAD;
    }
...
int main(int argc, char* argv[])
{
    ...
    while (argc > arg) {
        if (!strcmp("--version", argv[arg])) {
            ...
            if (argc > arg) {
                char* endptr;
                unsigned long ulnum = strtoul(argv[arg], &endptr, 10);
                if ((endptr != argv[arg] + strlen(argv[arg])) || (ulnum < 1025UL) ||
                    (ulnum > 65535UL)) {
    ...
    if (connectport) {
        ...
        if (rc) {
            error = SOCKERRNO;
            logmsg("Error connecting to port %hu: (%d) %s", connectport, error, strerror(error));
            write_stdout("FAIL\n", 5);
            goto sockfilt_cleanup;
        }
        logmsg("====> Client connect");
