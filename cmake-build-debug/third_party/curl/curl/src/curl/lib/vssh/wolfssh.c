...
#endif
static CURLcode wsftp_done(struct connectdata* conn, CURLcode, bool premature);
static CURLcode wsftp_doing(struct connectdata* conn, bool* dophase_done);
static CURLcode wsftp_disconnect(struct connectdata* conn, bool dead);
static int wssh_getsock(struct connectdata* conn, curl_socket_t* sock);
static int wssh_perform_getsock(const struct connectdata* conn, curl_socket_t* sock);
...
static void state(struct connectdata* conn, sshstate nowstate)
{
    ...
    if (sshc->state != nowstate) {
        infof(
            conn->data,
            "wolfssh %p state change from %s to %s\n",
            (void*)sshc,
            names[sshc->state],
            names[nowstate]);
    }
...
static ssize_t
wscp_send(struct connectdata* conn, int sockindex, const void* mem, size_t len, CURLcode* err)
{
    ssize_t nwrite = 0;
    (void)conn;
...
static ssize_t
wscp_recv(struct connectdata* conn, int sockindex, char* mem, size_t len, CURLcode* err)
{
    ssize_t nread = 0;
    (void)conn;
    (void)sockindex; /* we only support SCP on the fixed known primary socket */
...
static ssize_t
wsftp_send(struct connectdata* conn, int sockindex, const void* mem, size_t len, CURLcode* err)
{
    struct ssh_conn* sshc = &conn->proto.sshc;
    word32 offset[2];
    int rc;
    ...
    rc = wolfSSH_SFTP_SendWritePacket(
        sshc->ssh_session, sshc->handle, sshc->handleSz, &offset[0], (byte*)mem, (word32)len);
 ...
 */
static ssize_t
wsftp_recv(struct connectdata* conn, int sockindex, char* mem, size_t len, CURLcode* err)
{
    int rc;
    struct ssh_conn* sshc = &conn->proto.sshc;
    word32 offset[2];
    ...
    rc = wolfSSH_SFTP_SendReadPacket(
        sshc->ssh_session, sshc->handle, sshc->handleSz, &offset[0], (byte*)mem, (word32)len);
    if (rc == WS_FATAL_ERROR)
        rc = wolfSSH_get_error(sshc->ssh_session);
    if (rc == WS_WANT_READ) {
...
static int userauth(byte authtype, WS_UserAuthData* authdata, void* ctx)
{
    struct connectdata* conn = ctx;
    DEBUGF(infof(
        conn->data,
        "wolfssh callback: type %s\n",
        authtype == WOLFSSH_USERAUTH_PASSWORD ? "PASSWORD" : "PUBLICCKEY"));
    ...
    *block = FALSE; /* we're not blocking by default */
    ...
    do {
        switch (sshc->state) {
            ...
            case SSH_SFTP_CLOSE:
                if (sshc->handleSz)
                    rc = wolfSSH_SFTP_Close(sshc->ssh_session, sshc->handle, sshc->handleSz);
                else
            ...
            case SSH_SFTP_READDIR:
                ...
                if (rc == WS_WANT_READ) {
                    ...
                    *block = TRUE;
                    ...
                    while (name) {
                        char* line =
                            aprintf("%s\n", data->set.ftp_list_only ? name->fName : name->lName);
                        if (line == NULL) {
                            state(conn, SSH_SFTP_CLOSE);
                            sshc->actualcode = CURLE_OUT_OF_MEMORY;
                            break;
                        }
                        result = Curl_client_write(conn, CLIENTWRITE_BODY, line, strlen(line));
                        free(line);
                        if (result) {
                            sshc->actualcode = result;
                            break;
...
static CURLcode wscp_perform(struct connectdata* conn, bool* connected, bool* dophase_done)
{
    (void)conn;
    (void)connected;
    (void)dophase_done;
...
static CURLcode wsftp_perform(struct connectdata* conn, bool* connected, bool* dophase_done)
{
    CURLcode result = CURLE_OK;
...
static CURLcode wssh_block_statemach(struct connectdata* conn, bool disconnect)
{
    struct ssh_conn* sshc = &conn->proto.sshc;
    CURLcode result = CURLE_OK;
    struct Curl_easy* data = conn->data;
    ...
    while ((sshc->state != SSH_STOP) && !result) {
        ...
        if (!result) {
            ...
            (void)Curl_socket_check(
                fd_read, CURL_SOCKET_BAD, fd_write, left > 1000 ? 1000 : left); /* ignore result */
        }
    }
...
static CURLcode wsftp_done(struct connectdata* conn, CURLcode code, bool premature)
{
    (void)premature;
    state(conn, SSH_SFTP_CLOSE);
