...
#define sftp_libssh2_realpath(s, p, t, m) \
    libssh2_sftp_symlink_ex((s), (p), curlx_uztoui(strlen(p)), (t), (m), LIBSSH2_SFTP_REALPATH)
...
static int ssh_perform_getsock(const struct connectdata* conn, curl_socket_t* sock);
...
static CURLcode ssh_knownhost(struct connectdata* conn)
{
    ...
    if (data->set.str[STRING_SSH_KNOWNHOSTS]) {
        ...
        int keytype;
        size_t keylen;
        const char* remotekey = libssh2_session_hostkey(sshc->ssh_session, &keylen, &keytype);
        ...
        if (remotekey) {
            ...
            if (!keybit)
                ...
                keycheck = libssh2_knownhost_checkp(
                    sshc->kh,
                    conn->host.name,
                ...
                infof(
                    ...
                    keycheck,
                    (keycheck <= LIBSSH2_KNOWNHOST_CHECK_MISMATCH) ? host->key : "<none>");
                ...
                if (keycheck <= LIBSSH2_KNOWNHOST_CHECK_MISMATCH) {
                    ...
                    knownkey.len = 0;
                    knownkey.keytype =
                        (keytype == LIBSSH2_HOSTKEY_TYPE_RSA) ? CURLKHTYPE_RSA : CURLKHTYPE_DSS;
                    knownkeyp = &knownkey;
                }
        ...
        switch (rc) {
            ...
            case CURLKHSTAT_FINE_ADD_TO_FILE:
                ...
                if (keycheck != LIBSSH2_KNOWNHOST_CHECK_MATCH) {
                    ...
                    int addrc = libssh2_knownhost_add(
                        sshc->kh,
                        conn->host.name,
                        NULL,
                        remotekey,
                    ...
                    if (addrc)
                        ...
                        /* now we write the entire in-memory list of known hosts to the
                           known_hosts file */
                        int wrc = libssh2_knownhost_writefile(
                            sshc->kh,
    ...
    do {
        switch (sshc->state) {
            ...
            case SSH_AUTHLIST:
                 ...
                 */
                sshc->authlist = libssh2_userauth_list(
                    sshc->ssh_session, conn->user, curlx_uztoui(strlen(conn->user)));
            ...
            case SSH_AUTH_AGENT:
                ...
                rc = libssh2_agent_get_identity(
                    sshc->ssh_agent, &sshc->sshagent_identity, sshc->sshagent_prev_identity);
                if (rc == LIBSSH2_ERROR_EAGAIN)
                    break;

                if (rc == 0) {
                    rc = libssh2_agent_userauth(
                        sshc->ssh_agent, conn->user, sshc->sshagent_identity);
            ...
            case SSH_AUTH_KEY:
                ...
                rc = libssh2_userauth_keyboard_interactive_ex(
                    sshc->ssh_session, conn->user, curlx_uztoui(strlen(conn->user)), &kbd_callback);
                if (rc == LIBSSH2_ERROR_EAGAIN) {
                    break;
                }
                if (rc == 0) {
            ...
            case SSH_SFTP_QUOTE_STAT: {
                ...
                if (strncasecompare(cmd, "chgrp", 5)) {
                    ...
                    if (sshc->quote_attrs.permissions == 0 && !ISDIGIT(sshc->quote_path1[0])) {
                        Curl_safefree(sshc->quote_path1);
                        Curl_safefree(sshc->quote_path2);
            ...
            case SSH_SFTP_QUOTE_SETSTAT:
                ...
                if (rc != 0 && !sshc->acceptfail) {
                    ...
                    Curl_safefree(sshc->quote_path1);
                    Curl_safefree(sshc->quote_path2);
                    failf(data, "Attempt to set SFTP stats failed: %s", sftp_libssh2_strerror(err));
                    state(conn, SSH_SFTP_CLOSE);
                    sshc->nextstate = SSH_NO_STATE;
                    sshc->actualcode = CURLE_QUOTE_ERROR;
                    break;
            ...
            case SSH_SFTP_QUOTE_SYMLINK:
                ...
                if (rc != 0 && !sshc->acceptfail) {
                    ...
                    Curl_safefree(sshc->quote_path1);
                    Curl_safefree(sshc->quote_path2);
                    failf(data, "symlink command failed: %s", sftp_libssh2_strerror(err));
                    state(conn, SSH_SFTP_CLOSE);
                    sshc->nextstate = SSH_NO_STATE;
                    sshc->actualcode = CURLE_QUOTE_ERROR;
                    break;
            ...
            case SSH_SFTP_QUOTE_RMDIR:
                rc = libssh2_sftp_rmdir_ex(
                    sshc->sftp_session, sshc->quote_path1, curlx_uztoui(strlen(sshc->quote_path1)));
                if (rc == LIBSSH2_ERROR_EAGAIN) {
                    break;
                }
                if (rc != 0 && !sshc->acceptfail) {
            ...
            case SSH_SFTP_UPLOAD_INIT: {
                ...
                sshc->sftp_handle = libssh2_sftp_open_ex(
                    sshc->sftp_session,
                    sftp_scp->path,
                    curlx_uztoui(strlen(sftp_scp->path)),
                    flags,
                ...
                if (!sshc->sftp_handle) {
                    ...
                    if (sshc->secondCreateDirs) {
                        state(conn, SSH_SFTP_CLOSE);
                        sshc->actualcode =
                            err >= LIBSSH2_FX_OK ? sftp_libssh2_error_to_CURLE(err) : CURLE_SSH;
                        failf(data, "Creating the dir/file failed: %s", sftp_libssh2_strerror(err));
                        break;
                    }
                    if (((err == LIBSSH2_FX_NO_SUCH_FILE) || (err == LIBSSH2_FX_FAILURE) ||
                         (err == LIBSSH2_FX_NO_SUCH_PATH)) &&
                        (data->set.ftp_create_missing_dirs && (strlen(sftp_scp->path) > 1))) {
                ...
                if (data->state.resume_from > 0) {
                    ...
                    if (seekerr != CURL_SEEKFUNC_OK) {
                        ...
                        do {
                            ...
                            size_t actuallyread;
                            Curl_set_in_callback(data, true);
                            actuallyread = data->state.fread_func(
            ...
            case SSH_SFTP_CREATE_DIRS_MKDIR:
                ...
                *sshc->slash_pos = '/';
                ...
                if (rc < 0) {
                     ...
                     */
                    err = sftp_libssh2_last_error(sshc->sftp_session);
                    if ((err != LIBSSH2_FX_FILE_ALREADY_EXISTS) && (err != LIBSSH2_FX_FAILURE) &&
            ...
            case SSH_SFTP_READDIR:
                ...
                if (rc > 0) {
                    ...
                    if (data->set.ftp_list_only) {
                        ...
                        if (data->set.verbose) {
                            Curl_debug(
                                data, CURLINFO_DATA_OUT, sshc->readdir_filename, sshc->readdir_len);
                        }
                    } else {
            ...
            case SSH_SCP_UPLOAD_INIT:
                ...
                if (!sshc->ssh_channel) {
                    ...
                    ssh_err =
                        (int)(libssh2_session_last_error(sshc->ssh_session, &err_msg, NULL, 0));
            ...
            case SSH_SCP_DOWNLOAD_INIT: {
                ...
                sshc->ssh_channel = libssh2_scp_recv(sshc->ssh_session, sftp_scp->path, &sb);
#else
                libssh2_struct_stat sb;
                memset(&sb, 0, sizeof(libssh2_struct_stat));
                sshc->ssh_channel = libssh2_scp_recv2(sshc->ssh_session, sftp_scp->path, &sb);
#endif
            ...
            case SSH_SESSION_FREE:
                ...
                if (sshc->ssh_agent) {
                    ...
                    if (rc < 0) {
                        ...
                        (void)libssh2_session_last_error(sshc->ssh_session, &err_msg, NULL, 0);
                        infof(
                            data, "Failed to disconnect from libssh2 agent: %d %s\n", rc, err_msg);
                    }
...
static CURLcode ssh_block_statemach(struct connectdata* conn, bool disconnect)
{
    struct ssh_conn* sshc = &conn->proto.sshc;
    CURLcode result = CURLE_OK;
    struct Curl_easy* data = conn->data;
    ...
    while ((sshc->state != SSH_STOP) && !result) {
        ...
        if (block) {
            ...
            (void)Curl_socket_check(
                fd_read, CURL_SOCKET_BAD, fd_write, left > 1000 ? 1000 : (time_t)left);
        }
#endif
    }
...
static CURLcode ssh_connect(struct connectdata* conn, bool* done)
{
    ...
    ssh->ssh_session =
        libssh2_session_init_ex(my_libssh2_malloc, my_libssh2_free, my_libssh2_realloc, conn);
    ...
    if (data->set.str[STRING_SSH_KNOWNHOSTS]) {
        ...
        rc = libssh2_knownhost_readfile(
            ssh->kh, data->set.str[STRING_SSH_KNOWNHOSTS], LIBSSH2_KNOWNHOST_FILE_OPENSSH);
        if (rc < 0)
            infof(
                data, "Failed to read known hosts from %s\n", data->set.str[STRING_SSH_KNOWNHOSTS]);
    }
#endif /* HAVE_LIBSSH2_KNOWNHOST_API */
...
scp_send(struct connectdata* conn, int sockindex, const void* mem, size_t len, CURLcode* err)
{
    ...
    nwrite = (ssize_t)libssh2_channel_write(conn->proto.sshc.ssh_channel, mem, len);
