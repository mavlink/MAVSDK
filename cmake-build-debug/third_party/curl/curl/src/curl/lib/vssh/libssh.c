...
static CURLcode myssh_multi_statemach(struct connectdata* conn, bool* done);
...
static int myssh_perform_getsock(const struct connectdata* conn, curl_socket_t* sock);
...
)
{
    ...
    if (sshc->state != nowstate) {
        infof(
            conn->data,
            "SSH %p state change from %s to %s (line %d)\n",
            (void*)sshc,
            names[sshc->state],
            names[nowstate],
            lineno);
...
static int myssh_is_known(struct connectdata* conn)
{
    ...
    vstate = ssh_session_get_known_hosts_entry(sshc->ssh_session, &knownhostsentry);

    /* Case an entry was found in a known hosts file */
    if (knownhostsentry) {
        if (knownhostsentry->publickey) {
            rc = ssh_pki_export_pubkey_base64(knownhostsentry->publickey, &known_base64);
            if (rc != SSH_OK) {
                goto cleanup;
            }
    ...
    if (func) { /* use callback to determine action */
        ...
        rc = func(
            data,
            knownkeyp, /* from the knownhosts file */
            &foundkey, /* from the remote host */
            keymatch,
            data->set.ssh_keyfunc_userp);
    ...
    do {
      ...
      ssh_set_log_level(SSH_LOG_PROTOCOL);
            ...
            case SSH_AUTH_PKEY_INIT:
                ...
                if (data->set.str[STRING_SSH_PRIVATE_KEY]) {
                    ...
                    rc = ssh_userauth_publickey_auto(
                        sshc->ssh_session, NULL, data->set.ssl.key_passwd);
                    if (rc == SSH_AUTH_AGAIN) {
                        rc = SSH_AGAIN;
                        break;
                    }
            ...
            case SSH_SFTP_QUOTE_SETSTAT:
                rc = sftp_setstat(sshc->sftp_session, sshc->quote_path2, sshc->quote_attrs);
            ...
            case SSH_SFTP_QUOTE_SYMLINK:
                rc = sftp_symlink(sshc->sftp_session, sshc->quote_path2, sshc->quote_path1);
                if (rc != 0 && !sshc->acceptfail) {
                    Curl_safefree(sshc->quote_path1);
                    Curl_safefree(sshc->quote_path2);
                    failf(data, "symlink command failed: %s", ssh_get_error(sshc->ssh_session));
                    state(conn, SSH_SFTP_CLOSE);
                    sshc->nextstate = SSH_NO_STATE;
                    sshc->actualcode = CURLE_QUOTE_ERROR;
                    break;
            ...
            case SSH_SFTP_QUOTE_MKDIR:
                rc = sftp_mkdir(
                    sshc->sftp_session, sshc->quote_path1, (mode_t)data->set.new_directory_perms);
                if (rc != 0 && !sshc->acceptfail) {
                    Curl_safefree(sshc->quote_path1);
                    failf(data, "mkdir command failed: %s", ssh_get_error(sshc->ssh_session));
                    state(conn, SSH_SFTP_CLOSE);
                    sshc->nextstate = SSH_NO_STATE;
                    sshc->actualcode = CURLE_QUOTE_ERROR;
                    break;
            ...
            case SSH_SFTP_QUOTE_RENAME:
                rc = sftp_rename(sshc->sftp_session, sshc->quote_path1, sshc->quote_path2);
                if (rc != 0 && !sshc->acceptfail) {
                    Curl_safefree(sshc->quote_path1);
                    Curl_safefree(sshc->quote_path2);
                    failf(data, "rename command failed: %s", ssh_get_error(sshc->ssh_session));
                    state(conn, SSH_SFTP_CLOSE);
                    sshc->nextstate = SSH_NO_STATE;
                    sshc->actualcode = CURLE_QUOTE_ERROR;
                    break;
            ...
            case SSH_SFTP_QUOTE_RMDIR:
                ...
                if (rc != 0 && !sshc->acceptfail) {
                    Curl_safefree(sshc->quote_path1);
                    failf(data, "rmdir command failed: %s", ssh_get_error(sshc->ssh_session));
                    state(conn, SSH_SFTP_CLOSE);
                    sshc->nextstate = SSH_NO_STATE;
                    sshc->actualcode = CURLE_QUOTE_ERROR;
                    break;
            ...
            case SSH_SFTP_QUOTE_STATVFS: {
                ...
                if (!statvfs && !sshc->acceptfail) {
                    Curl_safefree(sshc->quote_path1);
                    failf(data, "statvfs command failed: %s", ssh_get_error(sshc->ssh_session));
                    state(conn, SSH_SFTP_CLOSE);
                    sshc->nextstate = SSH_NO_STATE;
                    sshc->actualcode = CURLE_QUOTE_ERROR;
                    break;
            ...
            case SSH_SFTP_UPLOAD_INIT: {
                ...
                sshc->sftp_file = sftp_open(
                    sshc->sftp_session, protop->path, flags, (mode_t)data->set.new_file_perms);
                if (!sshc->sftp_file) {
                ...
                if (data->state.resume_from > 0) {
                    ...
                    if (conn->seek_func) {
                        Curl_set_in_callback(data, true);
                        seekerr =
                            conn->seek_func(conn->seek_client, data->state.resume_from, SEEK_SET);
                        Curl_set_in_callback(data, false);
                    }
            ...
            case SSH_SFTP_CREATE_DIRS_MKDIR:
                ...
                rc = sftp_mkdir(
                    sshc->sftp_session, protop->path, (mode_t)data->set.new_directory_perms);
                *sshc->slash_pos = '/';
                ++sshc->slash_pos;
                if (rc < 0) {
                    /*
                     ...
                     */
                    err = sftp_get_error(sshc->sftp_session);
                    if ((err != SSH_FX_FILE_ALREADY_EXISTS) && (err != SSH_FX_FAILURE) &&
            ...
            case SSH_SFTP_READDIR_INIT:
                 ...
                 * listing
                 */
                sshc->sftp_dir = sftp_opendir(sshc->sftp_session, protop->path);
                if (!sshc->sftp_dir) {
                    failf(
                        data,
                ...
                if (sshc->readdir_attrs) {
                    ...
                    if (data->set.ftp_list_only) {
                        ...
                        result = Curl_client_write(
                            conn, CLIENTWRITE_BODY, tmpLine, sshc->readdir_len + 1);
                        free(tmpLine);
                        ...
                        memcpy(sshc->readdir_line, sshc->readdir_longentry, sshc->readdir_currLen);
                        if ((sshc->readdir_attrs->flags & SSH_FILEXFER_ATTR_PERMISSIONS) &&
                            ((sshc->readdir_attrs->permissions & S_IFMT) == S_IFLNK)) {
            ...
            case SSH_SFTP_READDIR_LINK:
                ...
                if (sshc->readdir_link_attrs == 0) {
                    failf(
                        data,
                        "Could not read symlink for reading: %s",
                        ssh_get_error(sshc->ssh_session));
                    MOVE_TO_SFTP_CLOSE_STATE();
                }
                ...
                new_readdir_line = Curl_saferealloc(sshc->readdir_line, sshc->readdir_totalLen);
                if (!new_readdir_line) {
                ...
                sshc->readdir_currLen += msnprintf(
                    sshc->readdir_line + sshc->readdir_currLen,
                    sshc->readdir_totalLen - sshc->readdir_currLen,
                    " -> %s",
                    sshc->readdir_filename);
            ...
            case SSH_SFTP_READDIR_BOTTOM:
                sshc->readdir_currLen += msnprintf(
                    ...
                    "\n");
                result = Curl_client_write(
                    conn, CLIENTWRITE_BODY, sshc->readdir_line, sshc->readdir_currLen);

                if (!result) {
                    /* output debug output if that is requested */
                    if (data->set.verbose) {
                        Curl_debug(
                            data, CURLINFO_DATA_OUT, sshc->readdir_line, sshc->readdir_currLen);
                    }
            ...
            case SSH_SFTP_DOWNLOAD_INIT:
                ...
                sshc->sftp_file = sftp_open(
                    sshc->sftp_session, protop->path, O_RDONLY, (mode_t)data->set.new_file_perms);
                if (!sshc->sftp_file) {
                    failf(
                        data,
                        "Could not open remote file for reading: %s",
            ...
            case SSH_SFTP_DOWNLOAD_STAT: {
                ...
                if (!attrs || !(attrs->flags & SSH_FILEXFER_ATTR_SIZE) || (attrs->size == 0)) {
                    /*
            ...
            case SSH_SCP_TRANS_INIT:
                ...
                if (data->set.upload) {
                    ...
                    sshc->scp_session = ssh_scp_new(sshc->ssh_session, SSH_SCP_WRITE, protop->path);
                    state(conn, SSH_SCP_UPLOAD_INIT);
                } else {
                    sshc->scp_session = ssh_scp_new(sshc->ssh_session, SSH_SCP_READ, protop->path);
...
static CURLcode myssh_block_statemach(struct connectdata* conn, bool disconnect)
{
    struct ssh_conn* sshc = &conn->proto.sshc;
    CURLcode result = CURLE_OK;
    struct Curl_easy* data = conn->data;
    ...
    while ((sshc->state != SSH_STOP) && !result) {
        ...
        if (block) {
            ...
            (void)Curl_socket_check(
                fd_read, CURL_SOCKET_BAD, CURL_SOCKET_BAD, left > 1000 ? 1000 : left);
        }
    }
...
static CURLcode myssh_connect(struct connectdata* conn, bool* done)
{
    ...
    if (data->set.str[STRING_SSH_KNOWNHOSTS]) {
        ...
        rc = ssh_options_set(
            ssh->ssh_session, SSH_OPTIONS_KNOWNHOSTS, data->set.str[STRING_SSH_KNOWNHOSTS]);
        if (rc != SSH_OK) {
    ...
    if (conn->remote_port) {
        rc = ssh_options_set(ssh->ssh_session, SSH_OPTIONS_PORT, &conn->remote_port);
        if (rc != SSH_OK) {
    ...
    if (data->set.ssh_compression) {
        rc = ssh_options_set(
            ssh->ssh_session, SSH_OPTIONS_COMPRESSION, "zlib,zlib@openssh.com,none");
        if (rc != SSH_OK) {
    ...
    if (data->set.str[STRING_SSH_PUBLIC_KEY]) {
        rc = ssh_pki_import_pubkey_file(data->set.str[STRING_SSH_PUBLIC_KEY], &ssh->pubkey);
        if (rc != SSH_OK) {
...
static CURLcode scp_perform(struct connectdata* conn, bool* connected, bool* dophase_done)
{
    CURLcode result = CURLE_OK;
...
static ssize_t
scp_send(struct connectdata* conn, int sockindex, const void* mem, size_t len, CURLcode* err)
{
    int rc;
...
static ssize_t
scp_recv(struct connectdata* conn, int sockindex, char* mem, size_t len, CURLcode* err)
{
    ssize_t nread;
    (void)err;
    (void)sockindex; /* we only support SCP on the fixed known primary socket */
...
static ssize_t
sftp_send(struct connectdata* conn, int sockindex, const void* mem, size_t len, CURLcode* err)
{
    ssize_t nwrite;
    (void)sockindex;
 ...
 */
static ssize_t
sftp_recv(struct connectdata* conn, int sockindex, char* mem, size_t len, CURLcode* err)
{
    ssize_t nread;
    (void)sockindex;
    ...
    switch (conn->proto.sshc.sftp_recv_state) {
        case 0:
            conn->proto.sshc.sftp_file_index =
                sftp_async_read_begin(conn->proto.sshc.sftp_file, (uint32_t)len);
        ...
        case 1:
            ...
            nread = sftp_async_read(
                conn->proto.sshc.sftp_file, mem, (uint32_t)len, conn->proto.sshc.sftp_file_index);
...
static void sftp_quote(struct connectdata* conn)
{
     ...
     * functions.
     */
    if (strncasecompare(cmd, "chgrp ", 6) || strncasecompare(cmd, "chmod ", 6) ||
        ...
        if (result) {
            if (result == CURLE_OUT_OF_MEMORY)
                ...
                failf(
                    data,
                    "Syntax error in chgrp/chmod/chown: "
...
static void sftp_quote_stat(struct connectdata* conn)
{
    ...
    if (sshc->quote_attrs == NULL) {
        Curl_safefree(sshc->quote_path1);
        Curl_safefree(sshc->quote_path2);
        failf(data, "Attempt to get SFTP stats failed: %d", sftp_get_error(sshc->sftp_session));
        state(conn, SSH_SFTP_CLOSE);
        sshc->nextstate = SSH_NO_STATE;
        sshc->actualcode = CURLE_QUOTE_ERROR;
        return;
    ...
    if (strncasecompare(cmd, "chgrp", 5)) {
        ...
        if (sshc->quote_attrs->gid == 0 && !ISDIGIT(sshc->quote_path1[0]) && !sshc->acceptfail) {
            Curl_safefree(sshc->quote_path1);
            Curl_safefree(sshc->quote_path2);
        ...
        if (sshc->quote_attrs->uid == 0 && !ISDIGIT(sshc->quote_path1[0]) && !sshc->acceptfail) {
            Curl_safefree(sshc->quote_path1);
            Curl_safefree(sshc->quote_path2);
