...
#define RTP_PKT_LENGTH(p) \
    ((((int)((unsigned char)((p)[2]))) << 8) | ((int)((unsigned char)((p)[3]))))
 ...
 */
static CURLcode rtsp_rtp_readwrite(
    struct Curl_easy* data, struct connectdata* conn, ssize_t* nread, bool* readmore);
...
static CURLcode rtsp_done(struct connectdata* conn, CURLcode status, bool premature)
{
    struct Curl_easy* data = conn->data;
    ...
    if (rtsp) {
        ...
        if (data->set.rtspreq == RTSPREQ_RECEIVE && (conn->proto.rtspc.rtp_channel == -1)) {
    ...
    *done = TRUE;
    ...
    if (rtspreq == RTSPREQ_SETUP && !p_transport) {
        ...
        if (data->set.str[STRING_RTSP_TRANSPORT]) {
            ...
            failf(data, "Refusing to issue an RTSP SETUP without a Transport: header.");
            return CURLE_BAD_FUNCTION_ARGUMENT;
        }
    ...
    if (rtspreq == RTSPREQ_DESCRIBE) {
        /* Accept Header */
        p_accept = Curl_checkheaders(conn, "Accept") ? NULL : "Accept: application/sdp\r\n";
    ...
    } else if (!Curl_checkheaders(conn, "User-Agent") && data->set.str[STRING_USERAGENT]) {
     ...
     */
    if (data->state.use_range && (rtspreq & (RTSPREQ_PLAY | RTSPREQ_PAUSE | RTSPREQ_RECORD))) {
...
static CURLcode
rtsp_rtp_readwrite(struct Curl_easy* data, struct connectdata* conn, ssize_t* nread, bool* readmore)
{
    ...
    if (rtspc->rtp_buf) {
        ...
        char* newptr = Curl_saferealloc(rtspc->rtp_buf, rtspc->rtp_bufsize + *nread);
        if (!newptr) {
            rtspc->rtp_buf = NULL;
            rtspc->rtp_bufsize = 0;
            return CURLE_OUT_OF_MEMORY;
    ...
    while ((rtp_dataleft > 0) && (rtp[0] == '$')) {
        if (rtp_dataleft > 4) {
            ...
            DEBUGF(infof(
                data, "RTP write channel %d rtp_length %d\n", rtspc->rtp_channel, rtp_length));
