...
#define STARTS_WITH_DRIVE_PREFIX(str) \
    ((('a' <= str[0] && str[0] <= 'z') || ('A' <= str[0] && str[0] <= 'Z')) && (str[1] == ':'))
...
/* move the full contents of one handle onto another and
   free the original */
static void mv_urlhandle(struct Curl_URL* from, struct Curl_URL* to)
{
    free_urlhandle(to);
    *to = *from;
    free(from);
 ...
 *
 */
static CURLUcode parse_hostname_login(struct Curl_URL* u, char** hostname, unsigned int flags)
{
...
static CURLUcode junkscan(const char* part)
{
    if (part) {
        static const char badbytes[] = {
            ...
            0x19,       0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x1f, 0x7f, 0x00 /* zero terminate */
        };
...
static CURLUcode seturl(const char* url, CURLU* u, unsigned int flags)
{
    ...
    if (url_has_scheme && strcasecompare(schemebuf, "file")) {
        ...
        if (url_has_scheme) {
            ...
            schemep = schemebuf;
            if (!Curl_builtin_scheme(schemep) && !(flags & CURLU_NON_SUPPORT_SCHEME))
...
CURLUcode curl_url_get(CURLU* u, CURLUPart what, char** part, unsigned int flags)
{
    char* ptr;
    ...
    switch (what) {
        ...
        case CURLUPART_PORT:
            ...
            if (!ptr && (flags & CURLU_DEFAULT_PORT) && u->scheme) {
                ...
                const struct Curl_handler* h = Curl_builtin_scheme(u->scheme);
                if (h) {
        ...
        case CURLUPART_URL: {
            ...
            if (u->scheme && strcasecompare("file", u->scheme)) {
                ...
                if ((u->host[0] == '[') && u->zoneid) {
                    ...
                    msnprintf(&allochost[hostlen - 1], alen - hostlen + 1, "%%25%s]", u->zoneid);
                }
    ...
    DEBUGASSERT(storep);
    {
        ...
        if (urlencode) {
            ...
            for (i = (const unsigned char*)part, o = enc; *i; i++) {
                if (Curl_isunreserved(*i) || ((*i == '/') && urlskipslash) ||
                    ((*i == '=') && equalsencode) || ((*i == '+') && plusencode)) {
                    if ((*i == '=') && equalsencode)
