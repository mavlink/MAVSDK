...
static CURLMcode
add_next_timeout(struct curltime now, struct Curl_multi* multi, struct Curl_easy* d);
...
struct Curl_multi* Curl_multi_handle(
    int hashsize, /* socket hash */
    int chashsize) /* connection hash */
{
    ...
    if (Curl_socketpair(AF_UNIX, SOCK_STREAM, 0, multi->wakeup_pair) < 0) {
        ...
        curlx_nonblock(multi->wakeup_pair[0], TRUE) < 0 ||
...
static CURLMcode Curl_multi_wait(
    struct Curl_multi* multi,
...
static CURLMcode
multi_runsingle(struct Curl_multi* multi, struct curltime now, struct Curl_easy* data)
{
    ...
    do {
        ...
        DEBUGASSERT(
            (data->mstate <= CURLM_STATE_CONNECT) || (data->mstate >= CURLM_STATE_DONE) ||
            data->conn);
        if (!data->conn && data->mstate > CURLM_STATE_CONNECT && data->mstate < CURLM_STATE_DONE) {
        ...
        if (data->conn && (data->mstate >= CURLM_STATE_CONNECT) &&
            ...
            if (timeout_ms < 0) {
                ...
                if (data->mstate == CURLM_STATE_WAITRESOLVE)
                    failf(
                        data,
                        "Resolving timed out after %" CURL_FORMAT_TIMEDIFF_T " milliseconds",
                        Curl_timediff(now, data->progress.t_startsingle));
                else if (data->mstate == CURLM_STATE_WAITCONNECT)
                    failf(
                        data,
                        "Connection timed out after %" CURL_FORMAT_TIMEDIFF_T " milliseconds",
                        Curl_timediff(now, data->progress.t_startsingle));
                else {
                    struct SingleRequest* k = &data->req;
                    if (k->size != -1) {
                        failf(
                            ...
                            " out of %" CURL_FORMAT_CURL_OFF_T " bytes received",
                            Curl_timediff(now, data->progress.t_startsingle),
                            k->bytecount,
                            k->size);
                    } else {
        ...
        switch (data->mstate) {
            ...
            case CURLM_STATE_CONNECT:
                ...
                if (CURLE_NO_CONNECTION_AVAILABLE == result) {
                    ...
                    Curl_llist_insert_next(
                        &multi->pending, multi->pending.tail, data, &data->connect_queue);
                    result = CURLE_OK;
                    break;
            ...
            case CURLM_STATE_WAITCONNECT:
                ...
                if (connected && !result) {
                    ...
                    multistate(
                        data,
                        data->conn->bits.tunnel_proxy ? CURLM_STATE_WAITPROXYCONNECT :
            ...
            case CURLM_STATE_DOING:
                ...
                if (!result) {
                    if (dophase_done) {
                        ...
                        multistate(
                            data,
                            data->conn->bits.do_more ? CURLM_STATE_DO_MORE : CURLM_STATE_DO_DONE);
                        rc = CURLM_CALL_MULTI_PERFORM;
                    } /* dophase_done */
                } else {
                    /* failure detected */
            ...
            case CURLM_STATE_DO_MORE:
                ...
                if (!result) {
                    if (control) {
                        ...
                        multistate(data, control == 1 ? CURLM_STATE_DO_DONE : CURLM_STATE_DOING);
                        rc = CURLM_CALL_MULTI_PERFORM;
                    } else
                        /* stay in DO_MORE */
                        rc = CURLM_OK;
    ...
    statemachine_end:
        ...
        if (data->mstate < CURLM_STATE_COMPLETED) {
            ...
            /* if there's still a connection to use, call the progress function */
                ...
                multistate(
                    data,
                    (data->mstate < CURLM_STATE_DONE) ? CURLM_STATE_DONE : CURLM_STATE_COMPLETED);
                rc = CURLM_CALL_MULTI_PERFORM;
            }
        }
    ...
    for (i = 0; i < data->numsocks; i++) {
        ...
        if (entry) {
            ...
            if (!entry->users) {
                if (multi->socket_cb)
                    multi->socket_cb(
                        data, s, CURL_POLL_REMOVE, multi->socket_userp, entry->socketp);
...
static CURLMcode multi_socket(
    struct Curl_multi* multi, bool checkall, curl_socket_t s, int ev_bitmask, int* running_handles)
{
    ...
    if (s != CURL_SOCKET_TIMEOUT) {
        ...
        if (!entry)
            ...
            for (he = Curl_hash_next_element(&iter); he; he = Curl_hash_next_element(&iter)) {
...
CURLMcode curl_multi_setopt(struct Curl_multi* multi, CURLMoption option, ...)
{
    ...
    switch (option) {
        ...
        case CURLMOPT_MAX_CONCURRENT_STREAMS: {
            ...
            if (streams < 1)
                streams = 100;
            multi->max_concurrent_streams = (streams > (long)INITIAL_MAX_CONCURRENT_STREAMS) ?
...
CURLMcode curl_multi_socket_action(
    struct Curl_multi* multi, curl_socket_t s, int ev_bitmask, int* running_handles)
{
    CURLMcode result;
    if (multi->in_callback)
        return CURLM_RECURSIVE_API_CALL;
...
{
    return (
        (easy->multi && easy->multi->in_callback) ||
