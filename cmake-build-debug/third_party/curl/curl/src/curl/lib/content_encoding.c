...
static CURLcode
exit_zlib(struct connectdata* conn, z_stream* z, zlibInitState* zlib_init, CURLcode result)
{
...
static CURLcode
inflate_stream(struct connectdata* conn, contenc_writer* writer, zlibInitState started)
{
    ...
    /* Check state. */
    if (zp->zlib_init != ZLIB_INIT && zp->zlib_init != ZLIB_INFLATING &&
        zp->zlib_init != ZLIB_INIT_GZIP && zp->zlib_init != ZLIB_GZIP_INFLATING)
    ...
    while (!done) {
        ...
        if (z->avail_out != DSIZ) {
            if (status == Z_OK || status == Z_STREAM_END) {
                ...
                result = Curl_unencode_write(conn, writer->downstream, decomp, DSIZ - z->avail_out);
                if (result) {
...
/* Deflate handler. */
static CURLcode deflate_init_writer(struct connectdata* conn, contenc_writer* writer)
{
    zlib_params* zp = (zlib_params*)&writer->params;
    z_stream* z = &zp->z; /* zlib state structure */
...
static CURLcode deflate_unencode_write(
    struct connectdata* conn, contenc_writer* writer, const char* buf, size_t nbytes)
{
    zlib_params* zp = (zlib_params*)&writer->params;
    z_stream* z = &zp->z; /* zlib state structure */
...
static void deflate_close_writer(struct connectdata* conn, contenc_writer* writer)
{
    zlib_params* zp = (zlib_params*)&writer->params;
    z_stream* z = &zp->z; /* zlib state structure */
...
/* Gzip handler. */
static CURLcode gzip_init_writer(struct connectdata* conn, contenc_writer* writer)
{
    zlib_params* zp = (zlib_params*)&writer->params;
    z_stream* z = &zp->z; /* zlib state structure */
...
static CURLcode gzip_unencode_write(
    struct connectdata* conn, contenc_writer* writer, const char* buf, size_t nbytes)
{
    zlib_params* zp = (zlib_params*)&writer->params;
    z_stream* z = &zp->z; /* zlib state structure */
...
static void gzip_close_writer(struct connectdata* conn, contenc_writer* writer)
{
    zlib_params* zp = (zlib_params*)&writer->params;
    z_stream* z = &zp->z; /* zlib state structure */
...
static CURLcode brotli_init_writer(struct connectdata* conn, contenc_writer* writer)
{
...
static CURLcode brotli_unencode_write(
    struct connectdata* conn, contenc_writer* writer, const char* buf, size_t nbytes)
{
    brotli_params* bp = (brotli_params*)&writer->params;
    ...
    while ((nbytes || r == BROTLI_DECODER_RESULT_NEEDS_MORE_OUTPUT) && result == CURLE_OK) {
        dst = (uint8_t*)decomp;
        dstleft = DSIZ;
        r = BrotliDecoderDecompressStream(bp->br, &nbytes, &src, &dstleft, &dst, NULL);
        result = Curl_unencode_write(conn, writer->downstream, decomp, DSIZ - dstleft);
        if (result)
            break;
        switch (r) {
...
static void brotli_close_writer(struct connectdata* conn, contenc_writer* writer)
{
    brotli_params* bp = (brotli_params*)&writer->params;
...
/* Identity handler. */
static CURLcode identity_init_writer(struct connectdata* conn, contenc_writer* writer)
{
    (void)conn;
    return writer->downstream ? CURLE_OK : CURLE_WRITE_ERROR;
}

static CURLcode identity_unencode_write(
    struct connectdata* conn, contenc_writer* writer, const char* buf, size_t nbytes)
{
    return Curl_unencode_write(conn, writer->downstream, buf, nbytes);
}

static void identity_close_writer(struct connectdata* conn, contenc_writer* writer)
{
    (void)conn;
    (void)writer;
}
...
static const content_encoding identity_encoding = {
    "identity", "none", identity_init_writer, identity_unencode_write, identity_close_writer, 0};
...
static CURLcode client_init_writer(struct connectdata* conn, contenc_writer* writer)
{
    (void)conn;
...
static void client_close_writer(struct connectdata* conn, contenc_writer* writer)
{
    (void)conn;
    (void)writer;
}
...
static const content_encoding client_encoding = {
    NULL, NULL, client_init_writer, client_unencode_write, client_close_writer, 0};

/* Deferred error dummy writer. */
static CURLcode error_init_writer(struct connectdata* conn, contenc_writer* writer)
{
    (void)conn;
    return writer->downstream ? CURLE_OK : CURLE_WRITE_ERROR;
}

static CURLcode error_unencode_write(
    struct connectdata* conn, contenc_writer* writer, const char* buf, size_t nbytes)
{
    ...
    failf(
        ...
        "libcurl understands %s content encodings.",
        all);
    free(all);
    return CURLE_BAD_CONTENT_ENCODING;
}

static void error_close_writer(struct connectdata* conn, contenc_writer* writer)
{
    (void)conn;
    (void)writer;
}
...
static const content_encoding error_encoding = {
    NULL, NULL, error_init_writer, error_unencode_write, error_close_writer, 0};

/* Create an unencoding writer stage using the given handler. */
static contenc_writer* new_unencoding_writer(
    struct connectdata* conn, const content_encoding* handler, contenc_writer* downstream)
{
...
CURLcode
Curl_build_unencoding_stack(struct connectdata* conn, const char* enclist, int maybechunked)
{
    struct Curl_easy* data = conn->data;
    struct SingleRequest* k = &data->req;
