...
static size_t Curl_trailers_read(char* buffer, size_t size, size_t nitems, void* raw)
{
    ...
    if (to_copy) {
        memcpy(buffer, &trailers_buf->buffer[data->state.trailers_bytes_sent], to_copy);
...
CURLcode Curl_fillreadbuffer(struct connectdata* conn, size_t bytes, size_t* nreadp)
{
    ...
    if (!data->req.forbidchunk && data->req.upload_chunky) {
        ...
        if (data->state.trailers_state != TRAILERS_SENDING) {
...
#ifndef CURL_DISABLE_HTTP
            if ((nread - hexlen) == 0 && data->set.trailer_callback != NULL &&
            ...
            {
                memcpy(
                    data->req.upload_fromhere + nread,
                    endofline_network,
...
CURLcode Curl_readrewind(struct connectdata* conn)
{
    ...
    */
    ...
    if (data->set.postfields)
        ; /* do nothing */
    else if (data->set.httpreq == HTTPREQ_POST_MIME || data->set.httpreq == HTTPREQ_POST_FORM) {
 ...
 */
static CURLcode readwrite_data(
    ...
    do {
        ...
        *didwhat |= KEEP_RECV;
        ...
        if (k->str && !k->header && (nread > 0 || is_empty_data)) {
            ...
            if (0 == k->bodywrites && !is_empty_data) {
                ...
                if (conn->handler->protocol & (PROTO_FAMILY_HTTP | CURLPROTO_RTSP)) {
                    ...
                    if (data->state.resume_from && !k->content_range &&
                        (data->set.httpreq == HTTPREQ_GET) && !k->ignorebody) {
            ...
            if (data->set.verbose) {
                if (k->badheader) {
                    Curl_debug(data, CURLINFO_DATA_IN, data->state.headerbuff, (size_t)k->hbuflen);
            ...
            if (k->chunk) {
                 ...
                 */
                CURLcode extra;
                CHUNKcode res = Curl_httpchunk_read(conn, k->str, nread, &nread, &extra);
                ...
                if (k->badheader && !k->ignorebody) {
                    ...
                    if (k->maxdownload == -1 || (curl_off_t)k->hbuflen <= k->maxdownload)
                        result = Curl_client_write(
                            conn, CLIENTWRITE_BODY, data->state.headerbuff, k->hbuflen);
                    else
                        result = Curl_client_write(
                            conn, CLIENTWRITE_BODY, data->state.headerbuff, (size_t)k->maxdownload);
                    ...
                    if (conn->data->set.http_ce_skip || !k->writer_stack) {
                        if (!k->ignorebody) {
                            ...
                            if (conn->handler->protocol & PROTO_FAMILY_POP3)
                                ...
                                result = Curl_client_write(conn, CLIENTWRITE_BODY, k->str, nread);
                        }
...
static void win_update_buffer_size(curl_socket_t sockfd)
{
    ...
    DWORD ideallen;
    result = WSAIoctl(
        sockfd, SIO_IDEAL_SEND_BACKLOG_QUERY, 0, 0, &ideal, sizeof(ideal), &ideallen, 0, 0);
    if (result == 0) {
    ...
    *didwhat |= KEEP_SEND;
    ...
    do {
        ...
        if (data->set.verbose)
            ...
            Curl_debug(data, CURLINFO_DATA_OUT, k->upload_fromhere, (size_t)bytes_written);
 ...
 */
CURLcode
Curl_readwrite(struct connectdata* conn, struct Curl_easy* data, bool* done, bool* comeback)
{
    struct SingleRequest* k = &data->req;
    CURLcode result;
    int didwhat = 0;
    ...
    if (k->keepon) {
        if (0 > Curl_timeleft(data, &k->now, FALSE)) {
            if (k->size != -1) {
                failf(
                    data,
                    "Operation timed out after %" CURL_FORMAT_TIMEDIFF_T
                    " milliseconds with %" CURL_FORMAT_CURL_OFF_T " out of %" CURL_FORMAT_CURL_OFF_T
                    " bytes received",
        ...
        if (!(data->set.opt_no_body) && (k->size != -1) && (k->bytecount != k->size) &&
#ifdef CURL_DO_LINEEND_CONV
            ...
            */
            ...
            failf(
                data,
                "transfer closed with %" CURL_FORMAT_CURL_OFF_T " bytes remaining to read",
                k->size - k->bytecount);
    ...
    *done = (0 == (k->keepon & (KEEP_RECV | KEEP_SEND | KEEP_RECV_PAUSE | KEEP_SEND_PAUSE))) ?
                TRUE :
                FALSE;
 ...
 */
int Curl_single_getsock(const struct connectdata* conn, curl_socket_t* sock)
{
    ...
    switch (data->info.httpcode) {
        ...
        case 303: /* See Other */
            ...
            if (data->set.httpreq != HTTPREQ_GET &&
                ((data->set.httpreq != HTTPREQ_POST && data->set.httpreq != HTTPREQ_POST_FORM &&
                  data->set.httpreq != HTTPREQ_POST_MIME) ||
...
CURLcode Curl_retry_request(struct connectdata* conn, char** url)
{
    ...
    *url = NULL;
    ...
    if ((data->req.bytecount + data->req.headerbytecount == 0) && conn->bits.reuse &&
...
)
{
    ...
    struct HTTP* http = data->req.protop;
    bool httpsending =
        ((conn->handler->protocol & PROTO_FAMILY_HTTP) && (http->sending == HTTPSEND_REQUEST));
    ...
    if (k->getheader || !data->set.opt_no_body) {
        ...
        if (writesockindex != -1) {
            ...
            */
            if ((data->state.expect100header) && (conn->handler->protocol & PROTO_FAMILY_HTTP) &&
