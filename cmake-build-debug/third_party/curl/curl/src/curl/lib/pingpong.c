...
time_t Curl_pp_state_timeout(struct pingpong* pp, bool disconnecting)
{
    ...
    long response_time =
        (data->set.server_response_timeout) ? data->set.server_response_timeout : pp->response_time;
    ...
    timeout_ms = response_time - (time_t)Curl_timediff(Curl_now(), pp->response); /* spent time */
...
CURLcode Curl_pp_statemach(struct pingpong* pp, bool block, bool disconnecting)
{
    ...
    if (Curl_ssl_data_pending(conn, FIRSTSOCKET))
        ...
        rc = Curl_socket_check(
            pp->sendleft ? CURL_SOCKET_BAD : sock, /* reading */
            CURL_SOCKET_BAD,
    ...
    while ((pp->nread_resp < (size_t)data->set.buffer_size) && (keepon && !result)) {
...
#endif
            DEBUGASSERT(
                (ptr + data->set.buffer_size - pp->nread_resp) <=
                (buf + data->set.buffer_size + 1));
            result =
                Curl_read(conn, sockfd, ptr, data->set.buffer_size - pp->nread_resp, &gotbytes);
#ifdef HAVE_GSSAPI
        ...
        if (!keepon)
            ...
            for (i = 0; i < gotbytes; ptr++, i++) {
                ...
                if (*ptr == '\n') {
                    ...
                    if (!conn->sec_complete)
                        ...
                        if (data->set.verbose)
                            Curl_debug(
                                data, CURLINFO_HEADER_IN, pp->linestart_resp, (size_t)perline);
                     ...
                     */
                    result =
                        Curl_client_write(conn, CLIENTWRITE_HEADER, pp->linestart_resp, perline);
                    if (result)
                        return result;
            ...
            if (!keepon && (i != gotbytes)) {
                ...
                DEBUGF(infof(
                    data,
                    "Curl_pp_readresp_ %d bytes of trailing "
                ...
                if ((perline == gotbytes) && (gotbytes > data->set.buffer_size / 2)) {
                    ...
                    infof(
                        data,
                        "Excessive server response line length received, "
                        "%zd bytes. Stripping\n",
                        gotbytes);
                    restart = TRUE;
...
CURLcode Curl_pp_flushsend(struct pingpong* pp)
{
    ...
    curl_socket_t sock = conn->sock[FIRSTSOCKET];
    CURLcode result =
        Curl_write(conn, sock, pp->sendthis + pp->sendsize - pp->sendleft, pp->sendleft, &written);
    if (result)
        return result;
