...
static void Curl_ares_sock_state_cb(void* data, ares_socket_t socket_fd, int readable, int writable)
{
...
int Curl_resolver_getsock(struct connectdata* conn, curl_socket_t* socks)
{
    ...
    long milli;
    int max = ares_getsock(
        (ares_channel)conn->data->state.resolver, (ares_socket_t*)socks, MAX_SOCKSPEREASYHANDLE);
...
static int waitperform(struct connectdata* conn, int timeout_ms)
{
    ...
    if (!nfds)
        ...
        for (i = 0; i < num; i++)
            ares_process_fd(
                (ares_channel)data->state.resolver,
...
CURLcode Curl_resolver_is_resolved(struct connectdata* conn, struct Curl_dns_entry** dns)
{
    ...
    if (res &&
        ...
        (Curl_timediff(Curl_now(), res->happy_eyeballs_dns_time) >= HAPPY_EYEBALLS_DNS_TIMEOUT)) {
        /* Remember that the EXPIRE_HAPPY_EYEBALLS_DNS timer is no longer
           running. */
        memset(&res->happy_eyeballs_dns_time, 0, sizeof(res->happy_eyeballs_dns_time));
    ...
    if (res && !res->num_pending) {
        ...
        if (!conn->async.dns) {
            ...
            result = conn->bits.proxy ? CURLE_COULDNT_RESOLVE_PROXY : CURLE_COULDNT_RESOLVE_HOST;
        } else if (dns)
            *dns = conn->async.dns;
 ...
 */
static void query_completed_cb(
    void* arg, /* (struct connectdata *) */
    int status,
    ...
    if (res) {
        ...
        if (res->num_pending
            ...
            */
            ...
            Curl_expire(conn->data, HAPPY_EYEBALLS_DNS_TIMEOUT, EXPIRE_HAPPY_EYEBALLS_DNS);
        }
    }
}
 ...
 */
Curl_addrinfo*
Curl_resolver_getaddrinfo(struct connectdata* conn, const char* hostname, int port, int* waitp)
{
    char* bufp;
    struct Curl_easy* data = conn->data;
    int family = PF_INET;
    ...
    *waitp = 0; /* default to synchronous response */
    ...
    if (bufp) {
...
#endif /* CURLRES_IPV6 */
        {
            ...
            /* areschannel is already setup in the Curl_open() function */
            ares_gethostbyname(
                (ares_channel)data->state.resolver, hostname, family, query_completed_cb, conn);
        }
