...
static CURLcode smtp_parse_address(
    struct connectdata* conn, const char* fqma, char** address, struct hostname* host);
...
static CURLcode smtp_perform_upgrade_tls(struct connectdata* conn)
{
    ...
    struct smtp_conn* smtpc = &conn->proto.smtpc;
    CURLcode result = Curl_ssl_connect_nonblocking(conn, FIRSTSOCKET, &smtpc->ssldone);
...
static CURLcode smtp_perform_command(struct connectdata* conn)
{
    ...
    if (smtp->rcpt) {
        ...
        if ((!smtp->custom) || (!smtp->custom[0])) {
            ...
            result = smtp_parse_address(conn, smtp->rcpt->data, &address, &host);
            if (result)
                return result;
...
static CURLcode smtp_perform_mail(struct connectdata* conn)
{
    ...
    if (data->set.str[STRING_MAIL_FROM]) {
        ...
        result = smtp_parse_address(conn, data->set.str[STRING_MAIL_FROM], &address, &host);
        if (result)
            return result;
        ...
        utf8 =
            (conn->proto.smtpc.utf8_supported) &&
    ...
    if (data->set.str[STRING_MAIL_AUTH] && conn->proto.smtpc.sasl.authused) {
        if (data->set.str[STRING_MAIL_AUTH][0] != '\0') {
            ...
            /* Parse the AUTH mailbox into the local address and host name parts,
               converting the host name to an IDN A-label if necessary */
            result = smtp_parse_address(conn, data->set.str[STRING_MAIL_AUTH], &address, &host);
            if (result) {
                free(from);
                return result;
            }
...
static CURLcode smtp_perform_rcpt_to(struct connectdata* conn)
{
    ...
    if (host.name)
        result = Curl_pp_sendf(&conn->proto.smtpc.pp, "RCPT TO:<%s@%s>", address, host.name);
    else
        /* An invalid mailbox was provided but we'll simply let the server worry
           about that and reply with a 501 error */
...
static CURLcode
smtp_state_servergreet_resp(struct connectdata* conn, int smtpcode, smtpstate instate)
{
    CURLcode result = CURLE_OK;
    struct Curl_easy* data = conn->data;
    ...
    if (smtpcode / 100 != 2 && smtpcode != 1) {
        ...
        /* Does the server support authentication? */
            ...
            for (;;) {
                ...
                /* Extract the word */
                for (wordlen = 0; wordlen < len && line[wordlen] != ' ' && line[wordlen] != '\t' &&
...
static CURLcode smtp_block_statemach(struct connectdata* conn, bool disconnecting)
{
    CURLcode result = CURLE_OK;
    struct smtp_conn* smtpc = &conn->proto.smtpc;
...
static CURLcode smtp_done(struct connectdata* conn, CURLcode status, bool premature)
{
    ...
    if (status) {
        ...
        !data->set.connect_only && data->set.mail_rcpt &&
