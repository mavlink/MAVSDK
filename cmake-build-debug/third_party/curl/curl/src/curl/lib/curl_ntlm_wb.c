...
static CURLcode ntlm_wb_init(struct Curl_easy* data, struct ntlmdata* ntlm, const char* userp)
{
    ...
    if (ntlm->ntlm_auth_hlpr_socket != CURL_SOCKET_BAD || ntlm->ntlm_auth_hlpr_pid)
        return CURLE_OK;
...
#if defined(HAVE_GETPWUID_R) && defined(HAVE_GETEUID)
        if ((!username || !username[0]) &&
            !getpwuid_r(geteuid(), &pw, pwbuf, sizeof(pwbuf), &pw_res) && pw_res) {
            username = pw.pw_name;
        }
#endif
        if (!username || !username[0])
...
static CURLcode
ntlm_wb_response(struct Curl_easy* data, struct ntlmdata* ntlm, const char* input, curlntlm state)
{
    ...
    if (len_out < 4)
        goto done;
    if (state == NTLMSTATE_TYPE1 && (buf[0] != 'Y' || buf[1] != 'R' || buf[2] != ' '))
        goto done;
    if (state == NTLMSTATE_TYPE2 && (buf[0] != 'K' || buf[1] != 'K' || buf[2] != ' ') &&
