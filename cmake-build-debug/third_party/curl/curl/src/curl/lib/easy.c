...
static int events_timer(
    struct Curl_multi* multi, /* multi handle */
 ...
 * monitor.
 */
static int events_socket(
    struct Curl_easy* easy, /* easy handle */
...
struct Curl_easy* curl_easy_duphandle(struct Curl_easy* data)
{
    ...
    if (data->cookies) {
        ...
        outcurl->cookies = Curl_cookie_init(
            data, data->cookies->filename, outcurl->cookies, data->set.cookiesession);
        if (!outcurl->cookies)
            goto fail;
    }
    ...
    if (!(newstate & KEEP_RECV_PAUSE)) {
        ...
        if (data->state.tempcount) {
            ...
            for (i = 0; i < count; i++) {
                ...
                if (!result)
                    result =
                        Curl_client_write(conn, writebuf[i].type, writebuf[i].buf, writebuf[i].len);
                free(writebuf[i].buf);
            }
...
static CURLcode
easy_connection(struct Curl_easy* data, curl_socket_t* sfd, struct connectdata** connp)
{
    if (data == NULL)
        return CURLE_BAD_FUNCTION_ARGUMENT;
