...
static int
ldap_win_bind_auth(LDAP* server, const char* user, const char* passwd, unsigned long authflags)
{
    ULONG method = 0;
...
static int
ldap_win_bind(struct connectdata* conn, LDAP* server, const char* user, const char* passwd)
{
    ...
    *done = TRUE; /* unconditionally */
    ...
    if (ldap_ssl) {
        ...
        if (conn->ssl_config.verifypeer) {
            ...
            if ((data->set.ssl.cert_type) && (strcasecompare(data->set.ssl.cert_type, "DER")))
        ...
        if (rc != LDAP_SUCCESS) {
            failf(data, "LDAP local: ERROR setting cert verify mode: %s", ldap_err2string(rc));
            result = CURLE_SSL_CERTPROBLEM;
            goto quit;
        }
        server = ldapssl_init(host, (int)conn->port, 1);
        if (server == NULL) {
            failf(data, "LDAP local: Cannot connect to %s:%ld", conn->host.dispname, conn->port);
        ...
        if (conn->ssl_config.verifypeer) {
            ...
            if ((data->set.ssl.cert_type) && (!strcasecompare(data->set.ssl.cert_type, "PEM"))) {
        ...
        if (rc != LDAP_SUCCESS) {
            failf(data, "LDAP local: ERROR setting SSL/TLS mode: %s", ldap_err2string(rc));
            result = CURLE_SSL_CERTPROBLEM;
            goto quit;
        }
/*
...
*/
    ...
    rc = ldap_search_s(
        server, ludp->lud_dn, ludp->lud_scope, ludp->lud_filter, ludp->lud_attrs, 0, &ldapmsg);
    ...
    for (num = 0, entryIterator = ldap_first_entry(server, ldapmsg); entryIterator;
        ...
        for (attribute = ldap_first_attribute(server, entryIterator, &ber); attribute;
            ...
            if (vals != NULL) {
                for (i = 0; (vals[i] != NULL); i++) {
                    ...
                    result = Curl_client_write(conn, CLIENTWRITE_BODY, (char*)attr, attr_len);
                    if (result) {
                        ldap_value_free_len(vals);
#if defined(USE_WIN32_LDAP)
                        Curl_unicodefree(attr);
                    ...
                    if ((attr_len > 7) && (strcmp(";binary", (char*)attr + (attr_len - 7)) == 0)) {
                        /* Binary attribute, encode to base64. */
                        result = Curl_base64_encode(
                            data, vals[i]->bv_val, vals[i]->bv_len, &val_b64, &val_b64_sz);
                        if (result) {
                            ldap_value_free_len(vals);
#if defined(USE_WIN32_LDAP)
                            Curl_unicodefree(attr);
                        ...
                        result = Curl_client_write(
                            conn, CLIENTWRITE_BODY, vals[i]->bv_val, vals[i]->bv_len);
                        if (result) {
                            ldap_value_free_len(vals);
#if defined(USE_WIN32_LDAP)
                            Curl_unicodefree(attr);
...
static int _ldap_url_parse2(const struct connectdata* conn, LDAPURLDesc* ludp)
{
    ...
    if (!conn->data || !conn->data->state.up.path || conn->data->state.up.path[0] != '/' ||
    ...
    if (*p) {
        ...
        for (i = 0; i < count; i++) {
            ...
            result = Curl_urldecode(conn->data, attributes[i], 0, &unescaped, NULL, FALSE);
            if (result) {
                free(attributes);
