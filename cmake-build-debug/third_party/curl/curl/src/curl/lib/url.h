...
CURLcode Curl_parse_login_details(
    const char* login, const size_t len, char** userptr, char** passwdptr, char** optionsptr);
...
#define CONNECT_PROXY_SSL() \
    (conn->http_proxy.proxytype == CURLPROXY_HTTPS && !conn->bits.proxy_ssl_connected[sockindex])

#define CONNECT_FIRSTSOCKET_PROXY_SSL() \
    (conn->http_proxy.proxytype == CURLPROXY_HTTPS && !conn->bits.proxy_ssl_connected[FIRSTSOCKET])
