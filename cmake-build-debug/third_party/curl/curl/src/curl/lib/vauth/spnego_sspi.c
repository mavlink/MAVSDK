...
bool Curl_auth_is_spnego_supported(void)
{
    ...
    status = s_pSecFn->QuerySecurityPackageInfo((TCHAR*)TEXT(SP_NAME_NEGOTIATE), &SecurityPackage);
 ...
 */
CURLcode Curl_auth_decode_spnego_message(
    ...
    struct negotiatedata* nego)
{
    ...
    if (!nego->output_token) {
        /* Query the security package for Negotiate */
        nego->status =
            s_pSecFn->QuerySecurityPackageInfo((TCHAR*)TEXT(SP_NAME_NEGOTIATE), &SecurityPackage);
    ...
    if (!nego->credentials) {
        ...
        nego->status = s_pSecFn->AcquireCredentialsHandle(
            NULL,
    ...
    if (chlg64 && *chlg64) {
        ...
        if (nego->sslContext) {
            ...
            nego->status = s_pSecFn->QueryContextAttributes(
                nego->sslContext, SECPKG_ATTR_ENDPOINT_BINDINGS, &pkgBindings);
            if (nego->status == SEC_E_OK) {
                chlg_desc.cBuffers++;
 ...
 * Returns CURLE_OK on success.
 */
CURLcode Curl_auth_create_spnego_message(
    struct Curl_easy* data, struct negotiatedata* nego, char** outptr, size_t* outlen)
{
    CURLcode result;

    /* Base64 encode the already generated response */
    result = Curl_base64_encode(
        data, (const char*)nego->output_token, nego->output_token_length, outptr, outlen);
