...
#define LONGQUARTET(x) \
    ((int)((x)&0xff)), ((int)(((x) >> 8) & 0xff)), ((int)(((x) >> 16) & 0xff)), \
        ((int)(((x) >> 24) & 0xff))
 ...
 */
static CURLcode ntlm_decode_type2_target(
    struct Curl_easy* data, unsigned char* buffer, size_t size, struct ntlmdata* ntlm)
{
    ...
    if (size >= 48) {
        ...
        if (target_info_len > 0) {
            if ((target_info_offset >= size) || ((target_info_offset + target_info_len) > size) ||
...
CURLcode Curl_auth_decode_ntlm_type2_message(
    struct Curl_easy* data, const char* type2msg, struct ntlmdata* ntlm)
{
    ...
    */
    ...
    if ((type2_len < 32) || (memcmp(type2, NTLMSSP_SIGNATURE, 8) != 0) ||
...
#define NTLM2FLAG 0
    ...
    msnprintf(
        ...
        0,
        0,
        0, /* part of type-1 long */

        LONGQUARTET(
            NTLMFLAG_NEGOTIATE_OEM | NTLMFLAG_REQUEST_TARGET | NTLMFLAG_NEGOTIATE_NTLM_KEY |
            NTLM2FLAG | NTLMFLAG_NEGOTIATE_ALWAYS_SIGN),
        SHORTPAIR(domlen),
        SHORTPAIR(domlen),
        SHORTPAIR(domoff),
        0,
...

{
    ...
    */
    ...
    if (ntlm->flags & NTLMFLAG_NEGOTIATE_NTLM2_KEY) {
        ...
        /* LMv2 response */
        result = Curl_ntlm_core_mk_lmv2_resp(ntlmv2hash, entropy, &ntlm->nonce[0], lmresp);
        if (result)
            return result;

        /* NTLMv2 response */
        result = Curl_ntlm_core_mk_ntlmv2_resp(ntlmv2hash, entropy, ntlm, &ntlmv2resp, &ntresplen);
        if (result)
            return result;
    ...
    size = msnprintf(
        ...
        0,
        0,
        0, /* type-3 long, the 24 upper bits */
