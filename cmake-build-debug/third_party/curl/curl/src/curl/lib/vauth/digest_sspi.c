    ...
    size_t* outlen)
{
    ...
    status = s_pSecFn->InitializeSecurityContext(
        &credentials, NULL, spn, 0, 0, 0, &chlg_desc, 0, &context, &resp_desc, &attrs, &expiry);
...
CURLcode Curl_auth_decode_digest_http_message(const char* chlg, struct digestdata* digest)
{
    ...
    if (digest->input_token) {
        ...
        for (;;) {
            ...
            if (strcasecompare(value, "stale") && strcasecompare(content, "true")) {
                stale = true;
                break;
            }
    ...
    size_t* outlen)
{
    ...
    if ((userp && !digest->user) || (!userp && digest->user) || (passwdp && !digest->passwd) ||
        (!passwdp && digest->passwd) || (userp && digest->user && strcmp(userp, digest->user)) ||
