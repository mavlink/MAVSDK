    ...
    size_t* outlen)
{
    ...
    if (!krb5->spn) {
        ...
        /* Import the SPN */
        major_status =
            gss_import_name(&minor_status, &spn_token, GSS_C_NT_HOSTBASED_SERVICE, &krb5->spn);
    ...
    if (output_token.value && output_token.length) {
        ...
        result = Curl_base64_encode(
            data, (char*)output_token.value, output_token.length, outptr, outlen);
    ...
    size_t* outlen)
{
    ...
    major_status = gss_inquire_context(
        &minor_status, krb5->context, &username, NULL, NULL, NULL, NULL, NULL, NULL);
    if (GSS_ERROR(major_status)) {
        Curl_gss_log_error(data, "gss_inquire_context() failed: ", major_status, minor_status);
    ...
    major_status = gss_display_name(&minor_status, username, &username_token, NULL);
    if (GSS_ERROR(major_status)) {
        Curl_gss_log_error(data, "gss_display_name() failed: ", major_status, minor_status);
    ...
    major_status =
        gss_unwrap(&minor_status, krb5->context, &input_token, &output_token, NULL, &qop);
    if (GSS_ERROR(major_status)) {
    ...
    major_status = gss_wrap(
        &minor_status, krb5->context, 0, GSS_C_QOP_DEFAULT, &input_token, NULL, &output_token);
    if (GSS_ERROR(major_status)) {
    ...
    /* Base64 encode the response */
    result =
        Curl_base64_encode(data, (char*)output_token.value, output_token.length, outptr, outlen);
