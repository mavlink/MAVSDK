...
bool Curl_auth_is_ntlm_supported(void)
{
    ...
    status = s_pSecFn->QuerySecurityPackageInfo((TCHAR*)TEXT(SP_NAME_NTLM), &SecurityPackage);
    ...
    size_t* outlen)
{
    ...
    return Curl_base64_encode(data, (char*)ntlm->output_token, type_1_buf.cbBuffer, outptr, outlen);
}
    ...
    size_t* outlen)
{
    ...
    if (ntlm->sslContext) {
        ...
        status = s_pSecFn->QueryContextAttributes(
            ntlm->sslContext, SECPKG_ATTR_ENDPOINT_BINDINGS, &pkgBindings);
        if (status == SEC_E_OK) {
    ...
    /* Base64 encode the response */
    result =
        Curl_base64_encode(data, (char*)ntlm->output_token, type_3_buf.cbBuffer, outptr, outlen);
