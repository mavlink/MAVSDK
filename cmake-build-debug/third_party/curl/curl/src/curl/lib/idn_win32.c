...
WINNORMALIZEAPI int WINAPI IdnToAscii(
    _In_ DWORD dwFlags,
...
WINNORMALIZEAPI int WINAPI IdnToUnicode(
    _In_ DWORD dwFlags,
    ...
    _In_ int cchUnicodeChar);
#else
WINBASEAPI int WINAPI IdnToAscii(
    DWORD dwFlags,
    const WCHAR* lpUnicodeCharStr,
    int cchUnicodeChar,
    WCHAR* lpASCIICharStr,
    int cchASCIIChar);
WINBASEAPI int WINAPI IdnToUnicode(
    DWORD dwFlags,
...
bool curl_win32_ascii_to_idn(const char* in, char** out)
{
    ...
    if (in_w) {
        ...
        int chars = IdnToUnicode(0, in_w, curlx_uztosi(in_len), unicode, IDN_MAX_LENGTH);
        free(in_w);
        if (chars) {
