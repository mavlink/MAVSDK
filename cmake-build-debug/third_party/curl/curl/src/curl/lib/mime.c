...
#if (!defined(CURL_DISABLE_HTTP) && !defined(CURL_DISABLE_MIME)) || !defined(CURL_DISABLE_SMTP) || \
...
static size_t
mime_subparts_read(char* buffer, size_t size, size_t nitems, void* instream, bool* hasread);
...
static int qp_lookahead_eol(mime_encoder_state* st, int ateof, size_t n)
{
    ...
    if (n + 2 > st->bufend)
        return ateof ? 0 : -1;
    if (qp_class[st->buf[n] & 0xFF] == QP_CR && qp_class[st->buf[n + 1] & 0xFF] == QP_LF)
        return 1;
    return 0;
}
...
static size_t
read_encoded_part_content(curl_mimepart* part, char* buffer, size_t bufsize, bool* hasread)
{
    mime_encoder_state* st = &part->encstate;
    size_t cursize = 0;
    size_t sz;
        ...
        switch (part->state.state) {
            ...
            case MIMESTATE_CURLHEADERS:
                if (!hdr)
                    ...
                    sz = readback_bytes(
                        &part->state, buffer, bufsize, hdr->data, strlen(hdr->data), "\r\n");
                    if (!sz)
            ...
            case MIMESTATE_BODY:
                ...
                if (part->easy && convbuf < buffer) {
                    CURLcode result =
                        Curl_convert_to_network(part->easy, convbuf, buffer - convbuf);
                    if (result)
                        return READ_ERROR;
                    convbuf = buffer;
                }
...
#ifdef CURL_DOES_CONVERSIONS
    if (part->easy && convbuf < buffer && part->state.state < MIMESTATE_BODY) {
            ...
            case MIMESTATE_BOUNDARY2:
                ...
                if (!sz) {
                    ...
                    if (mime->easy && convbuf < buffer) {
                        CURLcode result =
                            Curl_convert_to_network(mime->easy, convbuf, buffer - convbuf);
                        if (result)
                            return READ_ERROR;
                        convbuf = buffer;
                    }
    ...
    switch (src->kind) {
        ...
        case MIMEKIND_CALLBACK:
            res = curl_mime_data_cb(
                dst, src->datasize, src->readfunc, src->seekfunc, src->freefunc, src->arg);
            break;
        case MIMEKIND_MULTIPART:
...
curl_mime* curl_mime_init(struct Curl_easy* easy)
{
    ...
    if (mime) {
        ...
        if (Curl_rand_hex(
                easy, (unsigned char*)&mime->boundary[24], MIME_RAND_BOUNDARY_CHARS + 1)) {
...
static CURLcode add_content_type(struct curl_slist** slp, const char* type, const char* boundary)
{
    return Curl_mime_add_header(
        slp, "Content-Type: %s%s%s", type, boundary ? "; boundary=" : "", boundary ? boundary : "");
}
    ...
    enum mimestrategy strategy)
{
    ...
    if (!search_header(part->userheaders, "Content-Disposition")) {
        ...
        if (disposition && curl_strequal(disposition, "attachment") && !part->name &&
            !part->filename)
            disposition = NULL;
        if (disposition) {
            char* name = NULL;
