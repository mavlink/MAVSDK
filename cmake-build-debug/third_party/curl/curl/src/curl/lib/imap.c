...
static CURLcode
imap_perform_authenticate(struct connectdata* conn, const char* mech, const char* initresp);
...
static bool imap_endofresp(struct connectdata* conn, char* line, size_t len, int* resp)
{
            ...
            case IMAP_LIST:
                if ((!imap->custom && !imap_matchresp(line, len, "LIST")) ||
                    (imap->custom && !imap_matchresp(line, len, imap->custom) &&
                     ...
                     !strcasecompare(imap->custom, "UID") && !strcasecompare(imap->custom, "NOOP")))
                    return FALSE;
                break;
    ...
    if (imap && !imap->custom &&
        ((len == 3 && line[0] == '+') || (len >= 2 && !memcmp("+ ", line, 2)))) {
        switch (imapc->state) {
...
static void imap_get_message(char* buffer, char** outptr)
{
    ...
    if (len > 2) {
        ...
        len -= 2;
        for (message = buffer + 2; *message == ' ' || *message == '\t'; message++, len--)
            ;
...
static CURLcode imap_perform_list(struct connectdata* conn)
{
    ...
    if (imap->custom)
        ...
        result =
            imap_sendf(conn, "%s%s", imap->custom, imap->custom_params ? imap->custom_params : "");
    else {
...
static CURLcode imap_perform_fetch(struct connectdata* conn)
{
    ...
    if (imap->uid) {
        ...
        if (imap->partial)
            ...
            result = imap_sendf(
                conn, "UID FETCH %s BODY[%s]", imap->uid, imap->section ? imap->section : "");
    } else if (imap->mindex) {
        /* Send the FETCH command */
        if (imap->partial)
            result = imap_sendf(
            ...
            result = imap_sendf(
                conn, "FETCH %s BODY[%s]", imap->mindex, imap->section ? imap->section : "");
    } else {
...
static CURLcode imap_perform_append(struct connectdata* conn)
{
    ...
    result = imap_sendf(
        conn, "APPEND %s (\\Seen) {%" CURL_FORMAT_CURL_OFF_T "}", mailbox, data->state.infilesize);
...
static CURLcode
imap_state_servergreet_resp(struct connectdata* conn, int imapcode, imapstate instate)
{
    struct Curl_easy* data = conn->data;
...
static CURLcode
imap_state_capability_resp(struct connectdata* conn, int imapcode, imapstate instate)
{
    CURLcode result = CURLE_OK;
    struct Curl_easy* data = conn->data;
    struct imap_conn* imapc = &conn->proto.imapc;
    ...
    if (imapcode == '*') {
        ...
        for (;;) {
            ...
            /* Extract the word */
            for (wordlen = 0; line[wordlen] && line[wordlen] != ' ' && line[wordlen] != '\t' &&
                              line[wordlen] != '\r' && line[wordlen] != '\n';)
                wordlen++;
...
static CURLcode
imap_state_listsearch_resp(struct connectdata* conn, int imapcode, imapstate instate)
{
    CURLcode result = CURLE_OK;
...
static CURLcode imap_state_fetch_resp(struct connectdata* conn, int imapcode, imapstate instate)
{
    ...
    if (parsed) {
        ...
        if (pp->cache) {
            ...
            infof(
                data,
                "Written %zu bytes, %" CURL_FORMAT_CURL_OFF_TU " bytes are left for transfer\n",
                chunk,
                size - chunk);
...
static CURLcode
imap_state_fetch_final_resp(struct connectdata* conn, int imapcode, imapstate instate)
{
    CURLcode result = CURLE_OK;
...
static CURLcode
imap_state_append_final_resp(struct connectdata* conn, int imapcode, imapstate instate)
{
    CURLcode result = CURLE_OK;
...
static CURLcode imap_done(struct connectdata* conn, CURLcode status, bool premature)
{
    ...
    if (status) {
        ...
        !data->set.connect_only && !imap->custom &&
...
static CURLcode imap_perform(struct connectdata* conn, bool* connected, bool* dophase_done)
{
    ...
    *dophase_done = FALSE; /* not done yet */
    ...
    if (imap->mailbox && imapc->mailbox && strcasecompare(imap->mailbox, imapc->mailbox) &&
    ...
    if (conn->data->set.upload || data->set.mimepost.kind != MIMEKIND_NONE)
        ...
        imap->mailbox && !selected && (imap->custom || imap->uid || imap->mindex || imap->query))
        /* SELECT the mailbox */
...
static CURLcode imap_parse_url_path(struct connectdata* conn)
{
    ...
    if (ptr != begin) {
        ...
        result = Curl_urldecode(data, begin, end - begin, &imap->mailbox, NULL, TRUE);
        if (result)
            return result;
    } else
        imap->mailbox = NULL;
