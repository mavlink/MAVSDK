...
static CURLcode http_perhapsrewind(struct connectdata* conn)
{
    ...
    if ((expectsend == -1) || (expectsend > bytessent)) {
        ...
        if ((data->state.authproxy.picked == CURLAUTH_NTLM) ||
            ...
            if (((expectsend - bytessent) < 2000) || (conn->http_ntlm_state != NTLMSTATE_NONE) ||
            ...
            infof(
                data,
                "NTLM send, close instead of sending %" CURL_FORMAT_CURL_OFF_T " bytes\n",
        ...
        if ((data->state.authproxy.picked == CURLAUTH_NEGOTIATE) ||
            ...
            if (((expectsend - bytessent) < 2000) || (conn->http_negotiate_state != GSS_AUTHNONE) ||
            ...
            infof(
                data,
                "NEGOTIATE send, close instead of sending %" CURL_FORMAT_CURL_OFF_T " bytes\n",
                (curl_off_t)(expectsend - bytessent));
        }
#endif
...
CURLcode Curl_http_auth_act(struct connectdata* conn)
{
    ...
    if ((conn->bits.user_passwd || data->set.str[STRING_BEARER]) &&
        ((data->req.httpcode == 401) || (conn->bits.authneg && data->req.httpcode < 300))) {
    ...
    if (conn->bits.proxy_user_passwd &&
        ((data->req.httpcode == 407) || (conn->bits.authneg && data->req.httpcode < 300))) {
 ...
 */
static CURLcode output_auth_headers(
    ...
    bool proxy)
{
    ...
    if (authstatus->picked == CURLAUTH_NEGOTIATE) {
        ...
        result = Curl_output_digest(
            conn, proxy, (const unsigned char*)request, (const unsigned char*)path);
        if (result)
            return result;
    } else
#endif
                         ...
                         up the proxy tunnel */
{
    ...
    if (!data->state.this_is_a_follow || conn->bits.netrc || !data->state.first_host ||
...
/* when disabled */
CURLcode Curl_http_output_auth(
    struct connectdata* conn, const char* request, const char* path, bool proxytunnel)
{
    (void)conn;
    (void)request;
    (void)path;
...
CURLcode Curl_http_input_auth(
    struct connectdata* conn, bool proxy, const char* auth) /* the first non-space */
{
    /*
    ...
    while (*auth) {
        ...
        if (checkprefix("Negotiate", auth)) {
            ...
            if ((authp->avail & CURLAUTH_NTLM) || (authp->avail & CURLAUTH_NTLM_WB) ||
 ...
 */
CURLcode Curl_add_buffer(Curl_send_buffer** inp, const void* inptr, size_t size)
{
    char* new_rb;
    Curl_send_buffer* in = *inp;
...
bool Curl_compareheader(
    const char* headerline, /* line to check */
...
static CURLcode add_haproxy_protocol_header(struct connectdata* conn)
{
    ...
    result =
        Curl_add_buffer_send(&req_buffer, conn, &conn->data->info.request_size, 0, FIRSTSOCKET);
...
CURLcode Curl_http_done(struct connectdata* conn, CURLcode status, bool premature)
{
    ...
    if (!premature && /* this check is pointless when DONE is called before the
        ...
        !conn->bits.retry && !data->set.connect_only &&
        (data->req.bytecount + data->req.headerbytecount - data->req.deductheadercount) <= 0) {
...
static bool use_http_1_1plus(const struct Curl_easy* data, const struct connectdata* conn)
{
    ...
    if ((data->set.httpversion == CURL_HTTP_VERSION_1_0) && (conn->httpversion <= 10))
        return FALSE;
    return (
        (data->set.httpversion == CURL_HTTP_VERSION_NONE) ||
...
{
#ifdef ENABLE_QUIC
    if ((data->set.httpversion == CURL_HTTP_VERSION_3) || (conn->httpversion == 30))
        return "3";
#endif
...
static CURLcode
expect100(struct Curl_easy* data, struct connectdata* conn, Curl_send_buffer* req_buffer)
{
    CURLcode result = CURLE_OK;
...
CURLcode Curl_http_compile_trailers(
    struct curl_slist* trailers, Curl_send_buffer** buffer, struct Curl_easy* handle)
{
    char* ptr = NULL;
    CURLcode result = CURLE_OK;
    ...
    while (trailers) {
        ...
        if (ptr && *(ptr + 1) == ' ') {
            result = Curl_add_bufferf(buffer, "%s%s", trailers->data, endofline_native);
            if (result)
                return result;
        } else
    ...
    result = Curl_add_buffer(buffer, endofline_network, strlen(endofline_network));
    return result;
}

CURLcode
Curl_add_custom_headers(struct connectdata* conn, bool is_connect, Curl_send_buffer* req_buffer)
{
    char* ptr;
            ...
            if (ptr) {
                ...
                if (*ptr || semicolonp) {
                    ...
                    if (conn->allocptr.host &&
                        ...
                        (conn->httpversion >= 20) && checkprefix("Transfer-Encoding:", compare))
                        /* HTTP/2 doesn't support chunked requests */
                        ;
                    else if (
                        (checkprefix("Authorization:", compare) ||
                         checkprefix("Cookie:", compare)) &&
                        /* be careful of sending this potentially sensitive header to
                           other hosts */
                        (data->state.this_is_a_follow && data->state.first_host &&
                         !data->set.allow_auth_to_other_hosts &&
                         !strcasecompare(data->state.first_host, conn->host.name)))
                        ;
                    else {
...
CURLcode Curl_add_timecondition(const struct connectdata* conn, Curl_send_buffer* req_buffer)
{
    struct Curl_easy* data = conn->data;
    const struct tm* tm;
    struct tm keeptime;
...
#define TE_HEADER "TE: gzip\r\n"
    ...
    switch (httpreq) {
        ...
        case HTTPREQ_POST_FORM:
            ...
            Curl_mime_cleanpart(&http->form);
            result =
                Curl_getformdata(data, &http->form, data->set.httppost, data->state.fread_func);
            if (result)
                return result;
    ...
    if (ptr) {
        ...
        if ((conn->handler->protocol & PROTO_FAMILY_HTTP) &&
            (((httpreq == HTTPREQ_POST_MIME || httpreq == HTTPREQ_POST_FORM) &&
              http->postsize < 0) ||
             ((data->set.upload || httpreq == HTTPREQ_POST) && data->state.infilesize == -1))) {
            if (conn->bits.authneg)
    ...
    if (ptr && (!data->state.this_is_a_follow ||
        ...
        if (((conn->given->protocol & CURLPROTO_HTTPS) && (conn->remote_port == PORT_HTTPS)) ||
            ((conn->given->protocol & CURLPROTO_HTTP) && (conn->remote_port == PORT_HTTP)))
            /* if(HTTPS on port 443) OR (HTTP on port 80) then don't include
               the port number in the host string */
            conn->allocptr.host = aprintf(
                "Host: %s%s%s\r\n",
                conn->bits.ipv6_ip ? "[" : "",
                host,
                conn->bits.ipv6_ip ? "]" : "");
        ...
        if (data->state.resume_from && !data->state.this_is_a_follow) {
            ...
            if (seekerr != CURL_SEEKFUNC_OK) {
                ...
                do {
                    ...
                    size_t actuallyread = data->state.fread_func(
                        data->state.buffer, 1, readthisamountnow, data->state.in);
                    ...
                    if ((actuallyread == 0) || (actuallyread > readthisamountnow)) {
                        ...
                        failf(
                            data,
                            "Could only read %" CURL_FORMAT_CURL_OFF_T " bytes from the input",
                            passed);
                        return CURLE_READ_ERROR;
                    }
    ...
    if (data->state.use_range) {
        ...
        if (((httpreq == HTTPREQ_GET) || (httpreq == HTTPREQ_HEAD)) &&
            ...
            (httpreq == HTTPREQ_POST || httpreq == HTTPREQ_PUT) &&
            ...
            if (data->set.set_resume_from < 0) {
                ...
                conn->allocptr.rangeline = aprintf(
                    "Content-Range: bytes 0-%" CURL_FORMAT_CURL_OFF_T "/%" CURL_FORMAT_CURL_OFF_T
                    "\r\n",
                ...
                conn->allocptr.rangeline = aprintf(
                    "Content-Range: bytes %s%" CURL_FORMAT_CURL_OFF_T "/%" CURL_FORMAT_CURL_OFF_T
                    "\r\n",
                    data->state.range,
                ...
                /* Range was selected and then we just pass the incoming range and
                   append total size */
                conn->allocptr.rangeline = aprintf(
                    "Content-Range: bytes %s/%" CURL_FORMAT_CURL_OFF_T "\r\n",
                    data->state.range,
                    data->state.infilesize);
            }
    ...
    if (conn->bits.httpproxy && !conn->bits.tunnel_proxy) {
        ...
        result = Curl_add_bufferf(
            &req_buffer, "ftp://%s:%s@%s", conn->user, conn->passwd, path + sizeof("ftp://") - 1);
    else {
    ...
    result = Curl_add_bufferf(
        ...
        (data->state.use_range && conn->allocptr.rangeline) ? conn->allocptr.rangeline : "",
    ...
    if (data->cookies || addcookies) {
        ...
        if (data->cookies && data->state.cookie_engine) {
            ...
            co = Curl_cookie_getlist(
                data->cookies,
                conn->allocptr.cookiehost ? conn->allocptr.cookiehost : host,
                data->state.up.path,
        ...
        if (co) {
            ...
            while (co) {
                if (co->value) {
                    ...
                    result = Curl_add_bufferf(
                        &req_buffer, "%s%s=%s", count ? "; " : "", co->name, co->value);
                    if (result)
                        break;
                    count++;
                }
    ...
    switch (httpreq) {
        case HTTPREQ_PUT: /* Let's PUT the data to the server! */
            ...
            result =
                Curl_add_buffer_send(&req_buffer, conn, &data->info.request_size, 0, FIRSTSOCKET);
            if (result)
                failf(data, "Failed sending PUT request");
            else
                /* prepare for transfer */
                Curl_setup_transfer(data, FIRSTSOCKET, -1, TRUE, postsize ? FIRSTSOCKET : -1);
            if (result)
                return result;
            break;
        ...
        case HTTPREQ_POST_MIME:
            ...
            if (conn->bits.authneg) {
                ...
                result = Curl_add_buffer_send(
                    &req_buffer, conn, &data->info.request_size, 0, FIRSTSOCKET);
                if (result)
        ...
        case HTTPREQ_POST:
            ...
            if (data->set.postfields) {
                /* In HTTP2, we send request body in DATA frame regardless of
                   its size. */
                if (conn->httpversion != 20 && !data->state.expect100header &&
                    ...
                    if (!data->req.upload_chunky) {
                        ...
                        result =
                            Curl_add_buffer(&req_buffer, data->set.postfields, (size_t)postsize);
                        ...
                        if (postsize) {
                            ...
                            if (!result) {
                                result = Curl_add_buffer(
                                    &req_buffer, data->set.postfields, (size_t)postsize);
                                if (!result)
                                    result = Curl_add_buffer(&req_buffer, "\r\n", 2);
            ...
            /* issue the request */
            result = Curl_add_buffer_send(
                &req_buffer, conn, &data->info.request_size, (size_t)included_body, FIRSTSOCKET);

            if (result)
                failf(data, "Failed sending HTTP POST request");
            else
                Curl_setup_transfer(data, FIRSTSOCKET, -1, TRUE, http->postdata ? FIRSTSOCKET : -1);
            break;
    ...
    if (data->req.writebytecount) {
        ...
        if (data->req.writebytecount >= postsize) {
            ...
            infof(
                data,
                "upload completely sent off: %" CURL_FORMAT_CURL_OFF_T
...
static statusline
checkprotoprefix(struct Curl_easy* data, struct connectdata* conn, const char* s, size_t len)
{
#ifndef CURL_DISABLE_RTSP
...
static CURLcode header_append(struct Curl_easy* data, struct SingleRequest* k, size_t length)
{
    ...
    if (newsize > CURL_MAX_HTTP_HEADER) {
        /* The reason to have a max limit for this is to avoid the risk of a bad
           ...
           reallocs infinitely */
        failf(data, "Rejected %zu bytes header (max is %d)!", newsize, CURL_MAX_HTTP_HEADER);
        return CURLE_OUT_OF_MEMORY;
    }
...
CURLcode Curl_http_readwrite_headers(
    struct Curl_easy* data, struct connectdata* conn, ssize_t* nread, bool* stop_reading)
{
    CURLcode result;
    struct SingleRequest* k = &data->req;
    ...
    do {
        ...
        if (!k->end_ptr) {
            ...
            if (!k->headerline) {
                ...
                statusline st = checkprotoprefix(data, conn, data->state.headerbuff, k->hbuflen);
                if (st == STATUS_BAD) {
        ...
        if ((0x0a == *k->p) || (0x0d == *k->p)) {
            ...
            if (100 <= k->httpcode && 199 >= k->httpcode) {
                ...
                if ((k->size == -1) && !k->chunk && !conn->bits.close &&
                    (conn->httpversion == 11) && !(conn->handler->protocol & CURLPROTO_RTSP) &&
...
#if defined(USE_NTLM)
            if (conn->bits.close &&
                (((data->req.httpcode == 401) && (conn->http_ntlm_state == NTLMSTATE_TYPE2)) ||
...
#if defined(USE_SPNEGO)
            if (conn->bits.close &&
                (((data->req.httpcode == 401) && (conn->http_negotiate_state == GSS_AUTHRECV)) ||
            ...
            if (!k->header) {
                ...
                if (data->set.verbose)
                    Curl_debug(data, CURLINFO_HEADER_IN, k->str_start, headerlen);
...
#define HEADER1 k->p /* no conversion needed, just use k->p */
            ...
            if (nc) {
                ...
                if (data->state.resume_from && data->set.httpreq == HTTPREQ_GET &&
                    ...
                    data->set.http_fail_on_error && (k->httpcode >= 400) &&
        ...
        if (!k->http_bodyless && !data->set.ignorecl && checkprefix("Content-Length:", k->p)) {
        ...
        /* check for Content-Type: header lines to get the MIME-type */
            ...
            (conn->httpversion == 10) && conn->bits.httpproxy &&
            Curl_compareheader(k->p, "Proxy-Connection:", "keep-alive")) {
            /*
            ...
            (conn->httpversion == 11) && conn->bits.httpproxy &&
            Curl_compareheader(k->p, "Proxy-Connection:", "close")) {
            /*
            ...
            (conn->httpversion == 10) && Curl_compareheader(k->p, "Connection:", "keep-alive")) {
            /*
            ...
            !k->http_bodyless && checkprefix("Content-Encoding:", k->p) &&
            data->set.str[STRING_ENCODING]) {
            /*
            ...
            */
...
#if !defined(CURL_DISABLE_COOKIES)
        else if (data->cookies && data->state.cookie_engine && checkprefix("Set-Cookie:", k->p)) {
            Curl_share_lock(data, CURL_LOCK_DATA_COOKIE, CURL_LOCK_ACCESS_SINGLE);
            Curl_cookie_add(
                data,
                data->cookies,
            ...
            !k->http_bodyless && checkprefix("Last-Modified:", k->p) &&
            (data->set.timecondition || data->set.get_filetime)) {
            k->timeofdoc = Curl_getdate_capped(k->p + strlen("Last-Modified:"));
            if (data->set.get_filetime)
                data->info.filetime = k->timeofdoc;
        } else if (
            (checkprefix("WWW-Authenticate:", k->p) && (401 == k->httpcode)) ||
            (checkprefix("Proxy-authenticate:", k->p) && (407 == k->httpcode))) {
            ...
            (k->httpcode >= 300 && k->httpcode < 400) && checkprefix("Location:", k->p) &&
            !data->req.location) {
        ...
        /* If enabled, the header is incoming and this is over HTTPS */
            ...
            data->asi && checkprefix("Alt-Svc:", k->p) &&
