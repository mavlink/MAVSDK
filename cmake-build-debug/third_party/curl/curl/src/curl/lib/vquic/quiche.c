...
static CURLcode
process_ingress(struct connectdata* conn, curl_socket_t sockfd, struct quicsocket* qs);

static CURLcode flush_egress(struct connectdata* conn, curl_socket_t sockfd, struct quicsocket* qs);
...
static int quiche_perform_getsock(const struct connectdata* conn, curl_socket_t* socks)
{
    return quiche_getsock((struct connectdata*)conn, socks);
}

static CURLcode quiche_disconnect(struct connectdata* conn, bool dead_connection)
{
...
static unsigned int quiche_conncheck(struct connectdata* conn, unsigned int checks_to_perform)
{
    (void)conn;
    ...
    socklen_t addrlen)
{
    ...
    qs->conn = quiche_connect(conn->host.name, (const uint8_t*)qs->scid, sizeof(qs->scid), qs->cfg);
    if (!qs->conn) {
    ...
    if (!Curl_addr2string((struct sockaddr*)addr, addrlen, conn->primary_ip, &conn->primary_port)) {
...
static CURLcode quiche_has_connected(struct connectdata* conn, int sockindex, int tempindex)
{
    CURLcode result;
...
static CURLcode process_ingress(struct connectdata* conn, int sockfd, struct quicsocket* qs)
{
    ssize_t recvd;
    struct Curl_easy* data = conn->data;
    ...
    do {
        ...
        if (recvd < 0) {
            failf(
                conn->data,
                "quiche: recv() unexpectedly returned %d "
                "(errno: %d, socket %d)",
                recvd,
                SOCKERRNO,
                sockfd);
 ...
 */
static CURLcode flush_egress(struct connectdata* conn, int sockfd, struct quicsocket* qs)
{
    ssize_t sent;
...
static int
cb_each_header(uint8_t* name, size_t name_len, uint8_t* value, size_t value_len, void* argp)
{
    struct h3h1header* headers = (struct h3h1header*)argp;
    size_t olen = 0;

    if ((name_len == 7) && !strncmp(":status", (char*)name, 7)) {
        msnprintf(headers->dest, headers->destlen, "HTTP/3 %.*s\n", (int)value_len, value);
        ...
        switch (quiche_h3_event_type(ev)) {
            ...
            case QUICHE_H3_EVENT_DATA:
                if (!stream->firstbody) {
                    ...
                    recvd = 0;
                rcode = quiche_h3_recv_body(qs->h3c, qs->conn, s, (unsigned char*)buf, buffersize);
                if (rcode <= 0) {
                    recvd = -1;
                    break;
                }
    ...
    struct connectdata* conn, int sockindex, const void* mem, size_t len, CURLcode* curlcode)
{
    ...
    if (!stream->h3req) {
        ...
        sent =
            quiche_h3_send_body(qs->h3c, qs->conn, stream->stream3_id, (uint8_t*)mem, len, FALSE);
        if (sent < 0) {
            *curlcode = CURLE_SEND_ERROR;
            return -1;
        }
...
#define MAX_ACC 60000 /* <64KB to account for some overhead */
    ...
    switch (data->set.httpreq) {
        ...
        case HTTPREQ_PUT:
            ...
            stream3_id = quiche_h3_send_request(
                qs->h3c, qs->conn, nva, nheader, stream->upload_left ? FALSE : TRUE);
            ...
            break;
        default:
            stream3_id = quiche_h3_send_request(qs->h3c, qs->conn, nva, nheader, TRUE);
            break;
    }
...
CURLcode Curl_quic_done_sending(struct connectdata* conn)
{
    if (conn->handler == &Curl_handler_http3) {
        ...
        sent = quiche_h3_send_body(qs->h3c, qs->conn, stream->stream3_id, NULL, 0, TRUE);
        if (sent < 0)
            return CURLE_SEND_ERROR;
    }
