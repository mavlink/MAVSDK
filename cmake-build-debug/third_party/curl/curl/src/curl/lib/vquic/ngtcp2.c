...
static CURLcode
ng_process_ingress(struct connectdata* conn, curl_socket_t sockfd, struct quicsocket* qs);
static CURLcode ng_flush_egress(struct connectdata* conn, int sockfd, struct quicsocket* qs);
static int cb_h3_acked_stream_data(
    nghttp3_conn* conn, int64_t stream_id, size_t datalen, void* user_data, void* stream_user_data);
...
#ifdef USE_OPENSSL
static ngtcp2_crypto_level quic_from_ossl_level(OSSL_ENCRYPTION_LEVEL ossl_level)
{
    switch (ossl_level) {
...
#elif defined(USE_GNUTLS)
static ngtcp2_crypto_level quic_from_gtls_level(gnutls_record_encryption_level_t gtls_level)
{
    switch (gtls_level) {
...
static int setup_initial_crypto_context(struct quicsocket* qs)
{
    ...
    if (ngtcp2_crypto_derive_and_install_initial_key(
            qs->qconn, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, NULL, dcid) != 0)
        return -1;
...
#elif defined(USE_GNUTLS)
static int
keylog_callback(gnutls_session_t session, const char* label, const gnutls_datum_t* secret)
{
...
static int write_client_handshake(
    struct quicsocket* qs, ngtcp2_crypto_level level, const uint8_t* data, size_t len)
{
    ...
    rv = ngtcp2_conn_submit_crypto_data(
        qs->qconn, level, (uint8_t*)(&crypto_data->buf[crypto_data->len] - len), len);
    if (rv) {
...
#ifdef USE_OPENSSL
static int quic_set_encryption_secrets(
    SSL* ssl,
...
static int quic_send_alert(SSL* ssl, enum ssl_encryption_level_t level, uint8_t alert)
{
    struct quicsocket* qs = (struct quicsocket*)SSL_get_app_data(ssl);
    (void)level;
...
#elif defined(USE_GNUTLS)
static int secret_func(
    gnutls_session_t ssl,
    gnutls_record_encryption_level_t gtls_level,
    const void* rx_secret,
    const void* tx_secret,
    size_t secretlen)
{
...
static int tp_recv_func(gnutls_session_t ssl, const uint8_t* data, size_t data_size)
{
    struct quicsocket* qs = gnutls_session_get_ptr(ssl);
    ngtcp2_transport_params params;

    if (ngtcp2_decode_transport_params(
            &params, NGTCP2_TRANSPORT_PARAMS_TYPE_ENCRYPTED_EXTENSIONS, data, data_size) != 0)
        return -1;
...
static int tp_send_func(gnutls_session_t ssl, gnutls_buffer_t extdata)
{
    ...
    nwrite = ngtcp2_encode_transport_params(
        paramsbuf, sizeof(paramsbuf), NGTCP2_TRANSPORT_PARAMS_TYPE_CLIENT_HELLO, &params);
    if (nwrite < 0) {
...
static int quic_init_ssl(struct quicsocket* qs)
{
    ...
    rc = gnutls_session_ext_register(
        qs->ssl,
    ...
    if (rc < 0) {
        fprintf(
            stderr, "gnutls_certificate_allocate_credentials failed: %s\n", gnutls_strerror(rc));
        return 1;
    }
    ...
    if (rc < 0) {
        fprintf(
            stderr, "gnutls_certificate_set_x509_system_trust failed: %s\n", gnutls_strerror(rc));
        return 1;
    }
...
static int cb_initial(ngtcp2_conn* quic, void* user_data)
{
    ...
    if (ngtcp2_crypto_read_write_crypto_data(quic, qs->ssl, NGTCP2_CRYPTO_LEVEL_INITIAL, NULL, 0) !=
        0)
...
static int cb_recv_crypto_data(
    ngtcp2_conn* tconn,
    ...
    void* user_data)
{
    ...
    if (ngtcp2_crypto_read_write_crypto_data(tconn, qs->ssl, crypto_level, data, datalen) != 0)
...
static void extend_stream_window(ngtcp2_conn* tconn, struct HTTP* stream)
{
...
static int cb_recv_stream_data(
    ngtcp2_conn* tconn,
    int64_t stream_id,
    int fin,
    uint64_t offset,
    ...
    void* stream_user_data)
{
    ...
    nconsumed = nghttp3_conn_read_stream(qs->h3conn, stream_id, buf, buflen, fin);
    if (nconsumed < 0) {
        failf(
            qs->conn->data,
            "nghttp3_conn_read_stream returned error: %s\n",
...
static int cb_acked_stream_data_offset(
    ngtcp2_conn* tconn,
    int64_t stream_id,
    uint64_t offset,
    size_t datalen,
    ...
    void* stream_user_data)
{
    ...
    if (rv != 0) {
        failf(
            qs->conn->data,
            "nghttp3_conn_add_ack_offset returned error: %s\n",
            nghttp3_strerror(rv));
        return NGTCP2_ERR_CALLBACK_FAILURE;
    }
    ...
    void* stream_user_data)
{
    ...
    rv = nghttp3_conn_close_stream(qs->h3conn, stream_id, app_error_code);
    if (rv != 0) {
        failf(
            qs->conn->data, "nghttp3_conn_close_stream returned error: %s\n", nghttp3_strerror(rv));
        return NGTCP2_ERR_CALLBACK_FAILURE;
    }
    ...
    void* stream_user_data)
{
    ...
    if (rv != 0) {
        failf(
            qs->conn->data, "nghttp3_conn_reset_stream returned error: %s\n", nghttp3_strerror(rv));
        return NGTCP2_ERR_CALLBACK_FAILURE;
    }
...
static int cb_recv_retry(
    ngtcp2_conn* tconn, const ngtcp2_pkt_hd* hd, const ngtcp2_pkt_retry* retry, void* user_data)
{
...
static int cb_extend_max_stream_data(
    ngtcp2_conn* tconn,
    int64_t stream_id,
    uint64_t max_data,
    void* user_data,
    void* stream_user_data)
{
    ...
    if (rv != 0) {
        failf(
            qs->conn->data,
            "nghttp3_conn_unblock_stream returned error: %s\n",
            nghttp3_strerror(rv));
        return NGTCP2_ERR_CALLBACK_FAILURE;
    }
...
static int cb_get_new_connection_id(
    ngtcp2_conn* tconn, ngtcp2_cid* cid, uint8_t* token, size_t cidlen, void* user_data)
{
    struct quicsocket* qs = (struct quicsocket*)user_data;
    CURLcode result;
    (void)tconn;
    ...
    socklen_t addrlen)
{
    ...
    rv = getsockname(sockfd, (struct sockaddr*)&qs->local_addr, &qs->local_addrlen);
    if (rv == -1)
        return CURLE_QUIC_CONNECT_ERROR;

    ngtcp2_addr_init(&path.local, (uint8_t*)&qs->local_addr, qs->local_addrlen, NULL);
...
#endif
    rc = ngtcp2_conn_client_new(
        &qs->qconn, &qs->dcid, &qs->scid, &path, QUICVER, &ng_callbacks, &qs->settings, NULL, qs);
    if (rc)
        return CURLE_QUIC_CONNECT_ERROR;
...
int Curl_quic_ver(char* p, size_t len)
{
    ...
    return msnprintf(p, len, "ngtcp2/%s nghttp3/%s", ng2->version_str, ht3->version_str);
}
...
static int ng_perform_getsock(const struct connectdata* conn, curl_socket_t* socks)
{
    return ng_getsock((struct connectdata*)conn, socks);
}

static CURLcode ng_disconnect(struct connectdata* conn, bool dead_connection)
{
    int i;
...
static unsigned int ng_conncheck(struct connectdata* conn, unsigned int checks_to_perform)
{
    (void)conn;
 ...
 */
static CURLcode allocate_overflow(struct Curl_easy* data, struct HTTP* stream, size_t length)
{
    size_t maxleft;
    size_t newsize;
    ...
    if (length > maxleft) {
        ...
        failf(
            data,
            "Rejected %zu bytes of overflow data (max is %d)!",
            stream->overflow_buflen + length,
            CURL_MAX_READ_SIZE);
 ...
 */
static CURLcode
write_data(struct Curl_easy* data, struct HTTP* stream, const void* mem, size_t memlen)
{
...
static int cb_h3_deferred_consume(
    nghttp3_conn* conn, int64_t stream_id, size_t consumed, void* user_data, void* stream_user_data)
{
...
static int
cb_h3_end_headers(nghttp3_conn* conn, int64_t stream_id, void* user_data, void* stream_user_data)
{
    struct Curl_easy* data = stream_user_data;
    struct HTTP* stream = data->req.protop;
    CURLcode result = CURLE_OK;
    ...
    void* stream_user_data)
{
    ...
    if (h3name.len == sizeof(":status") - 1 && !memcmp(":status", h3name.base, h3name.len)) {
...
static int cb_h3_send_stop_sending(
    nghttp3_conn* conn,
    int64_t stream_id,
    uint64_t app_error_code,
    void* user_data,
...
static int init_ngh3_conn(struct quicsocket* qs)
{
    ...
    rc = nghttp3_conn_client_new(
        &qs->h3conn, &ngh3_callbacks, &qs->h3settings, nghttp3_mem_default(), qs);
    if (rc) {
...
static size_t drain_overflow_buffer(struct HTTP* stream)
{
    ...
    if (ncopy > 0) {
        ...
        stream->memlen += ncopy;
        stream->overflow_buflen -= ncopy;
        memmove(stream->overflow_buf, stream->overflow_buf + ncopy, stream->overflow_buflen);
    }
    return ncopy;
}

/* incoming data frames on the h3 stream */
static ssize_t ngh3_stream_recv(
    struct connectdata* conn, int sockindex, char* buf, size_t buffersize, CURLcode* curlcode)
{
...
static ssize_t cb_h3_readfunction(
    nghttp3_conn* conn,
    int64_t stream_id,
    nghttp3_vec* vec,
    size_t veccnt,
    ...
    void* stream_user_data)
{
    ...
    if (nread > 0) {
        ...
        H3BUGF(infof(
            data,
            "cb_h3_readfunction %zd bytes%s (at %zd unacked)\n",
            nread,
            *pflags == NGHTTP3_DATA_FLAG_EOF ? " EOF" : "",
            out->used));
    }
    if (stream->upload_done && !stream->upload_len && (stream->upload_left <= 0)) {
...
static CURLcode http_request(struct connectdata* conn, const void* mem, size_t len)
{
    struct HTTP* stream = conn->data->req.protop;
    size_t nheader;
    size_t i;
...
#define MAX_ACC 60000 /* <64KB to account for some overhead */
    {
        ...
        if (acc > MAX_ACC) {
            infof(
                data,
                "http_request: Warning: The cumulative length of all "
    ...
    switch (data->set.httpreq) {
        ...
        case HTTPREQ_PUT: {
            ...
            rc = nghttp3_conn_submit_request(
                qs->h3conn, stream->stream3_id, nva, nheader, &data_reader, conn->data);
            if (rc) {
                result = CURLE_SEND_ERROR;
                goto fail;
            }
...
static ssize_t ngh3_stream_send(
    struct connectdata* conn, int sockindex, const void* mem, size_t len, CURLcode* curlcode)
{
    ssize_t sent;
    struct quicsocket* qs = conn->quic;
    curl_socket_t sockfd = conn->sock[sockindex];
 ...
 */
CURLcode Curl_quic_is_connected(struct connectdata* conn, int sockindex, bool* done)
{
    CURLcode result;
    struct quicsocket* qs = &conn->hequic[sockindex];
...
static CURLcode ng_process_ingress(struct connectdata* conn, int sockfd, struct quicsocket* qs)
{
    ssize_t recvd;
    int rv;
    uint8_t buf[65536];
    ...
    for (;;) {
        ...
        while ((recvd = recvfrom(
                    sockfd, buf, bufsize, 0, (struct sockaddr*)&remote_addr, &remote_addrlen)) ==
                   -1 &&
               SOCKERRNO == EINTR)
            ;
        if (recvd == -1) {
        ...
        ngtcp2_addr_init(&path.local, (uint8_t*)&qs->local_addr, qs->local_addrlen, NULL);
        ngtcp2_addr_init(&path.remote, (uint8_t*)&remote_addr, remote_addrlen, NULL);
    ...
    for (;;) {
        ...
        if (qs->h3conn && ngtcp2_conn_get_max_data_left(qs->qconn)) {
            veccnt = nghttp3_conn_writev_stream(
                qs->h3conn, &stream_id, &fin, vec, sizeof(vec) / sizeof(vec[0]));
            if (veccnt < 0) {
                failf(
                    conn->data,
                    "nghttp3_conn_writev_stream returned error: %s\n",
                ...
                if (outlen < 0) {
                    if (outlen == NGTCP2_ERR_STREAM_DATA_BLOCKED ||
                        ...
                        failf(
                            conn->data,
                            "ngtcp2_conn_writev_stream returned error: %s\n",
        ...
        if (outlen < 0) {
            ...
            if (outlen < 0) {
                failf(
                    conn->data,
                    "ngtcp2_conn_write_pkt returned error: %s\n",
                    ngtcp2_strerror((int)outlen));
                return CURLE_SEND_ERROR;
            }
            if (outlen == 0)
        ...
        while ((sent = send(sockfd, out, outlen, 0)) == -1 && SOCKERRNO == EINTR)
            ;
