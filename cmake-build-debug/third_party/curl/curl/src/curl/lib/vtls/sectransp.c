...
#if ( \
    (TARGET_OS_IPHONE && __IPHONE_OS_VERSION_MIN_REQUIRED >= 100000) || \
...
static const unsigned char rsa4096SpkiHeader[] = {0x30, 0x82, 0x02, 0x22, 0x30, 0x0d, 0x06, 0x09,
                                                  0x2a, 0x86, 0x48, 0x86, 0xf7, 0x0d, 0x01, 0x01,
                                                  0x01, 0x05, 0x00, 0x03, 0x82, 0x02, 0x0f, 0x00};

static const unsigned char rsa2048SpkiHeader[] = {0x30, 0x82, 0x01, 0x22, 0x30, 0x0d, 0x06, 0x09,
                                                  0x2a, 0x86, 0x48, 0x86, 0xf7, 0x0d, 0x01, 0x01,
...
static OSStatus
SocketWrite(SSLConnectionRef connection, const void* data, size_t* dataLength) /* IN/OUT */
{
    size_t bytesSent = 0;
    /*int sock = *(int *)connection;*/
    struct ssl_connect_data* connssl = (struct ssl_connect_data*)connection;
...
static OSStatus CopyIdentityWithLabelOldSchool(char* label, SecIdentityRef* out_c_a_k)
{
    ...
    /* Start searching: */
    status =
        SecKeychainSearchCreateFromAttributes(NULL, kSecCertificateItemClass, &attr_list, &search);
    if (status == noErr) {
...
static OSStatus CopyIdentityFromPKCS12File(
    const char* cPath, const char* cPassword, SecIdentityRef* out_cert_and_key)
{
    OSStatus status = errSecItemNotFound;
    CFURLRef pkcs_url =
        CFURLCreateFromFileSystemRepresentation(NULL, (const UInt8*)cPath, strlen(cPath), false);
    CFStringRef password =
        cPassword ? CFStringCreateWithCString(NULL, cPassword, kCFStringEncodingUTF8) : NULL;
    ...
    if (CFURLCreateDataAndPropertiesFromResource(NULL, pkcs_url, &pkcs_data, NULL, NULL, &status)) {
        ...
        CFDictionaryRef options =
            CFDictionaryCreate(NULL, cKeys, cValues, password ? 1L : 0L, NULL, NULL);
        ...
        status =
            SecItemImport(pkcs_data, NULL, &inputFormat, &inputType, 0, &keyParams, NULL, &items);
#endif
                ...
                if (itemID == CFDictionaryGetTypeID()) {
                    ...
                    *out_cert_and_key = (SecIdentityRef)identity;
                    ...
                    status = SecIdentityCreateWithCertificate(
                        NULL, (SecCertificateRef)item, out_cert_and_key);
                    break;
                }
#endif
            }
...
static CURLcode sectransp_connect_step1(struct connectdata* conn, int sockindex)
{
    struct Curl_easy* data = conn->data;
    ...
    if (conn->ssl_config.version_max != CURL_SSLVERSION_MAX_NONE) {
        failf(
            data,
            "Your version of the OS does not support to set maximum"
            " SSL/TLS version");
        return CURLE_SSL_CONNECT_ERROR;
    }
    (void)SSLSetProtocolVersionEnabled(backend->ssl_ctx, kSSLProtocolAll, false);
    ...
    if (ssl_cert) {
        ...
        if (is_cert_file) {
            if (!SSL_SET_OPTION(cert_type))
                ...
                strncmp(SSL_SET_OPTION(cert_type), "P12", strlen(SSL_SET_OPTION(cert_type))) != 0)
                infof(
                    data,
    ...
    */
    ...
    if (SSLSetSessionOption != NULL) {
        ...
        err = SSLSetSessionOption(
            backend->ssl_ctx, kSSLSessionOptionBreakOnServerAuth, break_on_auth);
        if (err != noErr) {
    ...
    for (i = 0UL; i < all_ciphers_count; i++) {
        ...
        if (darwinver_maj == 12 && darwinver_min <= 3 && all_ciphers[i] >= 0xC001 &&
            all_ciphers[i] <= 0xC032) {
            continue;
        }
#endif /* CURL_BUILD_MAC */
    ...
    if (SSLSetSessionOption != NULL) {
        SSLSetSessionOption(
            backend->ssl_ctx, kSSLSessionOptionSendOneByteRecord, !data->set.ssl.enable_beast);
        SSLSetSessionOption(
            backend->ssl_ctx,
...
static int append_cert_to_array(
    struct Curl_easy* data, unsigned char* buf, size_t buflen, CFMutableArrayRef array)
{
...
#ifdef SECTRANSP_PINNEDPUBKEY
static CURLcode
pkp_pin_peer_pubkey(struct Curl_easy* data, SSLContextRef ctx, const char* pinnedpubkey)
{ /* Scratch */
...
static CURLcode sectransp_connect_step2(struct connectdata* conn, int sockindex)
{
    struct Curl_easy* data = conn->data;
    struct ssl_connect_data* connssl = &conn->ssl[sockindex];
    struct ssl_backend_data* backend = connssl->backend;
    ...
    if (err != noErr) {
        switch (err) {
            case errSSLWouldBlock: /* they're not done with us yet */
                connssl->connecting_state =
                    backend->ssl_direction ? ssl_connect_2_writing : ssl_connect_2_reading;
                return CURLE_OK;
            ...
            case errSSLPeerBadRecordMac:
                failf(
                    data,
                    "A record with a bad message authentication code (MAC) "
                    "was encountered");
                break;
            ...
            case errSSLCertNotYetValid:
                failf(
                    data,
                    "SSL certificate problem: The certificate chain had a "
            ...
            case errSSLPeerUnsupportedCert:
                failf(
                    data,
                    "SSL certificate problem: An unsupported certificate "
            ...
            case errSSLClientHelloReceived:
                failf(
                    data,
                    "A non-fatal result for providing a server name "
                    "indication");
                break;
#endif
        ...
        if (data->set.str[STRING_SSL_PINNEDPUBLICKEY_ORIG]) {
            CURLcode result = pkp_pin_peer_pubkey(
                data, backend->ssl_ctx, data->set.str[STRING_SSL_PINNEDPUBLICKEY_ORIG]);
            if (result) {
        ...
        if (conn->bits.tls_enable_alpn) {
            if (__builtin_available(macOS 10.13.4, iOS 11, tvOS 11, *)) {
...
#ifdef USE_NGHTTP2
                if (chosenProtocol &&
                    !CFStringCompare(chosenProtocol, CFSTR(NGHTTP2_PROTO_VERSION_ID), 0)) {
                ...
                Curl_multiuse_state(
                    conn,
                    conn->negnpn == CURL_HTTP_VERSION_2 ? BUNDLE_MULTIPLEX : BUNDLE_NO_MULTIUSE);
...
static CURLcode sectransp_connect_step3(struct connectdata* conn, int sockindex)
{
    struct Curl_easy* data = conn->data;
    struct ssl_connect_data* connssl = &conn->ssl[sockindex];
...
static CURLcode
sectransp_connect_common(struct connectdata* conn, int sockindex, bool nonblocking, bool* done)
{
    CURLcode result;
    struct Curl_easy* data = conn->data;
    struct ssl_connect_data* connssl = &conn->ssl[sockindex];
...
static CURLcode
Curl_sectransp_connect_nonblocking(struct connectdata* conn, int sockindex, bool* done)
{
...
static bool Curl_sectransp_data_pending(const struct connectdata* conn, int connindex)
{
...
static CURLcode
Curl_sectransp_random(struct Curl_easy* data UNUSED_PARAM, unsigned char* entropy, size_t length)
{
...
static CURLcode Curl_sectransp_md5sum(
...
static CURLcode Curl_sectransp_sha256sum(
...
static ssize_t
sectransp_recv(struct connectdata* conn, int num, char* buf, size_t buffersize, CURLcode* curlcode)
{
    /*struct Curl_easy *data = conn->data;*/
    ...
    if (err != noErr) {
        switch (err) {
            ...
            case -9841:
                if (SSL_CONN_CONFIG(CAfile) && SSL_CONN_CONFIG(verifypeer)) {
                    CURLcode result =
                        verify_cert(SSL_CONN_CONFIG(CAfile), conn->data, backend->ssl_ctx);
                    if (result)
                        return result;
                }
                goto again;
...
static void*
Curl_sectransp_get_internals(struct ssl_connect_data* connssl, CURLINFO info UNUSED_PARAM)
{
    struct ssl_backend_data* backend = connssl->backend;
    (void)info;
