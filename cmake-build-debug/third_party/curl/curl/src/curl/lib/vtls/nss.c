...
#define PK11_SETATTRS(_attr, _idx, _type, _val, _len) \
    do { \
...
static const cipher_s cipherlist[] = {
    ...
    {"ecdhe_rsa_chacha20_poly1305_sha_256", TLS_ECDHE_RSA_WITH_CHACHA20_POLY1305_SHA256},
    {"ecdhe_ecdsa_chacha20_poly1305_sha_256", TLS_ECDHE_ECDSA_WITH_CHACHA20_POLY1305_SHA256},
    {"dhe_rsa_chacha20_poly1305_sha_256", TLS_DHE_RSA_WITH_CHACHA20_POLY1305_SHA256},
#endif
...
static char* dup_nickname(struct Curl_easy* data, const char* str)
{
    ...
    if (!n) {
        infof(
            data,
            "warning: certificate file name \"%s\" handled as nickname; "
...
static CURLcode nss_create_object(
    struct ssl_connect_data* connssl, CK_OBJECT_CLASS obj_class, const char* filename, bool cacert)
{
    PK11SlotInfo* slot;
    ...
    int attr_cnt = 0;
    CURLcode result = (cacert) ? CURLE_SSL_CACERT_BADFILE : CURLE_SSL_CERTPROBLEM;
    ...
    PK11_SETATTRS(
        attrs, attr_cnt, CKA_LABEL, (unsigned char*)filename, (CK_ULONG)strlen(filename) + 1);
...
static SECStatus nss_auth_cert_hook(void* arg, PRFileDesc* fd, PRBool checksig, PRBool isServer)
{
    ...
    if (SSL_CONN_CONFIG(verifystatus)) {
        ...
        cacheResult = CERT_CacheOCSPResponseFromSideChannel(
            CERT_GetDefaultCertDB(), SSL_PeerCertificate(fd), PR_Now(), &csa->items[0], arg);
...
static SECStatus CanFalseStartCallback(PRFileDesc* sock, void* client_data, PRBool* canFalseStart)
{
    ...
    *canFalseStart = PR_FALSE;
    ...
    if (SSL_GetCipherSuiteInfo(channelInfo.cipherSuite, &cipherInfo, sizeof(cipherInfo)) !=
        SECSuccess)
        return SECFailure;
...
static CURLcode display_conn_info(struct connectdata* conn, PRFileDesc* sock)
{
    ...
    if (SSL_GetChannelInfo(sock, &channel, sizeof(channel)) == SECSuccess &&
 ...
 */
static SECStatus SelectClientCert(
    void* arg,
    PRFileDesc* sock,
    ...
    struct SECKEYPrivateKeyStr** pRetKey)
{
    ...
    if (backend->obj_clicert) {
        ...
        if (PK11_ReadRawAttribute(PK11_TypeGeneric, backend->obj_clicert, CKA_VALUE, &cert_der) !=
            SECSuccess) {
    ...
    if (SECSuccess != NSS_GetClientAuthData((void*)nickname, sock, caNames, pRetCert, pRetKey) ||
        ...
        if (NULL == nickname)
            failf(
                data,
                "NSS: client certificate not found (nickname not "
                "specified)");
        else
...
static PRInt32
nspr_io_recv(PRFileDesc* fd, void* buf, PRInt32 amount, PRIntn flags, PRIntervalTime timeout)
{
...
static PRInt32
nspr_io_send(PRFileDesc* fd, const void* buf, PRInt32 amount, PRIntn flags, PRIntervalTime timeout)
{
...
static CURLcode nss_init_core(struct Curl_easy* data, const char* cert_dir)
{
    ...
    if (cert_dir) {
        ...
        nss_context = NSS_InitContext(
            certpath, "", "", "", &initparams, NSS_INIT_READONLY | NSS_INIT_PK11RELOAD);
        free(certpath);
    ...
    nss_context = NSS_InitContext(
        ...
        "",
        &initparams,
        NSS_INIT_READONLY | NSS_INIT_NOCERTDB | NSS_INIT_NOMODDB | NSS_INIT_FORCEOPEN |
...
static CURLcode nss_load_ca_certificates(struct connectdata* conn, int sockindex)
{
    ...
    if (use_trust_module && !trust_module) {
        ...
        infof(data, "%s %s\n", (result) ? "failed to load" : "loaded", trust_library);
...
static CURLcode
nss_init_sslver(SSLVersionRange* sslver, struct Curl_easy* data, struct connectdata* conn)
{
    CURLcode result;
...
static CURLcode
nss_fail_connect(struct ssl_connect_data* connssl, struct Curl_easy* data, CURLcode curlerr)
{
    PRErrorCode err = 0;
    struct ssl_backend_data* backend = connssl->backend;
...
static CURLcode
nss_set_blocking(struct ssl_connect_data* connssl, struct Curl_easy* data, bool blocking)
{
...
static CURLcode nss_setup_connect(struct connectdata* conn, int sockindex)
{
    ...
    if (result == CURLE_FAILED_INIT)
        infof(
            data,
            "WARNING: failed to load NSS PEM library %s. Using "
    ...
    ssl_no_cache =
        (SSL_SET_OPTION(primary.sessionid) && SSL_CONN_CONFIG(verifypeer)) ? PR_FALSE : PR_TRUE;
    ...
    if (sslver_supported.max < sslver.max && sslver_supported.max >= sslver.min) {
        ...
        if (sslver_req_str && sslver_supp_str)
            infof(
                data,
                "Falling back from %s to max supported SSL version (%s)\n",
                sslver_req_str,
                sslver_supp_str);
        free(sslver_req_str);
...
#ifdef SSL_ENABLE_OCSP_STAPLING
    if (SSL_CONN_CONFIG(verifystatus)) {
        if (SSL_OptionSet(backend->handle, SSL_ENABLE_OCSP_STAPLING, PR_TRUE) != SECSuccess)
            goto error;
    }
#endif
    ...
    if (SSL_OptionSet(
            backend->handle, SSL_ENABLE_NPN, conn->bits.tls_enable_npn ? PR_TRUE : PR_FALSE) !=
        SECSuccess)
        goto error;
#endif
...
#ifdef SSL_ENABLE_ALPN
    if (SSL_OptionSet(
            backend->handle, SSL_ENABLE_ALPN, conn->bits.tls_enable_alpn ? PR_TRUE : PR_FALSE) !=
        SECSuccess)
        goto error;
#endif
    ...
    if (data->set.ssl.falsestart) {
        if (SSL_OptionSet(backend->handle, SSL_ENABLE_FALSE_START, PR_TRUE) != SECSuccess)
            goto error;

        if (SSL_SetCanFalseStartCallback(backend->handle, CanFalseStartCallback, conn) !=
            SECSuccess)
            goto error;
    }
#endif
    ...
    if (SSL_ResetHandshake(backend->handle, /* asServer */ PR_FALSE) != SECSuccess)
        goto error;

    /* propagate hostname to the TLS layer */
    if (SSL_SetURL(
            backend->handle, SSL_IS_PROXY() ? conn->http_proxy.host.name : conn->host.name) !=
        SECSuccess)
        goto error;
...
static CURLcode Curl_nss_connect_nonblocking(struct connectdata* conn, int sockindex, bool* done)
{
    return nss_connect_common(conn, sockindex, done);
}

static ssize_t nss_send(
    ...
    CURLcode* curlcode)
{
    ...
    if (rc < 0) {
        ...
        if (err == PR_WOULD_BLOCK_ERROR)
            ...
            *curlcode = (is_cc_error(err)) ? CURLE_SSL_CERTPROBLEM : CURLE_SEND_ERROR;
        }
    ...
    CURLcode* curlcode)
{
    ...
    if (nread < 0) {
        ...
        if (err == PR_WOULD_BLOCK_ERROR)
            ...
            *curlcode = (is_cc_error(err)) ? CURLE_SSL_CERTPROBLEM : CURLE_RECV_ERROR;
        }
...
/* data might be NULL */
static CURLcode Curl_nss_random(struct Curl_easy* data, unsigned char* entropy, size_t length)
{
...
static CURLcode Curl_nss_md5sum(
...
static CURLcode Curl_nss_sha256sum(
...
static void* Curl_nss_get_internals(struct ssl_connect_data* connssl, CURLINFO info UNUSED_PARAM)
{
    struct ssl_backend_data* backend = connssl->backend;
    (void)info;
...
const struct Curl_ssl Curl_ssl_nss = {
    ...
    SSLSUPP_CA_PATH | SSLSUPP_CERTINFO | SSLSUPP_PINNEDPUBKEY | SSLSUPP_HTTPS_PROXY,
