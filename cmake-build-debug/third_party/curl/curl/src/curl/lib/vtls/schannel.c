...
static CURLcode
pkp_pin_peer_pubkey(struct connectdata* conn, int sockindex, const char* pinnedpubkey);

static void
InitSecBuffer(SecBuffer* buffer, unsigned long BufType, void* BufDataPtr, unsigned long BufByteSize)
{
...
/*ifdefs for the options that are defined conditionally in wincrypt.h*/
    ...
    defined(__MINGW64_VERSION_MAJOR) || (__W32API_MAJOR_VERSION > 5) || \
...
#ifdef HAS_CLIENT_CERT_PATH
static CURLcode
get_cert_location(TCHAR* path, DWORD* store_name, TCHAR** store_path, TCHAR** thumbprint)
{
    TCHAR* sep;
    ...
    else if (_tcsnccmp(path, TEXT("CurrentUserGroupPolicy"), store_name_len) == 0)
        *store_name = CERT_SYSTEM_STORE_CURRENT_USER_GROUP_POLICY;
    else if (_tcsnccmp(path, TEXT("LocalMachineGroupPolicy"), store_name_len) == 0)
        *store_name = CERT_SYSTEM_STORE_LOCAL_MACHINE_GROUP_POLICY;
    else if (_tcsnccmp(path, TEXT("LocalMachineEnterprise"), store_name_len) == 0)
...
static CURLcode schannel_connect_step1(struct connectdata* conn, int sockindex)
{
    ...
    TCHAR* host_name;
    CURLcode result;
    char* const hostname = SSL_IS_PROXY() ? conn->http_proxy.host.name : conn->host.name;
    ...
    if (Curl_verify_windows_version(5, 1, PLATFORM_WINNT, VERSION_LESS_THAN_EQUAL)) {
        ...
        infof(
            data,
            "schannel: Windows version is old and may not be able to "
    ...
    BACKEND->use_alpn =
        ...
        !GetProcAddress(GetModuleHandleA("ntdll"), "wine_get_version") &&
        Curl_verify_windows_version(6, 3, PLATFORM_WINNT, VERSION_GREATER_THAN_EQUAL);
#else
...
#error "compiler too old to support requisite manual cert verify for Win CE"
    ...
    if (SSL_CONN_CONFIG(CAfile)) {
        if (Curl_verify_windows_version(6, 1, PLATFORM_WINNT, VERSION_GREATER_THAN_EQUAL)) {
            BACKEND->use_manual_cred_validation = true;
        } else {
            failf(
                data,
                "schannel: this version of Windows is too old to support "
    ...
    if (!BACKEND->cred) {
        ...
        if (conn->ssl_config.verifypeer) {
            ...
            if (data->set.ssl.no_revoke) {
                ...
                DEBUGF(infof(
                    data,
                    "schannel: disabled server certificate revocation "
                    "checks\n"));
        ...
        if (!conn->ssl_config.verifyhost) {
            ...
            DEBUGF(infof(
                data,
                "schannel: verifyhost setting prevents Schannel from "
        ...
        if (data->set.ssl.cert) {
            ...
            result = get_cert_location(
                cert_path, &cert_store_name, &cert_store_path, &cert_thumbprint_str);
            ...
            if ((result != CURLE_OK) && (fInCert == NULL)) {
                failf(
                    data,
                    "schannel: Failed to get certificate location"
                    " or file for %s",
                    data->set.ssl.cert);
            ...
            if (fInCert) {
                ...
                */
                ...
                if ((certdata == NULL) || ((int)fread(certdata, (size_t)filesize, 1, fInCert) != 1))
                    continue_reading = 0;
                fclose(fInCert);
                Curl_unicodefree(cert_path);

                if (!continue_reading) {
                    failf(data, "schannel: Failed to read cert file %s", data->set.ssl.cert);
                    free(certdata);
                    return CURLE_SSL_CERTPROBLEM;
                }
                ...
                if (cert_store == NULL) {
                    ...
                    if (errorcode == ERROR_INVALID_PASSWORD)
                        failf(
                            data,
                            "schannel: Failed to import cert file %s, "
                            "password is bad",
                            data->set.ssl.cert);
                    else
                        failf(
                ...
                if (client_certs[0] == NULL) {
                    failf(
                        data,
                        "schannel: Failed to get certificate from file %s"
                        ", last error is 0x%x",
                        data->set.ssl.cert,
                        GetLastError());
                ...
                if (!cert_store) {
                    failf(
                        data,
                        "schannel: Failed to open cert store %x %s, "
                        "last error is 0x%x",
                        cert_store_name,
                        cert_store_path,
                        GetLastError());
         ...
         */
        sspi_status = s_pSecFn->AcquireCredentialsHandle(
            NULL,
            (TCHAR*)UNISP_NAME,
    ...
    /* setup request flags */
    BACKEND->req_flags = ISC_REQ_SEQUENCE_DETECT | ISC_REQ_REPLAY_DETECT | ISC_REQ_CONFIDENTIALITY |
    ...
    */
    ...
    result =
        Curl_write_plain(conn, conn->sock[sockindex], outbuf.pvBuffer, outbuf.cbBuffer, &written);
    s_pSecFn->FreeContextBuffer(outbuf.pvBuffer);
    if ((result != CURLE_OK) || (outbuf.cbBuffer != (size_t)written)) {
        failf(
            data,
            "schannel: failed to send initial handshake data: "
            "sent %zd of %lu bytes",
            written,
            outbuf.cbBuffer);
        return CURLE_SSL_CONNECT_ERROR;
...
static CURLcode schannel_connect_step2(struct connectdata* conn, int sockindex)
{
    ...
    if (BACKEND->encdata_length - BACKEND->encdata_offset < CURL_SCHANNEL_BUFFER_FREE_SIZE) {
        /* increase internal encrypted data buffer */
        size_t reallocated_length = BACKEND->encdata_offset + CURL_SCHANNEL_BUFFER_FREE_SIZE;
        reallocated_buffer = realloc(BACKEND->encdata_buffer, reallocated_length);
    ...
    for (;;) {
        ...
        memcpy(inbuf[0].pvBuffer, BACKEND->encdata_buffer, BACKEND->encdata_offset);
        ...
        if (sspi_status == SEC_I_CONTINUE_NEEDED || sspi_status == SEC_E_OK) {
            for (i = 0; i < 3; i++) {
                ...
                if (outbuf[i].BufferType == SECBUFFER_TOKEN && outbuf[i].cbBuffer > 0) {
                    DEBUGF(infof(
                        data,
                        "schannel: sending next handshake data: "
                        "sending %lu bytes...\n",
                        outbuf[i].cbBuffer));
                    ...
                    if ((result != CURLE_OK) || (outbuf[i].cbBuffer != (size_t)written)) {
                        failf(
                            data,
                            "schannel: failed to send next handshake data: "
                            "sent %zd of %lu bytes",
                            written,
                            outbuf[i].cbBuffer);
                        return CURLE_SSL_CONNECT_ERROR;
        ...
        if (inbuf[1].BufferType == SECBUFFER_EXTRA && inbuf[1].cbBuffer > 0) {
            ...
            */
            ...
            if (BACKEND->encdata_offset > inbuf[1].cbBuffer) {
                memmove(
                    BACKEND->encdata_buffer,
                    (BACKEND->encdata_buffer + BACKEND->encdata_offset) - inbuf[1].cbBuffer,
                    inbuf[1].cbBuffer);
...
static void traverse_cert_store(const CERT_CONTEXT* context, Read_crt_func func, void* arg)
{
    ...
    while (should_continue && (current_context = CertEnumCertificatesInStore(
                                   context->hCertStore, current_context)) != NULL)
...
static bool add_cert_to_certinfo(const CERT_CONTEXT* ccert_context, void* raw_arg)
{
...
static CURLcode schannel_connect_step3(struct connectdata* conn, int sockindex)
{
    ...
    if (BACKEND->use_alpn) {
        sspi_status = s_pSecFn->QueryContextAttributes(
            &BACKEND->ctxt->ctxt_handle, SECPKG_ATTR_APPLICATION_PROTOCOL, &alpn_result);
        ...
        if (alpn_result.ProtoNegoStatus == SecApplicationProtocolNegotiationStatus_Success) {
            ...
            if (alpn_result.ProtocolIdSize == NGHTTP2_PROTO_VERSION_ID_LEN &&
                ...
                if (alpn_result.ProtocolIdSize == ALPN_HTTP_1_1_LENGTH &&
                    !memcmp(ALPN_HTTP_1_1, alpn_result.ProtocolId, ALPN_HTTP_1_1_LENGTH)) {
                conn->negnpn = CURL_HTTP_VERSION_1_1;
            }
        } else
            infof(data, "ALPN, server did not agree to a protocol\n");
        Curl_multiuse_state(
            conn, conn->negnpn == CURL_HTTP_VERSION_2 ? BUNDLE_MULTIPLEX : BUNDLE_NO_MULTIUSE);
    }
#endif
    ...
    if (SSL_SET_OPTION(primary.sessionid)) {
        ...
        if (!incache) {
            result = Curl_ssl_addsessionid(
                conn, (void*)BACKEND->cred, sizeof(struct curl_schannel_cred), sockindex);
            if (result) {
                Curl_ssl_sessionid_unlock(conn);
    ...
    if (data->set.ssl.certinfo) {
        int certs_count = 0;
        sspi_status = s_pSecFn->QueryContextAttributes(
            &BACKEND->ctxt->ctxt_handle, SECPKG_ATTR_REMOTE_CERT_CONTEXT, &ccert_context);
...
static CURLcode
schannel_connect_common(struct connectdata* conn, int sockindex, bool nonblocking, bool* done)
{
    CURLcode result;
    struct Curl_easy* data = conn->data;
    struct ssl_connect_data* connssl = &conn->ssl[sockindex];
    ...
    while (ssl_connect_2 == connssl->connecting_state ||
        ...
        if (connssl->connecting_state == ssl_connect_2_reading ||
            ...
            curl_socket_t writefd =
                ssl_connect_2_writing == connssl->connecting_state ? sockfd : CURL_SOCKET_BAD;
            curl_socket_t readfd =
                ssl_connect_2_reading == connssl->connecting_state ? sockfd : CURL_SOCKET_BAD;

            what = Curl_socket_check(
                readfd, CURL_SOCKET_BAD, writefd, nonblocking ? 0 : (time_t)timeout_ms);
            if (what < 0) {
                /* fatal error */
...
static ssize_t
schannel_send(struct connectdata* conn, int sockindex, const void* buf, size_t len, CURLcode* err)
{
    ssize_t written = -1;
    size_t data_len = 0;
    ...
    if (BACKEND->stream_sizes.cbMaximumMessage == 0) {
        sspi_status = s_pSecFn->QueryContextAttributes(
            &BACKEND->ctxt->ctxt_handle, SECPKG_ATTR_STREAM_SIZES, &BACKEND->stream_sizes);
        if (sspi_status != SEC_E_OK) {
    ...
    data_len = BACKEND->stream_sizes.cbHeader + len + BACKEND->stream_sizes.cbTrailer;
    ...
    InitSecBuffer(&outbuf[0], SECBUFFER_STREAM_HEADER, data, BACKEND->stream_sizes.cbHeader);
    InitSecBuffer(
        &outbuf[1], SECBUFFER_DATA, data + BACKEND->stream_sizes.cbHeader, curlx_uztoul(len));
    InitSecBuffer(
        &outbuf[2],
    ...
    sspi_status = s_pSecFn->EncryptMessage(&BACKEND->ctxt->ctxt_handle, 0, &outbuf_desc, 0);
        ...
        */
        ...
        while (len > (size_t)written) {
            ...
            if (timeout_ms < 0) {
                ...
                failf(
                    conn->data,
                    "schannel: timed out sending data "
                    "(bytes sent: %zd)",
                    written);
            ...
            result = Curl_write_plain(
                conn, conn->sock[sockindex], data + written, len - written, &this_write);
            if (result == CURLE_AGAIN)
                continue;
...
static ssize_t
schannel_recv(struct connectdata* conn, int sockindex, char* buf, size_t len, CURLcode* err)
{
    size_t size = 0;
    ssize_t nread = -1;
    struct Curl_easy* data = conn->data;
    ...
    *err = CURLE_OK;
    ...
    if (len && len <= BACKEND->decdata_offset) {
        ...
        */
        ...
        if (size < CURL_SCHANNEL_BUFFER_FREE_SIZE || BACKEND->encdata_length < min_encdata_length) {
            reallocated_length = BACKEND->encdata_offset + CURL_SCHANNEL_BUFFER_FREE_SIZE;
    ...
    while (BACKEND->encdata_offset > 0 && sspi_status == SEC_E_OK &&
           (!len || BACKEND->decdata_offset < len || BACKEND->recv_connection_closed)) {
         ...
         */
        sspi_status = s_pSecFn->DecryptMessage(&BACKEND->ctxt->ctxt_handle, &inbuf_desc, 0, NULL);
        ...
        if (sspi_status == SEC_E_OK || sspi_status == SEC_I_RENEGOTIATE ||
            ...
            if (inbuf[1].BufferType == SECBUFFER_DATA) {
                ...
                if (BACKEND->decdata_length - BACKEND->decdata_offset < size ||
                    ...
                    reallocated_buffer = realloc(BACKEND->decdata_buffer, reallocated_length);
                    if (reallocated_buffer == NULL) {
                        *err = CURLE_OUT_OF_MEMORY;
                        failf(data, "schannel: unable to re-allocate memory");
                        goto cleanup;
                ...
                if (size) {
                    memcpy(
                        BACKEND->decdata_buffer + BACKEND->decdata_offset, inbuf[1].pvBuffer, size);
                ...
                DEBUGF(infof(
                    ...
                    BACKEND->decdata_offset,
                    BACKEND->decdata_length));
            }
            ...
            if (inbuf[3].BufferType == SECBUFFER_EXTRA && inbuf[3].cbBuffer > 0) {
                ...
                if (BACKEND->encdata_offset > inbuf[3].cbBuffer) {
                    ...
                    memmove(
                        BACKEND->encdata_buffer,
                        (BACKEND->encdata_buffer + BACKEND->encdata_offset) - inbuf[3].cbBuffer,
                        inbuf[3].cbBuffer);
...
cleanup:
    ...
    */
    ...
    if (size) {
        ...
        memmove(
            BACKEND->decdata_buffer,
            BACKEND->decdata_buffer + size,
            BACKEND->decdata_offset - size);
        BACKEND->decdata_offset -= size;
...
static CURLcode
Curl_schannel_connect_nonblocking(struct connectdata* conn, int sockindex, bool* done)
{
...
static bool Curl_schannel_data_pending(const struct connectdata* conn, int sockindex)
{
...
static int Curl_schannel_shutdown(struct connectdata* conn, int sockindex)
{
    ...
    if (BACKEND->cred && BACKEND->ctxt) {
        ...
        sspi_status = s_pSecFn->ApplyControlToken(&BACKEND->ctxt->ctxt_handle, &BuffDesc);
        ...
        if ((sspi_status == SEC_E_OK) || (sspi_status == SEC_I_CONTEXT_EXPIRED)) {
            ...
            ssize_t written;
            result = Curl_write_plain(
                conn, conn->sock[sockindex], outbuf.pvBuffer, outbuf.cbBuffer, &written);

            s_pSecFn->FreeContextBuffer(outbuf.pvBuffer);
            if ((result != CURLE_OK) || (outbuf.cbBuffer != (size_t)written)) {
                infof(
                    data,
                    "schannel: failed to send close msg: %s"
                    " (bytes written: %zd)\n",
...
static CURLcode
Curl_schannel_random(struct Curl_easy* data UNUSED_PARAM, unsigned char* entropy, size_t length)
{
    ...
    if (!CryptAcquireContext(
            &hCryptProv, NULL, NULL, PROV_RSA_FULL, CRYPT_VERIFYCONTEXT | CRYPT_SILENT))
...
pkp_pin_peer_pubkey(struct connectdata* conn, int sockindex, const char* pinnedpubkey)
{
    ...
    do {
        ...
        sspi_status = s_pSecFn->QueryContextAttributes(
            &BACKEND->ctxt->ctxt_handle, SECPKG_ATTR_REMOTE_CERT_CONTEXT, &pCertContextServer);
...
static void Curl_schannel_checksum(
    ...
    const unsigned int algId)
{
    ...
    if (!CryptAcquireContext(&hProv, NULL, NULL, provType, CRYPT_VERIFYCONTEXT | CRYPT_SILENT))
        return; /* failed */
    ...
    do {
        ...
        /* get hash size */
        if (!CryptGetHashParam(hHash, HP_HASHSIZE, (BYTE*)&cbHashSize, &dwHashSizeLen, 0))
            break; /* failed */
...
static CURLcode
Curl_schannel_md5sum(unsigned char* input, size_t inputlen, unsigned char* md5sum, size_t md5len)
{
    Curl_schannel_checksum(input, inputlen, md5sum, md5len, PROV_RSA_FULL, CALG_MD5);
    return CURLE_OK;
}

static CURLcode Curl_schannel_sha256sum(
    const unsigned char* input, size_t inputlen, unsigned char* sha256sum, size_t sha256len)
{
    Curl_schannel_checksum(input, inputlen, sha256sum, sha256len, PROV_RSA_AES, CALG_SHA_256);
    return CURLE_OK;
}

static void*
Curl_schannel_get_internals(struct ssl_connect_data* connssl, CURLINFO info UNUSED_PARAM)
{
    (void)info;
...
const struct Curl_ssl Curl_ssl_schannel = {
    ...
    SSLSUPP_CERTINFO | SSLSUPP_PINNEDPUBKEY,
