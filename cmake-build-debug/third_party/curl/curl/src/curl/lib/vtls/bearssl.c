...
static CURLcode load_cafile(const char* path, br_x509_trust_anchor** anchors, size_t* anchors_len)
{
        ...
        while (n) {
            ...
            switch (br_pem_decoder_event(&pc)) {
                ...
                case BR_PEM_BEGIN_OBJ:
                    ...
                    new_anchors = realloc(ca.anchors, new_anchors_len * sizeof(ca.anchors[0]));
                    if (!new_anchors) {
                        ca.err = CURLE_OUT_OF_MEMORY;
                        goto fail;
                    }
...
static CURLcode bearssl_connect_step1(struct connectdata* conn, int sockindex)
{
    ...
    const char* hostname = SSL_IS_PROXY() ? conn->http_proxy.host.name : conn->host.name;
    ...
    br_ssl_client_init_full(
        &backend->ctx, &backend->x509.minimal, backend->anchors, backend->anchors_len);
    br_ssl_engine_set_versions(&backend->ctx.eng, version_min, version_max);
    br_ssl_engine_set_buffer(&backend->ctx.eng, backend->buf, sizeof(backend->buf), 1);
    ...
    if (conn->bits.tls_enable_alpn) {
        ...
        br_ssl_engine_set_protocol_names(&backend->ctx.eng, backend->protocols, cur);
    }
...
static CURLcode bearssl_run_until(struct connectdata* conn, int sockindex, unsigned target)
{
    struct Curl_easy* data = conn->data;
    struct ssl_connect_data* connssl = &conn->ssl[sockindex];
    struct ssl_backend_data* backend = connssl->backend;
...
static ssize_t
bearssl_send(struct connectdata* conn, int sockindex, const void* buf, size_t len, CURLcode* err)
{
    struct Curl_easy* data = conn->data;
    struct ssl_connect_data* connssl = &conn->ssl[sockindex];
    struct ssl_backend_data* backend = connssl->backend;
...
static ssize_t
bearssl_recv(struct connectdata* conn, int sockindex, char* buf, size_t len, CURLcode* err)
{
    struct ssl_connect_data* connssl = &conn->ssl[sockindex];
    struct ssl_backend_data* backend = connssl->backend;
    unsigned char* app;
...
static CURLcode
bearssl_connect_common(struct connectdata* conn, int sockindex, bool nonblocking, bool* done)
{
    CURLcode ret;
    struct Curl_easy* data = conn->data;
    struct ssl_connect_data* connssl = &conn->ssl[sockindex];
    ...
    while (ssl_connect_2 == connssl->connecting_state ||
        ...
        if (ssl_connect_2_reading == connssl->connecting_state ||
            ...
            what =
                Curl_socket_check(readfd, CURL_SOCKET_BAD, writefd, nonblocking ? 0 : timeout_ms);
            if (what < 0) {
                /* fatal error */
...
static bool Curl_bearssl_data_pending(const struct connectdata* conn, int connindex)
{
...
static CURLcode
Curl_bearssl_random(struct Curl_easy* data UNUSED_PARAM, unsigned char* entropy, size_t length)
{
...
static CURLcode
Curl_bearssl_connect_nonblocking(struct connectdata* conn, int sockindex, bool* done)
{
    return bearssl_connect_common(conn, sockindex, TRUE, done);
}

static void*
Curl_bearssl_get_internals(struct ssl_connect_data* connssl, CURLINFO info UNUSED_PARAM)
{
    struct ssl_backend_data* backend = connssl->backend;
    return &backend->ctx;
}
...
static CURLcode Curl_bearssl_md5sum(
    unsigned char* input, size_t inputlen, unsigned char* md5sum, size_t md5len UNUSED_PARAM)
{
    br_md5_context ctx;
...
static CURLcode Curl_bearssl_sha256sum(
