 ...
 */
static CURLcode wolfssl_connect_step1(struct connectdata* conn, int sockindex)
{
    char* ciphers;
...
#define use_sni(x) Curl_nop_stmt
    ...
    switch (SSL_CONN_CONFIG(version)) {
        ...
        case CURL_SSLVERSION_TLSv1:
            ...
            infof(
                data,
                "wolfSSL <3.3.0 cannot be configured to use TLS 1.0-1.2, "
    ...
    if (SSL_SET_OPTION(cert) && SSL_SET_OPTION(key)) {
        ...
        if (SSL_CTX_use_certificate_file(backend->ctx, SSL_SET_OPTION(cert), file_type) != 1) {
            failf(
                data,
...
#endif
            (wolfSSL_CTX_UseSNI(
                 backend->ctx, WOLFSSL_SNI_HOST_NAME, hostname, (unsigned short)hostname_len) !=
             1)) {
            infof(
                data,
    ...
    if (data->set.ssl.fsslctx) {
        ...
        failf(
            data,
            "SSL: Certificates can't be loaded because wolfSSL was built"
...
static CURLcode wolfssl_connect_step2(struct connectdata* conn, int sockindex)
{
    int ret = -1;
    struct Curl_easy* data = conn->data;
    struct ssl_connect_data* connssl = &conn->ssl[sockindex];
    ...
    if (conn->bits.tls_enable_alpn) {
        ...
        if (rc == SSL_SUCCESS) {
            ...
            if (protocol_len == ALPN_HTTP_1_1_LENGTH &&
                ...
                !memcmp(protocol, NGHTTP2_PROTO_VERSION_ID, NGHTTP2_PROTO_VERSION_ID_LEN))
...
static CURLcode wolfssl_connect_step3(struct connectdata* conn, int sockindex)
{
...
static ssize_t
wolfssl_recv(struct connectdata* conn, int num, char* buf, size_t buffersize, CURLcode* curlcode)
{
...
static bool Curl_wolfssl_data_pending(const struct connectdata* conn, int connindex)
{
...
static CURLcode
wolfssl_connect_common(struct connectdata* conn, int sockindex, bool nonblocking, bool* done)
{
    CURLcode result;
    struct Curl_easy* data = conn->data;
    struct ssl_connect_data* connssl = &conn->ssl[sockindex];
...
static CURLcode
Curl_wolfssl_connect_nonblocking(struct connectdata* conn, int sockindex, bool* done)
{
...
static CURLcode Curl_wolfssl_random(struct Curl_easy* data, unsigned char* entropy, size_t length)
{
    WC_RNG rng;
    (void)data;
    if (wc_InitRng(&rng))
...
static CURLcode Curl_wolfssl_sha256sum(
...
static void*
Curl_wolfssl_get_internals(struct ssl_connect_data* connssl, CURLINFO info UNUSED_PARAM)
{
    struct ssl_backend_data* backend = connssl->backend;
    (void)info;
