 ...
 */
static CURLcode mesalink_connect_step1(struct connectdata* conn, int sockindex)
{
    char* ciphers;
    ...
    SSL_CTX_set_verify(
        BACKEND->ctx, SSL_CONN_CONFIG(verifypeer) ? SSL_VERIFY_PEER : SSL_VERIFY_NONE, NULL);

    if (SSL_CONN_CONFIG(CAfile) || SSL_CONN_CONFIG(CApath)) {
        if (!SSL_CTX_load_verify_locations(
                BACKEND->ctx, SSL_CONN_CONFIG(CAfile), SSL_CONN_CONFIG(CApath))) {
    ...
    if (SSL_SET_OPTION(cert) && SSL_SET_OPTION(key)) {
        ...
        if (SSL_CTX_use_certificate_chain_file(BACKEND->ctx, SSL_SET_OPTION(cert), file_type) !=
            1) {
            failf(
                data,
                "unable to use client certificate (no key or wrong pass"
                " phrase?)");
            return CURLE_SSL_CONNECT_ERROR;
        }
...
static CURLcode mesalink_connect_step2(struct connectdata* conn, int sockindex)
{
    int ret = -1;
    struct Curl_easy* data = conn->data;
    struct ssl_connect_data* connssl = &conn->ssl[sockindex];
...
static CURLcode mesalink_connect_step3(struct connectdata* conn, int sockindex)
{
    ...
    if (SSL_SET_OPTION(primary.sessionid)) {
        ...
        if (!incache) {
            result =
                Curl_ssl_addsessionid(conn, our_ssl_sessionid, 0 /* unknown size */, sockindex);
            if (result) {
                Curl_ssl_sessionid_unlock(conn);
...
static void Curl_mesalink_close(struct connectdata* conn, int sockindex)
{
    struct ssl_connect_data* connssl = &conn->ssl[sockindex];
...
static ssize_t
mesalink_recv(struct connectdata* conn, int num, char* buf, size_t buffersize, CURLcode* curlcode)
{
...
static size_t Curl_mesalink_version(char* buffer, size_t size)
{
    return msnprintf(buffer, size, "MesaLink/%s", MESALINK_VERSION_STRING);
}

static int Curl_mesalink_init(void)
{
 ...
 */
static int Curl_mesalink_shutdown(struct connectdata* conn, int sockindex)
{
    int retval = 0;
    struct ssl_connect_data* connssl = &conn->ssl[sockindex];
...
static CURLcode
mesalink_connect_common(struct connectdata* conn, int sockindex, bool nonblocking, bool* done)
{
    CURLcode result;
    struct Curl_easy* data = conn->data;
    struct ssl_connect_data* connssl = &conn->ssl[sockindex];
...
static CURLcode
Curl_mesalink_connect_nonblocking(struct connectdata* conn, int sockindex, bool* done)
{
    return mesalink_connect_common(conn, sockindex, TRUE, done);
}

static CURLcode Curl_mesalink_connect(struct connectdata* conn, int sockindex)
{
    CURLcode result;
    bool done = FALSE;
...
static void*
Curl_mesalink_get_internals(struct ssl_connect_data* connssl, CURLINFO info UNUSED_PARAM)
{
    (void)info;
