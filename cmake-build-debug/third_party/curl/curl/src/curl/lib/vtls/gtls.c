...
static gnutls_datum_t load_file(const char* file)
{
    ...
    if (!f)
        return loaded_file;
    if (fseek(f, 0, SEEK_END) != 0 || (filelen = ftell(f)) < 0 || fseek(f, 0, SEEK_SET) != 0 ||
...
static CURLcode
handshake(struct connectdata* conn, int sockindex, bool duringconnect, bool nonblocking)
{
    ...
    for (;;) {
        ...
        if (connssl->connecting_state == ssl_connect_2_reading ||
            ...
            what = Curl_socket_check(
                ...
                CURL_SOCKET_BAD,
                writefd,
                nonblocking ? 0 : timeout_ms ? (time_t)timeout_ms : 1000);
            if (what < 0) {
                /* fatal error */
        ...
        if ((rc == GNUTLS_E_AGAIN) || (rc == GNUTLS_E_INTERRUPTED)) {
            connssl->connecting_state = gnutls_record_get_direction(session) ?
...
static CURLcode gtls_connect_step1(struct connectdata* conn, int sockindex)
{
    ...
    if (SSL_SET_OPTION(authtype) == CURL_TLSAUTH_SRP) {
        ...
        rc = gnutls_srp_set_client_credentials(
            backend->srp_client_cred, SSL_SET_OPTION(username), SSL_SET_OPTION(password));
        if (rc != GNUTLS_E_SUCCESS) {
    ...
    if (SSL_CONN_CONFIG(CAfile)) {
        ...
        rc = gnutls_certificate_set_x509_trust_file(
            backend->cred, SSL_CONN_CONFIG(CAfile), GNUTLS_X509_FMT_PEM);
        if (rc < 0) {
            infof(
                data,
    ...
    if (SSL_CONN_CONFIG(CApath)) {
        ...
        rc = gnutls_certificate_set_x509_trust_dir(
            backend->cred, SSL_CONN_CONFIG(CApath), GNUTLS_X509_FMT_PEM);
        if (rc < 0) {
            infof(
                data,
                "error reading ca cert file %s (%s)\n",
    ...
    if (SSL_SET_OPTION(CRLfile)) {
        ...
        rc = gnutls_certificate_set_x509_crl_file(
            backend->cred, SSL_SET_OPTION(CRLfile), GNUTLS_X509_FMT_PEM);
        if (rc < 0) {
            failf(
                data,
    ...
    if (SSL_SET_OPTION(cert)) {
        if (SSL_SET_OPTION(key_passwd)) {
            ...
            rc = gnutls_certificate_set_x509_key_file2(
                backend->cred,
                SSL_SET_OPTION(cert),
                SSL_SET_OPTION(key) ? SSL_SET_OPTION(key) : SSL_SET_OPTION(cert),
            ...
            if (gnutls_certificate_set_x509_key_file(
                    ...
                    SSL_SET_OPTION(cert),
                    SSL_SET_OPTION(key) ? SSL_SET_OPTION(key) : SSL_SET_OPTION(cert),
                    do_file_type(SSL_SET_OPTION(cert_type))) != GNUTLS_E_SUCCESS) {
...
static CURLcode
pkp_pin_peer_pubkey(struct Curl_easy* data, gnutls_x509_crt_t cert, const char* pinnedpubkey)
{
    /* Scratch */
...
static CURLcode gtls_connect_step3(struct connectdata* conn, int sockindex)
{
    ...
    ptr = gnutls_cipher_suite_get_name(
        gnutls_kx_get(session), gnutls_cipher_get(session), gnutls_mac_get(session));
    ...
    if (SSL_CONN_CONFIG(verifystatus)) {
        if (gnutls_ocsp_status_request_is_checked(session, 0) == 0) {
            ...
            (void)gnutls_ocsp_resp_get_single(
                ocsp_resp, 0, NULL, NULL, NULL, NULL, &status, NULL, NULL, NULL, &reason);
...
#define use_addr in_addr
        ...
        if (addrlen) {
            ...
            for (i = 0;; i++) {
                ...
                int ret = gnutls_x509_crt_get_subject_alt_name(
                    x509_cert, i, certaddr, &certaddrlen, NULL);
    ...
    if (conn->bits.tls_enable_alpn) {
        ...
        if (rc == 0) {
            ...
            if (proto.size == NGHTTP2_PROTO_VERSION_ID_LEN &&
                !memcmp(NGHTTP2_PROTO_VERSION_ID, proto.data, NGHTTP2_PROTO_VERSION_ID_LEN)) {
 ...
 */
static CURLcode
gtls_connect_common(struct connectdata* conn, int sockindex, bool nonblocking, bool* done)
{
    int rc;
    struct ssl_connect_data* connssl = &conn->ssl[sockindex];
...
static CURLcode Curl_gtls_connect_nonblocking(struct connectdata* conn, int sockindex, bool* done)
{
...
static bool Curl_gtls_data_pending(const struct connectdata* conn, int connindex)
{
...
static ssize_t
gtls_send(struct connectdata* conn, int sockindex, const void* mem, size_t len, CURLcode* curlcode)
{
    struct ssl_connect_data* connssl = &conn->ssl[sockindex];
    struct ssl_backend_data* backend = connssl->backend;
...
/* data might be NULL! */
static CURLcode Curl_gtls_random(struct Curl_easy* data, unsigned char* entropy, size_t length)
{
...
static CURLcode Curl_gtls_md5sum(
...
static CURLcode Curl_gtls_sha256sum(
...
static void* Curl_gtls_get_internals(struct ssl_connect_data* connssl, CURLINFO info UNUSED_PARAM)
{
    struct ssl_backend_data* backend = connssl->backend;
    (void)info;
