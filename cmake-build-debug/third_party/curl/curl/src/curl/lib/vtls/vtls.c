...
#define SSLSESSION_SHARED(data) \
    (data->share && (data->share->specifier & (1 << CURL_LOCK_DATA_SSL_SESSION)))
...
bool Curl_clone_primary_ssl_config(
    struct ssl_primary_config* source, struct ssl_primary_config* dest)
{
    ...
    int sockindex)
{
    ...
    struct ssl_primary_config* const ssl_config =
        isProxy ? &conn->proxy_ssl_config : &conn->ssl_config;
    ...
    *ssl_sessionid = NULL;
    ...
    for (i = 0; i < data->set.general_ssl.max_ssl_sessions; i++) {
        ...
        if (strcasecompare(name, check->name) &&
            ...
            (port == check->remote_port) && strcasecompare(conn->handler->scheme, check->scheme) &&
 ...
 */
CURLcode
Curl_ssl_addsessionid(struct connectdata* conn, void* ssl_sessionid, size_t idsize, int sockindex)
{
    size_t i;
    ...
    for (i = 1; (i < data->set.general_ssl.max_ssl_sessions) && data->state.session[i].sessionid;
         i++) {
...
void Curl_ssl_close_all(struct Curl_easy* data)
{
    ...
    defined(USE_SECTRANSP) || defined(USE_NSS) || defined(USE_MBEDTLS) || defined(USE_WOLFSSL) || \
    defined(USE_BEARSSL)
...
CURLcode Curl_ssl_push_certinfo_len(
    struct Curl_easy* data, int certnum, const char* label, const char* value, size_t valuelen)
{
    struct curl_certinfo* ci = &data->info.certs;
    char* output;
 ...
 */
CURLcode
Curl_ssl_push_certinfo(struct Curl_easy* data, int certnum, const char* label, const char* value)
{
...
CURLcode Curl_pin_peer_pubkey(
    struct Curl_easy* data, const char* pinnedpubkey, const unsigned char* pubkey, size_t pubkeylen)
{
    FILE* fp;
    ...
    if (strncmp(pinnedpubkey, "sha256//", 8) == 0) {
        ...
        encode = Curl_base64_encode(
            data, (char*)sha256sumdigest, CURL_SHA256_DIGEST_LENGTH, &encoded, &encodedlen);
...
int Curl_none_shutdown(struct connectdata* conn UNUSED_PARAM, int sockindex UNUSED_PARAM)
{
    (void)conn;
    (void)sockindex;
    return 0;
...
#else
CURLcode Curl_none_md5sum(
    unsigned char* input UNUSED_PARAM,
...
static CURLcode
Curl_multissl_connect_nonblocking(struct connectdata* conn, int sockindex, bool* done)
{
    if (multissl_init(NULL))
        return CURLE_FAILED_INIT;
...
CURLsslset curl_global_sslset(curl_sslbackend id, const char* name, const curl_ssl_backend*** avail)
{
    ...
    if (Curl_ssl != &Curl_ssl_multi)
        return id == Curl_ssl->info.id || (name && strcasecompare(name, Curl_ssl->info.name)) ?
                   CURLSSLSET_OK :
#if defined(CURL_WITH_MULTI_SSL)
                   CURLSSLSET_TOO_LATE;
#else
