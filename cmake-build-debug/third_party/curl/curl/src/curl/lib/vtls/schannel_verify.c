...
static CURLcode
add_certs_to_store(HCERTSTORE trust_store, const char* ca_file, struct connectdata* conn)
{
    CURLcode result;
    ...
    while (total_bytes_read < ca_file_bufsize) {
        ...
        if (!ReadFile(
                ca_file_handle,
                ca_file_buffer + total_bytes_read,
                bytes_to_read,
                &bytes_read,
                NULL)) {
            char buffer[STRERROR_LEN];
    ...
    while (more_certs && *current_ca_file_ptr != '\0') {
        ...
        if (!begin_cert_ptr || !is_cr_or_lf(begin_cert_ptr[strlen(BEGIN_CERT)])) {
            ...
            if (!end_cert_ptr) {
                failf(data, "schannel: CA file '%s' is not correctly formatted", ca_file);
                result = CURLE_SSL_CACERT_BADFILE;
                more_certs = 0;
            } else {
                CERT_BLOB cert_blob;
                ...
                DWORD cert_size = (DWORD)((end_cert_ptr + END_CERT_LEN) - begin_cert_ptr);
                ...
                if (!CryptQueryObject(
                    ...
                    if (CERT_QUERY_CONTENT_CERT != actual_content_type) {
                        ...
                        add_cert_result = CertAddCertificateContextToStore(
                            trust_store, cert_context, CERT_STORE_ADD_ALWAYS, NULL);
    ...
    if (result == CURLE_OK) {
        if (!num_certs) {
            ...
            infof(
                data, "schannel: added %d certificate(s) from CA file '%s'\n", num_certs, ca_file);
        }
    }
 ...
 * string is returned.
 */
static DWORD cert_get_name_string(
    struct Curl_easy* data, CERT_CONTEXT* cert_context, LPTSTR host_names, DWORD length)
{
    ...
    if (Curl_verify_windows_version(6, 2, PLATFORM_WINNT, VERSION_GREATER_THAN_EQUAL)) {
#ifdef CERT_NAME_SEARCH_ALL_NAMES_FLAG
        /* CertGetNameString will provide the 8-bit character string without
         * any decoding */
        DWORD name_flags = CERT_NAME_DISABLE_IE4_UTF8_FLAG | CERT_NAME_SEARCH_ALL_NAMES_FLAG;
        actual_length = CertGetNameString(
            cert_context, CERT_NAME_DNS_TYPE, name_flags, NULL, host_names, length);
    ...
    extension =
        CertFindExtension(szOID_SUBJECT_ALT_NAME2, cert_info->cExtension, cert_info->rgExtension);
    if (!extension) {
...
static CURLcode verify_host(
    struct Curl_easy* data, CERT_CONTEXT* pCertContextServer, const char* const conn_hostname)
{
    ...
    actual_len = cert_get_name_string(data, pCertContextServer, (LPTSTR)cert_hostname_buff, len);
    ...
    while (cert_hostname_buff_index < len &&
         ...
         */
        cert_hostname = Curl_convert_tchar_to_UTF8(&cert_hostname_buff[cert_hostname_buff_index]);
        if (!cert_hostname) {
            result = CURLE_OUT_OF_MEMORY;
        } else {
            int match_result;
            ...
            if (match_result == CURL_HOST_MATCH) {
                ...
                cert_hostname_len = _tcslen(&cert_hostname_buff[cert_hostname_buff_index]);
...
CURLcode Curl_verify_certificate(struct connectdata* conn, int sockindex)
{
    ...
    if (result == CURLE_OK && SSL_CONN_CONFIG(CAfile) && BACKEND->use_manual_cred_validation) {
        /*
        ...
        if (Curl_verify_windows_version(6, 1, PLATFORM_WINNT, VERSION_LESS_THAN)) {
            ...
            trust_store = CertOpenStore(
                CERT_STORE_PROV_MEMORY, 0, (HCRYPTPROV)NULL, CERT_STORE_CREATE_NEW_FLAG, NULL);
            if (!trust_store) {
                char buffer[STRERROR_LEN];
                failf(
                    data,
                ...
                result = add_certs_to_store(trust_store, SSL_CONN_CONFIG(CAfile), conn);
            }
        }
        ...
        if (result == CURLE_OK) {
             ...
             */
            create_engine_result = CertCreateCertificateChainEngine(
    ...
    if (result == CURLE_OK) {
        ...
        if (!CertGetCertificateChain(
                ...
                &ChainPara,
                (data->set.ssl.no_revoke ? 0 : CERT_CHAIN_REVOCATION_CHECK_CHAIN),
                NULL,
                &pChainContext)) {
            char buffer[STRERROR_LEN];
            failf(
