...
static const gskit_cipher ciphertable[] = {
    {"null-md5",
     "01",
     CURL_GSKPROTO_SSLV3_MASK | CURL_GSKPROTO_TLSV10_MASK | CURL_GSKPROTO_TLSV11_MASK |
         CURL_GSKPROTO_TLSV12_MASK},
    {"null-sha",
     "02",
     CURL_GSKPROTO_SSLV3_MASK | CURL_GSKPROTO_TLSV10_MASK | CURL_GSKPROTO_TLSV11_MASK |
         CURL_GSKPROTO_TLSV12_MASK},
    {"exp-rc4-md5", "03", CURL_GSKPROTO_SSLV3_MASK | CURL_GSKPROTO_TLSV10_MASK},
    {"rc4-md5",
     "04",
     CURL_GSKPROTO_SSLV3_MASK | CURL_GSKPROTO_TLSV10_MASK | CURL_GSKPROTO_TLSV11_MASK |
         CURL_GSKPROTO_TLSV12_MASK},
    {"rc4-sha",
     ...
     CURL_GSKPROTO_SSLV3_MASK | CURL_GSKPROTO_TLSV10_MASK | CURL_GSKPROTO_TLSV11_MASK |
         CURL_GSKPROTO_TLSV12_MASK},
    {"exp-rc2-cbc-md5", "06", CURL_GSKPROTO_SSLV3_MASK | CURL_GSKPROTO_TLSV10_MASK},
    {"exp-des-cbc-sha",
     "09",
     CURL_GSKPROTO_SSLV3_MASK | CURL_GSKPROTO_TLSV10_MASK | CURL_GSKPROTO_TLSV11_MASK},
    {"des-cbc3-sha",
    ...
    {"aes128-sha",
     "2F",
     CURL_GSKPROTO_TLSV10_MASK | CURL_GSKPROTO_TLSV11_MASK | CURL_GSKPROTO_TLSV12_MASK},
    {"aes256-sha",
     "35",
     CURL_GSKPROTO_TLSV10_MASK | CURL_GSKPROTO_TLSV11_MASK | CURL_GSKPROTO_TLSV12_MASK},
    ...
    {"aes128-gcm-sha256", "9C", CURL_GSKPROTO_TLSV12_MASK},
    {"aes256-gcm-sha384", "9D", CURL_GSKPROTO_TLSV12_MASK},
...
static CURLcode set_enum(
    struct Curl_easy* data, gsk_handle h, GSK_ENUM_ID id, GSK_ENUM_VALUE value, bool unsupported_ok)
{
...
static CURLcode set_buffer(
    struct Curl_easy* data, gsk_handle h, GSK_BUF_ID id, const char* buffer, bool unsupported_ok)
{
...
static CURLcode set_ciphers(struct connectdata* conn, gsk_handle h, unsigned int* protoflags)
{
    ...
    for (;;) {
        ...
        if (!ctp->name) {
            ...
            unsupported |=
                !(ctp->versions & (CURL_GSKPROTO_SSLV2_MASK | CURL_GSKPROTO_SSLV3_MASK |
    ...
    if (*protoflags & CURL_GSKPROTO_TLSV11_MASK) {
        result =
            set_buffer(data, h, GSK_TLSV11_CIPHER_SPECS, ciphers[CURL_GSKPROTO_TLSV11].buf, TRUE);
    ...
    if (!result && (*protoflags & CURL_GSKPROTO_TLSV12_MASK)) {
        result =
            set_buffer(data, h, GSK_TLSV12_CIPHER_SPECS, ciphers[CURL_GSKPROTO_TLSV12].buf, TRUE);
        if (result == CURLE_UNSUPPORTED_PROTOCOL) {
            result = CURLE_OK;
            if (unsupported) {
    ...
    if (!result && (*protoflags & CURL_GSKPROTO_TLSV10_MASK)) {
        result =
            set_buffer(data, h, GSK_TLSV10_CIPHER_SPECS, ciphers[CURL_GSKPROTO_TLSV10].buf, TRUE);
        if (result == CURLE_UNSUPPORTED_PROTOCOL) {
            result = CURLE_OK;
...
static CURLcode init_environment(
    ...
    const char* password)
{
    ...
    if (!result) {
        ...
        result = gskit_status(
            data, gsk_environment_init(h), "gsk_environment_init()", CURLE_SSL_CERTPROBLEM);
        if (!result) {
            *envir = h;
            return result;
        }
...
static void close_one(struct ssl_connect_data* connssl, struct connectdata* conn, int sockindex)
{
    if (BACKEND->handle) {
        gskit_status(
            conn->data, gsk_secure_soc_close(&BACKEND->handle), "gsk_secure_soc_close()", 0);
...
static ssize_t
gskit_send(struct connectdata* conn, int sockindex, const void* mem, size_t len, CURLcode* curlcode)
{
    struct ssl_connect_data* connssl = &conn->ssl[sockindex];
    struct Curl_easy* data = conn->data;
    ...
    if (pipe_ssloverssl(conn, sockindex, SOS_WRITE) >= 0) {
        cc = gskit_status(
            data,
            gsk_secure_soc_write(BACKEND->handle, (char*)mem, (int)len, &written),
...
static ssize_t
gskit_recv(struct connectdata* conn, int num, char* buf, size_t buffersize, CURLcode* curlcode)
{
    ...
    if (pipe_ssloverssl(conn, num, SOS_READ) >= 0) {
        ...
        cc = gskit_status(
            data,
            gsk_secure_soc_read(BACKEND->handle, buf, buffsize, &nread),
            "gsk_secure_soc_read()",
            CURLE_RECV_ERROR);
    }
    switch (cc) {
...
static CURLcode gskit_connect_step1(struct connectdata* conn, int sockindex)
{
    ...
    if (keyringlabel && *keyringlabel && !keyringpwd && !strcmp(keyringfile, CURL_CA_BUNDLE)) {
        ...
        init_environment(
            data, &envir, keyringlabel, (const char*)NULL, (const char*)NULL, (const char*)NULL);
    }
    ...
    result = gskit_status(
        data,
        gsk_secure_soc_open(envir, &BACKEND->handle),
        "gsk_secure_soc_open()",
    ...
    if (!result) {
        ...
        if (timeout < 0)
            ...
            result =
                set_numeric(data, BACKEND->handle, GSK_HANDSHAKE_TIMEOUT, (timeout + 999) / 1000);
    }
    if (!result)
    ...
    if (!result)
        result = set_numeric(
            ...
            BACKEND->handle,
            GSK_FD,
            BACKEND->localfd >= 0 ? BACKEND->localfd : conn->sock[sockindex]);
    if (!result)
    ...
    if (!result)
        result = set_enum(
            ...
            BACKEND->handle,
            GSK_PROTOCOL_SSLV2,
            (protoflags & CURL_GSKPROTO_SSLV2_MASK) ? GSK_PROTOCOL_SSLV2_ON :
    ...
    if (!result)
        result = set_enum(
            ...
            BACKEND->handle,
            GSK_PROTOCOL_SSLV3,
            (protoflags & CURL_GSKPROTO_SSLV3_MASK) ? GSK_PROTOCOL_SSLV3_ON :
    ...
    if (!result)
        result = set_enum(
            ...
            BACKEND->handle,
            GSK_PROTOCOL_TLSV1,
            (protoflags & CURL_GSKPROTO_TLSV10_MASK) ? GSK_PROTOCOL_TLSV1_ON :
    ...
    if (!result) {
        result = set_enum(
            ...
            BACKEND->handle,
            GSK_PROTOCOL_TLSV11,
            (protoflags & CURL_GSKPROTO_TLSV11_MASK) ? GSK_TRUE : GSK_FALSE,
            TRUE);
        if (result == CURLE_UNSUPPORTED_PROTOCOL) {
            result = CURLE_OK;
            if (protoflags == CURL_GSKPROTO_TLSV11_MASK) {
    ...
    if (!result) {
        result = set_enum(
            ...
            BACKEND->handle,
            GSK_PROTOCOL_TLSV12,
            (protoflags & CURL_GSKPROTO_TLSV12_MASK) ? GSK_TRUE : GSK_FALSE,
            TRUE);
        if (result == CURLE_UNSUPPORTED_PROTOCOL) {
            result = CURLE_OK;
    ...
    if (!result)
        result = set_enum(
            ...
            verifypeer ? GSK_SERVER_AUTH_FULL : GSK_SERVER_AUTH_PASSTHRU,
            FALSE);
    ...
    if (!result) {
        ...
        if (BACKEND->iocport != -1) {
            result = gskit_status(
                data,
                gsk_secure_soc_startInit(BACKEND->handle, BACKEND->iocport, &commarea),
            ...
            result = gskit_status(
                data,
                gsk_secure_soc_init(BACKEND->handle),
                "gsk_secure_soc_init()",
                CURLE_SSL_CONNECT_ERROR);
            if (!result) {
...
static CURLcode gskit_connect_step2(struct connectdata* conn, int sockindex, bool nonblocking)
{
    struct Curl_easy* data = conn->data;
    struct ssl_connect_data* connssl = &conn->ssl[sockindex];
    Qso_OverlappedIO_t cstat;
...
static CURLcode gskit_connect_step3(struct connectdata* conn, int sockindex)
{
    ...
    if (gskit_status(
            data,
            gsk_attribute_get_cert_info(BACKEND->handle, GSK_PARTNER_CERT_INFO, &cdev, &cdec),
            "gsk_attribute_get_cert_info()",
            CURLE_SSL_CONNECT_ERROR) == CURLE_OK) {
        int i;
...
static CURLcode
gskit_connect_common(struct connectdata* conn, int sockindex, bool nonblocking, bool* done)
{
    struct Curl_easy* data = conn->data;
    struct ssl_connect_data* connssl = &conn->ssl[sockindex];
...
static CURLcode Curl_gskit_connect_nonblocking(struct connectdata* conn, int sockindex, bool* done)
{
    CURLcode result;
...
static int Curl_gskit_check_cxn(struct connectdata* cxn)
{
    ...
    if (getsockopt(cxn->sock[FIRSTSOCKET], SOL_SOCKET, SO_ERROR, (unsigned char*)&err, &errlen) ||
...
static void* Curl_gskit_get_internals(struct ssl_connect_data* connssl, CURLINFO info UNUSED_PARAM)
{
    (void)info;
