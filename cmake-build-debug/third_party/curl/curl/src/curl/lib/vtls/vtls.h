...
struct Curl_ssl {
    ...
    CURLcode (*md5sum)(
        unsigned char* input, size_t inputlen, unsigned char* md5sum, size_t md5sumlen);
    CURLcode (*sha256sum)(
        const unsigned char* input, size_t inputlen, unsigned char* sha256sum, size_t sha256sumlen);
};
...
CURLcode
Curl_none_md5sum(unsigned char* input, size_t inputlen, unsigned char* md5sum, size_t md5len);
...
#define SSL_IS_PROXY() \
    (CURLPROXY_HTTPS == conn->http_proxy.proxytype && \
     ssl_connection_complete != \
         conn->proxy_ssl \
             [conn->sock[SECONDARYSOCKET] == CURL_SOCKET_BAD ? FIRSTSOCKET : SECONDARYSOCKET] \
                 .state)
