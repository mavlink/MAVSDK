...
#if OPENSSL_VERSION_NUMBER >= 0x10002003L && OPENSSL_VERSION_NUMBER <= 0x10002FFFL && \
...
static void tap_ssl_key(const SSL* ssl, ssl_tap_state_t* state)
{
    ...
    master_key_length =
        (int)SSL_SESSION_get_master_key(session, master_key, SSL_MAX_MASTER_KEY_LENGTH);
#else
...
#define RANDOM_FILE "" /* doesn't matter won't be used */
#endif
    {
        ...
        RAND_load_file(
            (data->set.str[STRING_SSL_RANDOM_FILE] ? data->set.str[STRING_SSL_RANDOM_FILE] :
                                                     RANDOM_FILE),
            RAND_LOAD_LENGTH);
        if (rand_enough())
            return CURLE_OK;
...
#define EGD_SOCKET "" /* doesn't matter won't be used */
#endif
    {
        /* If there's an option and a define, the option overrides the
           define */
        int ret = RAND_egd(
            data->set.str[STRING_SSL_EGDSOCKET] ? data->set.str[STRING_SSL_EGDSOCKET] : EGD_SOCKET);
        if (-1 != ret) {
            if (rand_enough())
                return CURLE_OK;
        }
...
static CURLcode Curl_ossl_set_engine(struct Curl_easy* data, const char* engine);
        ...
        switch (file_type) {
            case SSL_FILETYPE_PEM:
                ...
                if (SSL_CTX_use_certificate_chain_file(ctx, cert_file) != 1) {
                    failf(
                        data,
                        "could not load PEM client certificate, " OSSL_PACKAGE " error %s, "
            ...
            case SSL_FILETYPE_ASN1:
                ...
                if (SSL_CTX_use_certificate_file(ctx, cert_file, file_type) != 1) {
                    failf(
                        data,
                        "could not load ASN1 client certificate, " OSSL_PACKAGE " error %s, "
                        "(no key found, wrong pass phrase, or wrong file format?)",
                        ossl_strerror(ERR_get_error(), error_buffer, sizeof(error_buffer)));
                    return 0;
                }
            ...
            case SSL_FILETYPE_PKCS12: {
                ...
                if (!PKCS12_parse(p12, key_passwd, &pri, &x509, &ca)) {
                    failf(
                        data,
                        "could not parse PKCS12 file, check password, " OSSL_PACKAGE " error %s",
                        ossl_strerror(ERR_get_error(), error_buffer, sizeof(error_buffer)));
                    PKCS12_free(p12);
                    return 0;
                }
                ...
                if (SSL_CTX_use_certificate(ctx, x509) != 1) {
                    failf(
                        data,
                        "could not load PKCS12 client certificate, " OSSL_PACKAGE " error %s",
                        ossl_strerror(ERR_get_error(), error_buffer, sizeof(error_buffer)));
                    goto fail;
                }
        ...
        switch (file_type) {
            ...
            { /* XXXX still needs some work */
                ...
                if (data->state.engine) {
                    ...
                    priv_key = (EVP_PKEY*)ENGINE_load_private_key(
                        data->state.engine, key_file, ui_method, key_passwd);
...
#define CONF_MFLAGS_DEFAULT_SECTION 0x0
    ...
    CONF_modules_load_file(
        NULL, NULL, CONF_MFLAGS_DEFAULT_SECTION | CONF_MFLAGS_IGNORE_MISSING_FILE);
#endif
...
static int Curl_ossl_check_cxn(struct connectdata* conn)
{
    ...
    ssize_t nread;
    nread = recv(
        (RECV_TYPE_ARG1)conn->sock[FIRSTSOCKET],
        (RECV_TYPE_ARG2)&buf,
        (RECV_TYPE_ARG3)1,
...
static CURLcode Curl_ossl_set_engine(struct Curl_easy* data, const char* engine)
{
    ...
    if (!ENGINE_init(e)) {
        ...
        failf(
            ...
            engine,
            ossl_strerror(ERR_get_error(), buf, sizeof(buf)));
            ...
            if (what > 0) {
                ...
                switch (err) {
                    ...
                    default:
                        ...
                        failf(
                            conn->data,
                            OSSL_PACKAGE " SSL_read on shutdown: %s, errno %d",
                            (sslerror ? ossl_strerror(sslerror, buf, sizeof(buf)) :
 ...
 */
static bool subj_alt_hostcheck(
    struct Curl_easy* data, const char* match_pattern, const char* hostname, const char* dispname)
...
#else
{
    ...
    if (Curl_cert_hostcheck(match_pattern, hostname)) {
        infof(
            data, " subjectAltName: host \"%s\" matched cert's \"%s\"\n", dispname, match_pattern);
        return TRUE;
    }
    return FALSE;
}
...
#if ( \
    (OPENSSL_VERSION_NUMBER <= 0x1000201fL) /* Fixed after 1.0.2a */ || \
    ...
    void* userp)
{
    ...
    Curl_debug(
        data, (direction == 1) ? CURLINFO_SSL_DATA_OUT : CURLINFO_SSL_DATA_IN, (char*)buf, len);
    (void)ssl;
}
#endif
...
static int select_next_protocol(
    unsigned char** out,
    ...
    void* arg)
{
    ...
    if (conn->data->set.httpversion >= CURL_HTTP_VERSION_2 &&
        !select_next_protocol(
            out, outlen, in, inlen, NGHTTP2_PROTO_VERSION_ID, NGHTTP2_PROTO_VERSION_ID_LEN)) {
...
#if (OPENSSL_VERSION_NUMBER < 0x10100000L) /* 1.1.0 */
static CURLcode
set_ssl_version_min_max_legacy(ctx_option_t* ctx_options, struct connectdata* conn, int sockindex)
{
...
static int ossl_new_session_cb(SSL* ssl, SSL_SESSION* ssl_sessionid)
{
    ...
    if (SSL_SET_OPTION(primary.sessionid)) {
        ...
        if (!incache) {
            if (!Curl_ssl_addsessionid(conn, ssl_sessionid, 0 /* unknown size */, sockindex)) {
...
static CURLcode ossl_connect_step1(struct connectdata* conn, int sockindex)
{
...
#endif
#endif
    long* const certverifyresult =
        SSL_IS_PROXY() ? &data->set.proxy_ssl.certverifyresult : &data->set.ssl.certverifyresult;
    ...
    */
    ...
    SSL_CTX_set_session_cache_mode(
        backend->ctx, SSL_SESS_CACHE_CLIENT | SSL_SESS_CACHE_NO_INTERNAL);
...
#endif
        sni && !SSL_set_tlsext_host_name(backend->handle, hostname))
        infof(
            data,
    ...
    if (1 != err) {
        ...
        if (SSL_ERROR_WANT_ASYNC == detail) {
            ...
            if (CURLE_SSL_CONNECT_ERROR == result && errdetail == 0) {
                ...
                failf(
                    data,
                    OSSL_PACKAGE " SSL_connect: %s in connection to %s:%ld ",
...
static void
X509V3_ext(struct Curl_easy* data, int certnum, CONST_EXTS STACK_OF(X509_EXTENSION) * exts)
{
    int i;
...
static CURLcode ossl_connect_step3(struct connectdata* conn, int sockindex)
{
    ...
    result =
        servercert(conn, connssl, (SSL_CONN_CONFIG(verifypeer) || SSL_CONN_CONFIG(verifyhost)));
...
static CURLcode
ossl_connect_common(struct connectdata* conn, int sockindex, bool nonblocking, bool* done)
{
    CURLcode result;
    struct Curl_easy* data = conn->data;
    struct ssl_connect_data* connssl = &conn->ssl[sockindex];
...
static CURLcode Curl_ossl_connect_nonblocking(struct connectdata* conn, int sockindex, bool* done)
{
...
static bool Curl_ossl_data_pending(const struct connectdata* conn, int connindex)
{
...
static ssize_t
ossl_send(struct connectdata* conn, int sockindex, const void* mem, size_t len, CURLcode* curlcode)
{
    ...
    if (rc <= 0) {
        ...
        switch (err) {
            ...
            case SSL_ERROR_SYSCALL: {
                ...
                failf(conn->data, OSSL_PACKAGE " SSL_write: %s, errno %d", error_buffer, sockerr);
        ...
        /* a true error */
        failf(
            conn->data, OSSL_PACKAGE " SSL_write: %s, errno %d", SSL_ERROR_to_str(err), SOCKERRNO);
        *curlcode = CURLE_SEND_ERROR;
        return -1;
    }
    *curlcode = CURLE_OK;
        ...
        switch (err) {
            ...
            default:
                ...
                if ((nread < 0) || sslerror) {
                    ...
                    failf(
                        conn->data, OSSL_PACKAGE " SSL_read: %s, errno %d", error_buffer, sockerr);
                ...
                if (err == SSL_ERROR_SYSCALL) {
                    ...
                    failf(
                        conn->data,
                        OSSL_PACKAGE " SSL_read: %s, errno %d"
...
static CURLcode Curl_ossl_random(struct Curl_easy* data, unsigned char* entropy, size_t length)
{
    int rc;
    if (data) {
...
static CURLcode Curl_ossl_md5sum(
...
static CURLcode Curl_ossl_sha256sum(
...
static void* Curl_ossl_get_internals(struct ssl_connect_data* connssl, CURLINFO info)
{
