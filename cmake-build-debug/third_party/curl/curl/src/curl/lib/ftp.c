...
const struct Curl_handler Curl_handler_ftp = {
    ...
    PROTOPT_DUAL | PROTOPT_CLOSEACTION | PROTOPT_NEEDSPWD | PROTOPT_NOURLQUERY |
        PROTOPT_PROXY_AS_HTTP | PROTOPT_WILDCARD /* flags */
};
...
const struct Curl_handler Curl_handler_ftps = {
    ...
    PROTOPT_SSL | PROTOPT_DUAL | PROTOPT_CLOSEACTION | PROTOPT_NEEDSPWD | PROTOPT_NOURLQUERY |
    ...
    *connected = FALSE;
    ...
    if (*connected) {
        ...
        if (*connected == FALSE) {
            Curl_expire(
                data,
                data->set.accepttimeout > 0 ? data->set.accepttimeout : DEFAULT_ACCEPT_TIMEOUT,
                0);
        }
    }
...
static const char* const ftp_state_names[] = {
    "STOP",          "WAIT220",   "AUTH", "USER",      "PASS",      "ACCT",  "PBSZ",
    "PROT",          "CCC",       "PWD",  "SYST",      "NAMEFMT",   "QUOTE", "RETR_PREQUOTE",
    "STOR_PREQUOTE", "POSTQUOTE", "CWD",  "MKD",       "MDTM",      "TYPE",  "LIST_TYPE",
    "RETR_TYPE",     "STOR_TYPE", "SIZE", "RETR_SIZE", "STOR_SIZE", "REST",  "RETR_REST",
    "PORT",          "PRET",      "PASV", "LIST",      "RETR",      "STOR",  "QUIT"};
#endif
...
typedef enum { EPRT, PORT, DONE } ftpport;

static CURLcode ftp_state_use_port(struct connectdata* conn, ftpport fcmd) /* start with this */

{
...
#ifdef ENABLE_IPV6
        size_t addrlen =
            INET6_ADDRSTRLEN > strlen(string_ftpport) ? INET6_ADDRSTRLEN : strlen(string_ftpport);
#else
        size_t addrlen =
            INET_ADDRSTRLEN > strlen(string_ftpport) ? INET_ADDRSTRLEN : strlen(string_ftpport);
#endif
    ...
    if (!host) {
        ...
        if (getsockname(conn->sock[FIRSTSOCKET], sa, &sslen)) {
            failf(
                data, "getsockname() failed: %s", Curl_strerror(SOCKERRNO, buffer, sizeof(buffer)));
            free(addr);
            return CURLE_FTP_PORT_FAILED;
        }
        switch (sa->sa_family) {
...
static CURLcode ftp_state_pasv_resp(struct connectdata* conn, int ftpcode)
{
    ...
    if ((ftpc->count1 == 0) && (ftpcode == 229)) {
        ...
        while (*str) {
            if (6 ==
                sscanf(
                    str, "%u,%u,%u,%u,%u,%u", &ip[0], &ip[1], &ip[2], &ip[3], &port[0], &port[1]))
                break;
            str++;
        }

        if (!*str || (ip[0] > 255) || (ip[1] > 255) || (ip[2] > 255) || (ip[3] > 255) ||
            (port[0] > 255) || (port[1] > 255)) {
    ...
    if (conn->bits.proxy) {
        ...
        const char* const host_name =
            conn->bits.socksproxy ? conn->socks_proxy.host.name : conn->http_proxy.host.name;
        ...
        connectport = (unsigned short)conn->port; /* we connect to the proxy's port */
...
static CURLcode ftp_state_retr(struct connectdata* conn, curl_off_t filesize)
{
    ...
    if (data->state.resume_from) {
        ...
        infof(
            data,
            "Instructs server to resume from offset %" CURL_FORMAT_CURL_OFF_T "\n",
            data->state.resume_from);
...
static CURLcode ftp_state_size_resp(struct connectdata* conn, int ftpcode, ftpstate instate)
{
    ...
    if (instate == FTP_SIZE) {
        ...
        if (-1 != filesize) {
            char clbuf[128];
            msnprintf(
                clbuf, sizeof(clbuf), "Content-Length: %" CURL_FORMAT_CURL_OFF_T "\r\n", filesize);
...
static CURLcode ftp_state_user_resp(struct connectdata* conn, int ftpcode, ftpstate instate)
{
    ...
    if ((ftpcode == 331) && (ftpc->state == FTP_USER)) {
        ...
        if (conn->data->set.str[STRING_FTP_ALTERNATIVE_TO_USER] &&
            ...
            PPSENDF(
                &conn->proto.ftpc.pp, "%s", conn->data->set.str[STRING_FTP_ALTERNATIVE_TO_USER]);
    ...
    if (ftpcode) {
        ...
        switch (ftpc->state) {
            case FTP_WAIT220:
                ...
                if (data->set.use_ssl &&
                    (!conn->ssl[FIRSTSOCKET].use || (conn->bits.proxy_ssl_connected[FIRSTSOCKET] &&
            ...
            case FTP_CWD:
                if (ftpcode / 100 != 2) {
                    ...
                    if (conn->data->set.ftp_create_missing_dirs && ftpc->cwdcount &&
                        !ftpc->count2) {
                        /* try making it */
 ...
 *
 */
static CURLcode ftp_connect(struct connectdata* conn, bool* done) /* see description above */
{
    CURLcode result;
    struct ftp_conn* ftpc = &conn->proto.ftpc;
    struct pingpong* pp = &ftpc->pp;
...
static CURLcode ftp_done(struct connectdata* conn, CURLcode status, bool premature)
{
    ...
    if (!result && (ftp->transfer == FTPTRANSFER_BODY) && ftpc->ctl_valid && pp->pending_resp &&
        !premature) {
        /*
    ...
    if (result || premature)
        ...
        if ((-1 != data->state.infilesize) &&
            (data->state.infilesize != data->req.writebytecount) && !data->set.crlf &&
            (ftp->transfer == FTPTRANSFER_BODY)) {
            failf(
                data,
                "Uploaded unaligned file size (%" CURL_FORMAT_CURL_OFF_T
        ...
        if ((-1 != data->req.size) && (data->req.size != data->req.bytecount) &&
            ...
            failf(
                data,
                "Received only partial file: %" CURL_FORMAT_CURL_OFF_T " bytes",
                data->req.bytecount);
            result = CURLE_PARTIAL_FILE;
