...
static bool init_resolve_thread(
...
static void destroy_thread_sync_data(struct thread_sync_data* tsd)
{
    if (tsd->mtx) {
...
static int init_thread_sync_data(
    struct thread_data* td, const char* hostname, int port, const struct addrinfo* hints)
{
...
static CURLcode resolver_error(struct connectdata* conn)
{
    ...
    failf(conn->data, "Could not resolve %s: %s", host_or_proxy, conn->async.hostname);

    return result;
}

static CURLcode
thread_wait_resolv(struct connectdata* conn, struct Curl_dns_entry** entry, bool report)
{
 ...
 */
CURLcode Curl_resolver_is_resolved(struct connectdata* conn, struct Curl_dns_entry** entry)
{
    ...
    *entry = NULL;
    ...
    if (done) {
        ...
        *entry = conn->async.dns;
        ...
        time_t elapsed = (time_t)Curl_timediff(Curl_now(), data->progress.t_startsingle);
        if (elapsed < 0)
            elapsed = 0;
