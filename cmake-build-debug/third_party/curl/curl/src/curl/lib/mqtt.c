...
static CURLcode mqtt_connect(struct connectdata* conn)
{
    ...
    char packet[32] = {
        ...
        0x00,
        0x3c, /* keep-alive 0 = disabled */
        0x00,
        0x00 /* payload1 length */
    };
    packet[1] = (packetlen - 2) & 0x7f;
    packet[client_id_offset - 1] = MQTT_CLIENTID_LEN;

    result = Curl_rand_hex(
        conn->data, (unsigned char*)&client_id[curl_len], MQTT_CLIENTID_LEN - curl_len + 1);
...
static CURLcode mqtt_verify_suback(struct connectdata* conn)
{
    ...
    /* verify SUBACK */
    if (readbuf[0] != ((mqtt->packetid >> 8) & 0xff) || readbuf[1] != (mqtt->packetid & 0xff) ||
        readbuf[2] != 0x00)
        result = CURLE_WEIRD_SERVER_REPLY;
