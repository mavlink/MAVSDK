...
static void pre_receive_plain(struct connectdata* conn, int num)
{
    ...
    if ((conn->handler->protocol & PROTO_FAMILY_HTTP) != 0 && conn->recv[num] == Curl_recv_plain &&
        (!psnd->buffer || bytestorecv)) {
        const int readymask = Curl_socket_check(sockfd, CURL_SOCKET_BAD, CURL_SOCKET_BAD, 0);
        if (readymask != -1 && (readymask & CURL_CSELECT_IN) != 0) {
            ...
            if (psnd->buffer) {
                ...
                recvedbytes = sread(sockfd, psnd->buffer + psnd->recv_size, bytestorecv);
                if (recvedbytes > 0)
 ...
 */
CURLcode Curl_write(
    struct connectdata* conn, curl_socket_t sockfd, const void* mem, size_t len, ssize_t* written)
{
    ssize_t bytes_written;
    CURLcode result = CURLE_OK;
...
ssize_t
Curl_send_plain(struct connectdata* conn, int num, const void* mem, size_t len, CURLcode* code)
{
    ...
    if (conn->bits.tcp_fastopen) {
        bytes_written = sendto(
            sockfd, mem, len, MSG_FASTOPEN, conn->ip_addr->ai_addr, conn->ip_addr->ai_addrlen);
    ...
    *code = CURLE_OK;
    if (-1 == bytes_written) {
        ...
        if (
            ...
            (EWOULDBLOCK == err) || (EAGAIN == err) || (EINTR == err) || (EINPROGRESS == err)
#endif
        ) {
            ...
            *code = CURLE_AGAIN;
            ...
            failf(conn->data, "Send failure: %s", Curl_strerror(err, buffer, sizeof(buffer)));
 ...
 * callbacks.
 */
static CURLcode chop_write(struct connectdata* conn, int type, char* optr, size_t olen)
{
    struct Curl_easy* data = conn->data;
    ...
    if ((type & CLIENTWRITE_HEADER) && (data->set.fwrite_header || data->set.writeheader)) {
        /*
         * Write headers to the same callback or to the especially setup
         * header callback function (added after version 7.7.1).
         */
        writeheader = data->set.fwrite_header ? data->set.fwrite_header : data->set.fwrite_func;
    }
...
CURLcode Curl_client_write(struct connectdata* conn, int type, char* ptr, size_t len)
{
    ...
    if ((type & CLIENTWRITE_BODY) && (conn->handler->protocol & PROTO_FAMILY_FTP) &&
...
/* return 0 on success */
int Curl_debug(struct Curl_easy* data, curl_infotype type, char* ptr, size_t size)
{
