...
static CURLcode pop3_perform_upgrade_tls(struct connectdata* conn)
{
    ...
    struct pop3_conn* pop3c = &conn->proto.pop3c;
    CURLcode result = Curl_ssl_connect_nonblocking(conn, FIRSTSOCKET, &pop3c->ssldone);
...
static CURLcode pop3_perform_user(struct connectdata* conn)
{
    ...
    result = Curl_pp_sendf(&conn->proto.pop3c.pp, "USER %s", conn->user ? conn->user : "");
    if (!result)
...
static CURLcode pop3_perform_apop(struct connectdata* conn)
{
    ...
    Curl_MD5_update(
        ctxt,
        (const unsigned char*)pop3c->apoptimestamp,
...
static CURLcode pop3_perform_command(struct connectdata* conn)
{
    ...
    if (pop3->id[0] != '\0')
        result = Curl_pp_sendf(
            &conn->proto.pop3c.pp,
            "%s %s",
            (pop3->custom && pop3->custom[0] != '\0' ? pop3->custom : command),
            pop3->id);
    else
        result = Curl_pp_sendf(
            &conn->proto.pop3c.pp,
...
static CURLcode
pop3_state_servergreet_resp(struct connectdata* conn, int pop3code, pop3state instate)
{
    CURLcode result = CURLE_OK;
    struct Curl_easy* data = conn->data;
    struct pop3_conn* pop3c = &conn->proto.pop3c;
        ...
        if (prev && prev >= pop3c->eob) {
            ...
            if (prev) {
                ...
                result = Curl_client_write(
                    conn, CLIENTWRITE_BODY, (char*)POP3_EOB, strip_dot ? prev - 1 : prev);
