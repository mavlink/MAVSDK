...
static void time2str(char* r, curl_off_t seconds)
{
    ...
    if (h <= CURL_OFF_T_C(99)) {
        ...
        msnprintf(
            r,
            9,
            "%2" CURL_FORMAT_CURL_OFF_T ":%02" CURL_FORMAT_CURL_OFF_T ":%02" CURL_FORMAT_CURL_OFF_T,
            h,
            m,
            s);
    } else {
        ...
        if (d <= CURL_OFF_T_C(999))
            msnprintf(r, 9, "%3" CURL_FORMAT_CURL_OFF_T "d %02" CURL_FORMAT_CURL_OFF_T "h", d, h);
        else
...
#define ONE_PETABYTE (CURL_OFF_T_C(1024) * ONE_TERABYTE)
    ...
    if (bytes < CURL_OFF_T_C(100000))
        ...
        msnprintf(
            max5,
            6,
            "%2" CURL_FORMAT_CURL_OFF_T ".%0" CURL_FORMAT_CURL_OFF_T "M",
            bytes / ONE_MEGABYTE,
        ...
        msnprintf(
            max5,
            6,
            "%2" CURL_FORMAT_CURL_OFF_T ".%0" CURL_FORMAT_CURL_OFF_T "G",
            bytes / ONE_GIGABYTE,
...
int Curl_pgrsDone(struct connectdata* conn)
{
    ...
    if (!(data->progress.flags & PGRS_HIDE) && !data->progress.callback)
...
void Curl_ratelimit(struct Curl_easy* data, struct curltime now)
{
    ...
    if (data->set.max_recv_speed > 0) {
        if (Curl_timediff(now, data->progress.dl_limit_start) >= MIN_RATE_LIMIT_PERIOD) {
            data->progress.dl_limit_start = now;
            data->progress.dl_limit_size = data->progress.downloaded;
        }
    }
    if (data->set.max_send_speed > 0) {
        if (Curl_timediff(now, data->progress.ul_limit_start) >= MIN_RATE_LIMIT_PERIOD) {
...
static bool progress_calc(struct connectdata* conn, struct curltime now)
{
    ...
    if (data->progress.lastshow != now.tv_sec) {
        ...
        countindex =
            ((data->progress.speeder_c >= CURR_TIME) ? CURR_TIME : data->progress.speeder_c) - 1;
        ...
        if (countindex) {
            ...
            checkindex =
                (data->progress.speeder_c >= CURR_TIME) ? data->progress.speeder_c % CURR_TIME : 0;
            ...
            {
                curl_off_t amount =
                    data->progress.speeder[nowindex] - data->progress.speeder[checkindex];

                if (amount > CURL_OFF_T_C(4294967) /* 0xffffffff/1000 */)
                    /* the 'amount' value is bigger than would fit in 32 bits if
                       multiplied with 1000, so we use the double math for this */
                    data->progress.current_speed =
                        (curl_off_t)((double)amount / ((double)span_ms / 1000.0));
                else
            ...
            data->progress.current_speed = data->progress.ulspeed + data->progress.dlspeed;
...
static void progress_meter(struct connectdata* conn)
{
    ...
    total_expected_transfer =
        ((data->progress.flags & PGRS_UL_SIZE_KNOWN) ? data->progress.size_ul :
                                                       data->progress.uploaded) +
        ((data->progress.flags & PGRS_DL_SIZE_KNOWN) ? data->progress.size_dl :
