...
typedef enum { TFTP_MODE_NETASCII = 0, TFTP_MODE_OCTET } tftp_mode_t;

typedef enum { TFTP_STATE_START = 0, TFTP_STATE_RX, TFTP_STATE_TX, TFTP_STATE_FIN } tftp_state_t;
...
static CURLcode tftp_set_timeouts(tftp_state_data_t* state)
{
    ...
    infof(
        ...
        (int)state->state,
        (long)(state->max_time - state->start_time),
        state->retry_time,
        state->retry_max);
...
static const char*
tftp_option_get(const char* buf, size_t len, const char** option, const char** value)
{
    size_t loc;
    ...
    while (tmp < ptr + len) {
        ...
        if (checkprefix(option, TFTP_OPTION_BLKSIZE)) {
            ...
            if (blksize > TFTP_BLKSIZE_MAX) {
                 ...
                 * requests */
                failf(data, "%s (%ld)", "server requested blksize larger than allocated", blksize);
                return CURLE_TFTP_ILLEGAL;
            }
...
static CURLcode
tftp_option_add(tftp_state_data_t* state, size_t* csize, char* buf, const char* option)
{
    ...
    switch (event) {
        ...
        case TFTP_EVENT_TIMEOUT: /* Resend the first packet out */
            ...
            result = Curl_urldecode(
                data, &state->conn->data->state.up.path[1], 0, &filename, NULL, FALSE);
            if (result)
                return result;
            ...
            if (!data->set.tftp_no_options) {
                ...
                if (data->set.upload && (data->state.infilesize != -1))
                    msnprintf(buf, sizeof(buf), "%" CURL_FORMAT_CURL_OFF_T, data->state.infilesize);
                else
                    strcpy(buf, "0"); /* the destination is large enough */

                result = tftp_option_add(
                    state, &sbytes, (char*)state->spacket.data + sbytes, TFTP_OPTION_TSIZE);
                if (result == CURLE_OK)
                    result =
                        tftp_option_add(state, &sbytes, (char*)state->spacket.data + sbytes, buf);

                /* add blksize option */
                msnprintf(buf, sizeof(buf), "%d", state->requested_blksize);
                if (result == CURLE_OK)
                    result = tftp_option_add(
                        state, &sbytes, (char*)state->spacket.data + sbytes, TFTP_OPTION_BLKSIZE);
                if (result == CURLE_OK)
                    result =
                        tftp_option_add(state, &sbytes, (char*)state->spacket.data + sbytes, buf);
                ...
                if (result == CURLE_OK)
                    result = tftp_option_add(
                        state, &sbytes, (char*)state->spacket.data + sbytes, TFTP_OPTION_INTERVAL);
                if (result == CURLE_OK)
                    result =
                        tftp_option_add(state, &sbytes, (char*)state->spacket.data + sbytes, buf);
        ...
        case TFTP_EVENT_OACK:
            if (event == TFTP_EVENT_ACK) {
                ...
                if (rblock != state->block &&
                    ...
                    if (state->retries > state->retry_max) {
                        failf(data, "tftp_tx: giving up waiting for block %d ack", state->block);
...
static CURLcode tftp_connect(struct connectdata* conn, bool* done)
{
    ...
    if (!conn->bits.bound) {
         ...
         */
        int rc =
            bind(state->sockfd, (struct sockaddr*)&state->local_addr, conn->ip_addr->ai_addrlen);
        if (rc) {
            char buffer[STRERROR_LEN];
            failf(
                conn->data, "bind() failed; %s", Curl_strerror(SOCKERRNO, buffer, sizeof(buffer)));
        ...
        switch (state->event) {
            case TFTP_EVENT_DATA:
                ...
                if (state->rbytes > 4 &&
                    ...
                    result = Curl_client_write(
                        conn, CLIENTWRITE_BODY, (char*)state->rpacket.data + 4, state->rbytes - 4);
                    if (result) {
            ...
            case TFTP_EVENT_OACK:
                result = tftp_parse_option_ack(
                    state, (const char*)state->rpacket.data + 2, state->rbytes - 2);
                if (result)
                    return result;
                break;
            case TFTP_EVENT_RRQ:
