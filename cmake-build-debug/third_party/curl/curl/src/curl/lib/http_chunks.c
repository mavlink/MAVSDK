 ...
 */
CHUNKcode Curl_httpchunk_read(
    struct connectdata* conn, char* datap, ssize_t datalen, ssize_t* wrotep, CURLcode* extrap)
{
            ...
            case CHUNK_TRAILER:
                if ((*datap == 0x0d) || (*datap == 0x0a)) {
                    ...
                    if (conn->trlPos) {
                        ...
                        result = Curl_convert_from_network(conn->data, conn->trailer, conn->trlPos);
                        if (result)
                            /* Curl_convert_from_network calls failf if unsuccessful */
                        ...
                        if (!data->set.http_te_skip) {
                            result = Curl_client_write(
                                conn, CLIENTWRITE_HEADER, conn->trailer, conn->trlPos);
                            if (result) {
                                *extrap = result;
                                return CHUNKE_PASSTHRU_ERROR;
                            }
