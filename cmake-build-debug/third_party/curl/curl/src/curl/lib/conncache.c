...
static void bundle_add_conn(struct connectbundle* cb_ptr, struct connectdata* conn)
{
...
int Curl_conncache_init(struct conncache* connc, int size)
{
    ...
    rc = Curl_hash_init(
        &connc->hash, size, Curl_hash_str, Curl_str_key_compare, free_bundle_hash_entry);
    if (rc)
...
Curl_conncache_extract_bundle(struct Curl_easy* data, struct connectbundle* bundle)
{
    ...
    if (conn_candidate) {
        ...
        DEBUGF(
            infof(data, "The cache now contains %zu members\n", data->state.conn_cache->num_conn));
