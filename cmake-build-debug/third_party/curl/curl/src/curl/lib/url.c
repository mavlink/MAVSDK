...
static const struct Curl_handler* const protocols[] = {
    ...
    &Curl_handler_ldap,
#if !defined(CURL_DISABLE_LDAPS) && ((defined(USE_OPENLDAP) && defined(USE_SSL)) || \
...
#ifdef USE_LIBRTMP
    &Curl_handler_rtmp,        &Curl_handler_rtmpt, &Curl_handler_rtmpe,
    &Curl_handler_rtmpte,      &Curl_handler_rtmps, &Curl_handler_rtmpts,
#endif
...
static void conn_reset_postponed_data(struct connectdata* conn, int num)
{
    ...
    if (psnd->buffer) {
        ...
        DEBUGASSERT(
            psnd->recv_size ? (psnd->recv_processed < psnd->recv_size) :
...
{
    struct curltime now = Curl_now();
    timediff_t elapsed = Curl_timediff(now, data->state.conn_cache->last_cleanup);
...
#ifdef USE_NTLM
    bool wantNTLMhttp =
        ((data->state.authhost.want & (CURLAUTH_NTLM | CURLAUTH_NTLM_WB)) &&
         (needle->handler->protocol & PROTO_FAMILY_HTTP));
    bool wantProxyNTLMhttp =
        (needle->bits.proxy_user_passwd &&
         ((data->state.authproxy.want & (CURLAUTH_NTLM | CURLAUTH_NTLM_WB)) &&
          (needle->handler->protocol & PROTO_FAMILY_HTTP)));
#endif
    ...
    if (bundle) {
        ...
        while (curr) {
            ...
            if ((needle->handler->flags & PROTOPT_SSL) != (check->handler->flags & PROTOPT_SSL))
                ...
                if (get_protocol_family(check->handler->protocol) != needle->handler->protocol ||
                    !check->tls_upgraded)
            ...
            if (needle->bits.socksproxy &&
                !socks_proxy_info_matches(&needle->socks_proxy, &check->socks_proxy))
                continue;
                ...
                if (needle->http_proxy.proxytype == CURLPROXY_HTTPS) {
                    ...
                    if (needle->handler->flags & PROTOPT_SSL) {
                        ...
                        if (!Curl_ssl_config_matches(
                                &needle->proxy_ssl_config, &check->proxy_ssl_config))
                            continue;
            ...
            if (!needle->bits.httpproxy || (needle->handler->flags & PROTOPT_SSL) ||
                ...
                if ((strcasecompare(needle->handler->scheme, check->handler->scheme) ||
                     (get_protocol_family(check->handler->protocol) == needle->handler->protocol &&
                      check->tls_upgraded)) &&
                    (!needle->bits.conn_to_host ||
                     strcasecompare(needle->conn_to_host.name, check->conn_to_host.name)) &&
                    (!needle->bits.conn_to_port || needle->conn_to_port == check->conn_to_port) &&
            ...
            if (match) {
                ...
                if (wantNTLMhttp || wantProxyNTLMhttp) {
                    ...
                    if ((wantNTLMhttp && (check->http_ntlm_state != NTLMSTATE_NONE)) ||
                        (wantProxyNTLMhttp && (check->proxy_ntlm_state != NTLMSTATE_NONE))) {
                ...
                if (canmultiplex) {
                    ...
                    if (check->bits.multiplex) {
                        ...
                        if (multiplexed >= httpc->settings.max_concurrent_streams) {
                            infof(
                                data, "MAX_CONCURRENT_STREAMS reached, skip (%zu)\n", multiplexed);
                            continue;
                        } else if (
                            multiplexed >= Curl_multi_max_concurrent_streams(needle->data->multi)) {
                            infof(
                                data,
                                "client side MAX_CONCURRENT_STREAMS reached"
                                ", skip (%zu)\n",
                                multiplexed);
                            continue;
                        }
    ...
    if (conn->data->set.verbose)
        infof(
            ...
            conn->bits.socksproxy ?
                ...
                conn->bits.conn_to_host ? conn->conn_to_host.dispname : conn->host.dispname,
            conn->ip_addr_str,
            conn->port,
            conn->connection_id);
}
...
static struct connectdata* allocate_conn(struct Curl_easy* data)
{
    ...
    conn->bits.httpproxy = (conn->bits.proxy && (conn->http_proxy.proxytype == CURLPROXY_HTTP ||
...
static CURLcode parseurlandfillconn(struct Curl_easy* data, struct connectdata* conn)
{
    ...
    uc = curl_url_get(uh, CURLUPART_USER, &data->state.up.user, CURLU_URLDECODE);
    if (!uc) {
    ...
    uc = curl_url_get(uh, CURLUPART_PASSWORD, &data->state.up.password, CURLU_URLDECODE);
    if (!uc) {
    ...
    if (no_proxy && no_proxy[0]) {
        ...
        for (tok_start = 0; tok_start < no_proxy_len; tok_start = tok_end + 1) {
            ...
            for (tok_end = tok_start;
                 tok_end < no_proxy_len && strchr(separator, no_proxy[tok_end]) == NULL;
                 ++tok_end)
 ...
 */
static CURLcode
parse_proxy(struct Curl_easy* data, struct connectdata* conn, char* proxy, curl_proxytype proxytype)
{
    char* portptr = NULL;
    long port = -1;
    char* proxyuser = NULL;
    ...
    sockstype = proxytype == CURLPROXY_SOCKS5_HOSTNAME || proxytype == CURLPROXY_SOCKS5 ||
                proxytype == CURLPROXY_SOCKS4A || proxytype == CURLPROXY_SOCKS4;
...
static CURLcode create_conn_helper_init_proxy(struct connectdata* conn)
{
    ...
    if (check_noproxy(
            conn->host.name,
            data->set.str[STRING_NOPROXY] ? data->set.str[STRING_NOPROXY] : no_proxy)) {
        Curl_safefree(proxy);
...
CURLcode Curl_parse_login_details(
    const char* login, const size_t len, char** userp, char** passwdp, char** optionsp)
{
    CURLcode result = CURLE_OK;
    char* ubuf = NULL;
    char* pbuf = NULL;
    ...
    ulen =
        (psep ? (size_t)(osep && psep > osep ? osep - login : psep - login) :
                (osep ? (size_t)(osep - login) : len));
    plen =
        (psep ? (osep && osep > psep ? (size_t)(osep - psep) : (size_t)(login + len - psep)) - 1 :
                0);
    olen =
        (osep ? (psep && psep > osep ? (size_t)(psep - osep) : (size_t)(login + len - osep)) - 1 :
                0);
 ...
 */
static CURLcode override_login(
    struct Curl_easy* data, struct connectdata* conn, char** userp, char** passwdp, char** optionsp)
{
...
static CURLcode parse_connect_to_host_port(
    struct Curl_easy* data, const char* host, char** hostname_result, int* port_result)
{
    char* host_dup;
    char* hostptr;
    char* host_portno;
    ...
    *port_result = -1;
    ...
    if (*hostptr == '[') {
        ...
        if (*ptr == '%') {
            ...
            while (*ptr && (ISALPHA(*ptr) || ISXDIGIT(*ptr) || (*ptr == '-') || (*ptr == '.') ||
                            (*ptr == '_') || (*ptr == '~')))
                ptr++;
        }
        if (*ptr == ']')
    ...
    int* port_result)
{
    ...
    *port_result = -1;
    ...
    if (*ptr == ':') {
        ...
        char* hostname_to_match = aprintf(
            "%s%s%s",
            conn->bits.ipv6_ip ? "[" : "",
            conn->host.name,
            conn->bits.ipv6_ip ? "]" : "");
 ...
 */
static CURLcode parse_connect_to_slist(
    struct Curl_easy* data, struct connectdata* conn, struct curl_slist* conn_to_host)
{
    CURLcode result = CURLE_OK;
    char* host = NULL;
    int port = -1;
        ...
        *async = FALSE;
        ...
        if (conn->unix_domain_socket) {
            ...
            rc = Curl_resolv_timeout(
                conn, conn->hostname_resolve, (int)conn->port, &hostaddr, timeout_ms);
            ...
            struct hostname* const host =
                conn->bits.socksproxy ? &conn->socks_proxy.host : &conn->http_proxy.host;
        ...
        *in_connect = conn;
        ...
        infof(
            ...
            conn->socks_proxy.host.name ?
                conn->socks_proxy.host.dispname :
                conn->http_proxy.host.name ? conn->http_proxy.host.dispname : conn->host.dispname);
    } else {
        ...
        if (connections_available && (max_total_connections > 0) &&
