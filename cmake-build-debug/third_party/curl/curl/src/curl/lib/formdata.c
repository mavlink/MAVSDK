...
static CURLFORMcode
FormAdd(struct curl_httppost** httppost, struct curl_httppost** last_post, va_list params)
{
    ...
    while (return_value == CURL_FORMADD_OK) {
        ...
        switch (option) {
            ...
            case CURLFORM_CONTENTHEADER: {
                ...
                struct curl_slist* list = array_state ? (struct curl_slist*)(void*)array_value :
    ...
    if (CURL_FORMADD_OK == return_value) {
        ...
        for (form = first_form; form != NULL; form = form->more) {
            if (((!form->name || !form->value) && !post) ||
                ((form->contentslength) && (form->flags & HTTPPOST_FILENAME)) ||
                ((form->flags & HTTPPOST_FILENAME) && (form->flags & HTTPPOST_PTRCONTENTS)) ||

                ((!form->buffer) && (form->flags & HTTPPOST_BUFFER) &&
            ...
            if (((form->flags & HTTPPOST_FILENAME) || (form->flags & HTTPPOST_BUFFER)) &&
                !form->contenttype) {
            ...
            if (!(form->flags & HTTPPOST_PTRNAME) && (form == first_form)) {
                ...
                if (form->name) {
                    ...
                    form->name = Curl_memdup(
                        form->name, form->namelength ? form->namelength : strlen(form->name) + 1);
                }
                if (!form->name) {
                    return_value = CURL_FORMADD_MEMORY;
                    break;
            ...
            if (!(form->flags & (HTTPPOST_FILENAME | HTTPPOST_READFILE | HTTPPOST_PTRCONTENTS |
                                 HTTPPOST_PTRBUFFER | HTTPPOST_CALLBACK)) &&
                form->value) {
...
int curl_formget(struct curl_httppost* form, void* arg, curl_formget_callback append)
{
    ...
    if (!result)
        result =
            Curl_mime_prepare_headers(&toppart, "multipart/form-data", NULL, MIMESTRATEGY_FORM);
    ...
    for (; !result && post; post = post->next) {
        ...
        for (file = post; !result && file; file = file->more) {
                ...
                if (post->flags & (HTTPPOST_FILENAME | HTTPPOST_READFILE)) {
                    if (!strcmp(file->contents, "-")) {
                        ...
                        result = curl_mime_data_cb(
                            ...
                            CURLX_FUNCTION_CAST(curl_seek_callback, fseek),
                            NULL,
                            (void*)stdin);
                    } else
                    ...
                    result = curl_mime_data(
                        part, post->buffer, post->bufferlength ? post->bufferlength : -1);
                else if (post->flags & HTTPPOST_CALLBACK)
                    /* the contents should be read with the callback and the size is set
                       with the contentslength */
                    result = curl_mime_data_cb(part, clen, fread_func, NULL, NULL, post->userp);
                else {
            ...
            if (!result && post->showfilename)
                if (post->more ||
                    (post->flags & (HTTPPOST_FILENAME | HTTPPOST_BUFFER | HTTPPOST_CALLBACK)))
