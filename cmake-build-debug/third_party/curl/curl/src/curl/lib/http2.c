...
static ssize_t
http2_recv(struct connectdata* conn, int sockindex, char* mem, size_t len, CURLcode* err);
static bool http2_connisdead(struct connectdata* conn);
static int h2_session_send(struct Curl_easy* data, nghttp2_session* h2);
static int
h2_process_pending_input(struct connectdata* conn, struct http_conn* httpc, CURLcode* err);
...
static unsigned int http2_conncheck(struct connectdata* check, unsigned int checks_to_perform)
{
    ...
    if (checks_to_perform & CONNCHECK_KEEPALIVE) {
        ...
        if (elapsed > check->upkeep_interval_ms) {
            ...
            if (!rc) {
                ...
                failf(
                    check->data, "nghttp2_submit_ping() failed: %s(%d)", nghttp2_strerror(rc), rc);
            }
    ...
    if (send_frames) {
        ...
        if (rc)
            failf(check->data, "nghttp2_session_send() failed: %s(%d)", nghttp2_strerror(rc), rc);
    }
 ...
 */
static ssize_t
send_callback(nghttp2_session* h2, const uint8_t* data, size_t length, int flags, void* userp)
{
...
char* curl_pushheader_byname(struct curl_pushheaders* h, const char* header)
{
    ...
    if (!h || !GOOD_EASY_HANDLE(h->data) || !header || !header[0] || !strcmp(header, ":") ||
        strchr(header + 1, ':'))
        return NULL;
    else {
        struct HTTP* stream = h->data->req.protop;
...
static int
push_promise(struct Curl_easy* data, struct connectdata* conn, const nghttp2_push_promise* frame)
{
    int rv;
    ...
    if (data->multi->push_cb) {
        ...
        rv = data->multi->push_cb(
            data, newhandle, stream->push_headers_used, &heads, data->multi->push_userp);
    ...
    void* userp)
{
    ...
    if (nread < len) {
        ...
        H2BUGF(infof(
            data_s,
            "NGHTTP2_ERR_PAUSE - %zu bytes out of buffer"
            ", stream %u\n",
            len - nread,
            stream_id));
    ...
    void* userp)
{
    ...
    if (frame->hd.type == NGHTTP2_PUSH_PROMISE) {
        ...
        if (!stream->push_headers) {
            ...
            headp =
                Curl_saferealloc(stream->push_headers, stream->push_headers_alloc * sizeof(char*));
            if (!headp) {
                stream->push_headers = NULL;
                return NGHTTP2_ERR_TEMPORAL_CALLBACK_FAILURE;
            }
...
void Curl_http2_done(struct Curl_easy* data, bool premature)
{
    ...
    if (premature) {
        ...
        if (!nghttp2_submit_rst_stream(
                httpc->h2, NGHTTP2_FLAG_NONE, http->stream_id, NGHTTP2_STREAM_CLOSED))
 ...
 */
CURLcode Curl_http2_request_upgrade(Curl_send_buffer* req, struct connectdata* conn)
{
    CURLcode result;
    ssize_t binlen;
    char* base64;
    ...
    binlen = nghttp2_pack_settings_payload(
        binsettings, H2_BINSETTINGS_LEN, httpc->local_settings, httpc->local_settings_num);
    if (!binlen) {
...
static ssize_t http2_handle_stream_close(
    struct connectdata* conn, struct Curl_easy* data, struct HTTP* stream, CURLcode* err)
{
    ...
    if (httpc->error_code == NGHTTP2_REFUSED_STREAM) {
        H2BUGF(infof(
            data, "REFUSED_STREAM (%d), try again on a new connection!\n", stream->stream_id));
        ...
        *err = CURLE_RECV_ERROR; /* trigger Curl_retry_request() later */
        ...
        failf(
            data,
            "HTTP/2 stream %d was not closed cleanly: %s (err %u)",
            stream->stream_id,
            nghttp2_strerror(httpc->error_code),
            httpc->error_code);
    ...
    if (!stream->bodystarted) {
        failf(
            data,
            "HTTP/2 stream %d was closed cleanly, but before getting "
...
{
    struct HTTP* depstream =
        (data->set.stream_depends_on ? data->set.stream_depends_on->req.protop : NULL);
    int32_t depstream_id = depstream ? depstream->stream_id : 0;
    nghttp2_priority_spec_init(
        pri_spec, depstream_id, data->set.stream_weight, data->set.stream_depends_e);
...
http2_recv(struct connectdata* conn, int sockindex, char* mem, size_t len, CURLcode* err)
{
    ...
    if (stream->bodystarted && stream->nread_header_recvbuf < stream->header_recvbuf->size_used) {
        /* If there is body data pending for this stream to return, do that */
        size_t left = stream->header_recvbuf->size_used - stream->nread_header_recvbuf;
        size_t ncopy = CURLMIN(len, left);
        memcpy(mem, stream->header_recvbuf->buffer + stream->nread_header_recvbuf, ncopy);
        stream->nread_header_recvbuf += ncopy;
    ...
    H2BUGF(infof(
        data,
        "http2_recv: easy %p (stream %u) win %u/%u\n",
        data,
        stream->stream_id,
...
static header_instruction
inspect_header(const char* name, size_t namelen, const char* value, size_t valuelen)
{
    switch (namelen) {
        case 2:
        ...
        case 7:
            return strncasecompare("upgrade", name, namelen) ? HEADERINST_IGNORE :
                                                               HEADERINST_FORWARD;
        case 10:
            return (strncasecompare("connection", name, namelen) ||
                    strncasecompare("keep-alive", name, namelen)) ?
                       ...
                       HEADERINST_FORWARD;
        case 16:
            return strncasecompare("proxy-connection", name, namelen) ? HEADERINST_IGNORE :
                                                                        HEADERINST_FORWARD;
        case 17:
            return strncasecompare("transfer-encoding", name, namelen) ? HEADERINST_IGNORE :
                                                                         HEADERINST_FORWARD;
        default:
            return HEADERINST_FORWARD;
    }
...
static ssize_t
http2_send(struct connectdata* conn, int sockindex, const void* mem, size_t len, CURLcode* err)
{
    /*
...
#define MAX_ACC 60000 /* <64KB to account for some overhead */
    {
        ...
        if (acc > MAX_ACC) {
            infof(
                conn->data,
                "http2_send: Warning: The cumulative length of all "
    ...
    switch (conn->data->set.httpreq) {
        ...
        case HTTPREQ_PUT:
            ...
            break;
        default:
            stream_id = nghttp2_submit_request(h2, &pri_spec, nva, nheader, NULL, conn->data);
    }
...
CURLcode Curl_http2_switched(struct connectdata* conn, const char* mem, size_t nread)
{
    ...
    if (conn->data->req.upgr101 == UPGR101_RECEIVED) {
        ...
        rv = nghttp2_submit_settings(
            httpc->h2, NGHTTP2_FLAG_NONE, httpc->local_settings, httpc->local_settings_num);
        if (rv != 0) {
    ...
    rv = nghttp2_session_set_local_window_size(
        httpc->h2, NGHTTP2_FLAG_NONE, 0, HTTP2_HUGE_WINDOW_SIZE);
    if (rv != 0) {
        failf(
            data,
    ...
    if (H2_BUFSIZE < nread) {
        failf(
            data,
            "connection buffer size is too small to store data following "
...
CURLcode Curl_http2_stream_pause(struct Curl_easy* data, bool pause)
{
    ...
    if (!data->conn->proto.httpc.h2)
        ...
        int rv = nghttp2_session_set_local_window_size(
            httpc->h2, NGHTTP2_FLAG_NONE, stream->stream_id, window);
        if (rv) {
            failf(
                data,
                "nghttp2_session_set_local_window_size() failed: %s(%d)",
        ...
        DEBUGF(
            infof(data, "Set HTTP/2 window size to %u for stream %u\n", window, stream->stream_id));
...
#ifdef DEBUGBUILD
        {
            ...
            uint32_t window2 =
                nghttp2_session_get_stream_local_window_size(httpc->h2, stream->stream_id);
            DEBUGF(infof(
                data, "HTTP/2 window size is now %u for stream %u\n", window2, stream->stream_id));
        }
#endif
    }
#endif
