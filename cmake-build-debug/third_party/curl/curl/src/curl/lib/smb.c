...
{
    return ((curl_off_t)smb_swap32((unsigned int)x) << 32) | smb_swap32((unsigned int)(x >> 32));
}
...
static void conn_state(struct connectdata* conn, enum smb_conn_state newstate)
{
    ...
    static const char* const names[] = {
        "SMB_NOT_CONNECTED", "SMB_CONNECTING", "SMB_NEGOTIATE", "SMB_SETUP", "SMB_CONNECTED",
        /* LAST */
    };
...
static CURLcode smb_recv_message(struct connectdata* conn, void** msg)
{
    ...
    nbt_size =
        Curl_read16_be((const unsigned char*)(buf + sizeof(unsigned short))) + sizeof(unsigned int);
    ...
    if (nbt_size >= msg_size + 1) {
        ...
        if (nbt_size >= msg_size + sizeof(unsigned short)) {
            /* Add the byte count */
            msg_size +=
                sizeof(unsigned short) + Curl_read16_le((const unsigned char*)&buf[msg_size]);
...
static void
smb_format_message(struct connectdata* conn, struct smb_header* h, unsigned char cmd, size_t len)
{
    struct smb_conn* smbc = &conn->proto.smbc;
    struct smb_request* req = conn->data->req.protop;
    unsigned int pid;
    ...
    memset(h, 0, sizeof(*h));
    h->nbt_length = htons((unsigned short)(sizeof(*h) - sizeof(unsigned int) + len));
...
static CURLcode smb_send(struct connectdata* conn, ssize_t len, size_t upload_size)
{
    struct smb_conn* smbc = &conn->proto.smbc;
    ssize_t bytes_written;
    CURLcode result;

    result = Curl_write(conn, FIRSTSOCKET, conn->data->state.ulbuf, len, &bytes_written);
    if (result)
        return result;
...
static CURLcode smb_flush(struct connectdata* conn)
{
    ...
    result =
        Curl_write(conn, FIRSTSOCKET, conn->data->state.ulbuf + smbc->sent, len, &bytes_written);
    if (result)
        return result;
...
static CURLcode
smb_send_message(struct connectdata* conn, unsigned char cmd, const void* msg, size_t msg_len)
{
    CURLcode result = Curl_get_upload_buffer(conn->data);
    if (result)
        return result;
    smb_format_message(conn, (struct smb_header*)conn->data->state.ulbuf, cmd, msg_len);
    memcpy(conn->data->state.ulbuf + sizeof(struct smb_header), msg, msg_len);
...
static CURLcode smb_send_setup(struct connectdata* conn)
{
    ...
    return smb_send_message(
        conn, SMB_COM_SETUP_ANDX, &msg, sizeof(msg) - sizeof(msg.bytes) + byte_count);
}
...
static CURLcode smb_send_tree_connect(struct connectdata* conn)
{
    ...
    return smb_send_message(
        conn, SMB_COM_TREE_CONNECT_ANDX, &msg, sizeof(msg) - sizeof(msg.bytes) + byte_count);
}
...
static CURLcode smb_send_open(struct connectdata* conn)
{
    ...
    return smb_send_message(
        conn, SMB_COM_NT_CREATE_ANDX, &msg, sizeof(msg) - sizeof(msg.bytes) + byte_count);
}
...
static CURLcode smb_send_write(struct connectdata* conn)
{
    ...
    smb_format_message(
        conn, &msg->h, SMB_COM_WRITE_ANDX, sizeof(*msg) - sizeof(msg->h) + (size_t)upload_size);
...
static CURLcode smb_connection_state(struct connectdata* conn, bool* done)
{
    ...
    switch (smbc->state) {
        case SMB_NEGOTIATE:
            if ((smbc->got < sizeof(*nrsp) + sizeof(smbc->challenge) - 1) || h->status) {
...
static CURLcode smb_request_state(struct connectdata* conn, bool* done)
{
    ...
    switch (req->state) {
        ...
        case SMB_DOWNLOAD:
            ...
            len = Curl_read16_le(((const unsigned char*)msg) + sizeof(struct smb_header) + 11);
            off = Curl_read16_le(((const unsigned char*)msg) + sizeof(struct smb_header) + 13);
            if (len > 0) {
                if (off + sizeof(unsigned int) + len > smbc->got) {
                    ...
                    result = Curl_client_write(
                        conn, CLIENTWRITE_BODY, (char*)msg + off + sizeof(unsigned int), len);
                if (result) {
                    req->result = result;
                    next_state = SMB_CLOSE;
                    break;
        ...
        case SMB_UPLOAD:
            ...
            len = Curl_read16_le(((const unsigned char*)msg) + sizeof(struct smb_header) + 5);
            conn->data->req.bytecount += len;
            conn->data->req.offset += len;
...
static CURLcode smb_parse_url_path(struct connectdata* conn)
{
    ...
    /* URL decode the path */
    CURLcode result = Curl_urldecode(data, data->state.up.path, 0, &path, NULL, TRUE);
    if (result)
        return result;
