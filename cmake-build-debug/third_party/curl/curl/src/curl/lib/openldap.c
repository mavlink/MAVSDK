...
static CURLcode ldap_connect(struct connectdata* conn, bool* done)
{
    ...
    if (conn->handler->flags & PROTOPT_SSL)
        *ptr++ = 's';
    msnprintf(
        ptr, sizeof(hosturl) - (ptr - hosturl), "://%s:%d", conn->host.name, conn->remote_port);
    ...
    if (rc) {
        failf(data, "LDAP local: Cannot connect to %s, %s", hosturl, ldap_err2string(rc));
...
static CURLcode ldap_connecting(struct connectdata* conn, bool* done)
{
    ...
    if (conn->handler->flags & PROTOPT_SSL) {
        ...
        if (!li->ssldone) {
            CURLcode result = Curl_ssl_connect_nonblocking(conn, FIRSTSOCKET, &li->ssldone);
...
retry:
    if (!li->didbind) {
        ...
        rc = ldap_sasl_bind(li->ld, binddn, LDAP_SASL_SIMPLE, &passwd, NULL, NULL, &li->msgid);
        if (rc)
    ...
    if (err) {
        failf(data, "LDAP remote: bind failed %s %s", ldap_err2string(rc), info ? info : "");
        if (info)
            ldap_memfree(info);
...
static CURLcode ldap_done(struct connectdata* conn, CURLcode res, bool premature)
{
...
static ssize_t
ldap_recv(struct connectdata* conn, int sockindex, char* buf, size_t len, CURLcode* err)
{
    struct ldapconninfo* li = conn->proto.ldapc;
    struct Curl_easy* data = conn->data;
    ...
    *err = CURLE_AGAIN;
    ...
    for (ent = ldap_first_message(li->ld, msg); ent; ent = ldap_next_message(li->ld, ent)) {
        ...
        writeerr = Curl_client_write(conn, CLIENTWRITE_BODY, (char*)bv.bv_val, bv.bv_len);
        if (writeerr) {
            *err = writeerr;
            return -1;
        }
        ...
        for (rc = ldap_get_attribute_ber(li->ld, ent, ber, &bv, &bvals); rc == LDAP_SUCCESS;
            ...
            for (i = 0; bvals[i].bv_val != NULL; i++) {
                ...
                if (binary || binval) {
                    ...
                    CURLcode error = Curl_base64_encode(
                        data, bvals[i].bv_val, bvals[i].bv_len, &val_b64, &val_b64_sz);
                    if (error) {
                        ber_memfree(bvals);
                        ber_free(ber, 0);
                        ldap_msgfree(msg);
                    ...
                    writeerr = Curl_client_write(conn, CLIENTWRITE_BODY, (char*)": ", 2);
                    if (writeerr) {
                        *err = writeerr;
                        return -1;
                    }
                    ...
                    if (val_b64_sz > 0) {
                        writeerr = Curl_client_write(conn, CLIENTWRITE_BODY, val_b64, val_b64_sz);
                        if (writeerr) {
                            *err = writeerr;
                            return -1;
                        }
                    ...
                    writeerr =
                        Curl_client_write(conn, CLIENTWRITE_BODY, bvals[i].bv_val, bvals[i].bv_len);
                    if (writeerr) {
                        *err = writeerr;
                        return -1;
                    }
