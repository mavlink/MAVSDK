...
static CURLcode
CONNECT(struct connectdata* conn, int sockindex, const char* hostname, int remote_port)
{
    int subversion = 0;
    ...
    do {
        ...
        if (TUNNEL_INIT == s->tunnel_state) {
            ...
            if (!result) {
                ...
                hostheader = /* host:port with IPv6 support */
                    aprintf(
                        "%s%s%s:%d", ipv6_ip ? "[" : "", hostname, ipv6_ip ? "]" : "", remote_port);
                if (!hostheader) {
                    Curl_add_buffer_free(&req_buffer);
                    return CURLE_OUT_OF_MEMORY;
                }
                ...
                if (!Curl_checkProxyheaders(conn, "User-Agent") && data->set.str[STRING_USERAGENT])
                ...
                if (!result) {
                    ...
                    result = Curl_add_buffer_send(
                        &req_buffer, conn, &data->info.request_size, 0, sockindex);
                }
                req_buffer = NULL;
                if (result)
        ...
        { /* READING RESPONSE PHASE */
            ...
            while (s->keepon) {
                ...
                result = Curl_convert_from_network(data, s->line_start, (size_t)s->perline);
                ...
                if (data->set.verbose)
                    Curl_debug(data, CURLINFO_HEADER_IN, s->line_start, (size_t)s->perline);
                ...
                if (!data->set.suppress_connect_headers) {
                    ...
                    result = Curl_client_write(conn, writetype, s->line_start, s->perline);
                    if (result)
                        return result;
                }
                ...
                if (('\r' == s->line_start[0]) || ('\n' == s->line_start[0])) {
                    ...
                    if ((407 == k->httpcode) && !data->state.authproblem) {
                        ...
                        if (s->cl) {
                            infof(
                                data,
                                "Ignore %" CURL_FORMAT_CURL_OFF_T " bytes of response-body\n",
                                s->cl);
                            ...
                            /* now parse the chunked piece of data so that we can
                               properly tell when the stream ends */
                            r = Curl_httpchunk_read(conn, s->line_start + 1, 1, &gotbytes, &extra);
                            if (r == CHUNKE_STOP) {
                                /* we're done reading chunks! */
                                infof(data, "chunk reading DONE\n");
                                s->keepon = FALSE;
                ...
                if ((checkprefix("WWW-Authenticate:", s->line_start) && (401 == k->httpcode)) ||
                    (checkprefix("Proxy-authenticate:", s->line_start) && (407 == k->httpcode))) {
                    ...
                    if (k->httpcode / 100 == 2) {
                        ...
                        (void)curlx_strtoofft(
                            s->line_start + strlen("Content-Length:"), NULL, 10, &s->cl);
                    }
                    ...
                    if (k->httpcode / 100 == 2) {
                        ...
                        infof(
                            data,
                            "Ignoring Transfer-Encoding in "
                            "CONNECT %03d response\n",
                            k->httpcode);
                    } else if (Curl_compareheader(s->line_start, "Transfer-Encoding:", "chunked")) {
                        infof(data, "CONNECT responded chunked\n");
                        s->chunked_encoding = TRUE;
                        /* init our chunky engine */
                        Curl_httpchunk_init(conn);
                    }
                } else if (Curl_compareheader(s->line_start, "Proxy-Connection:", "close"))
                    s->close_connection = TRUE;
                else if (2 == sscanf(s->line_start, "HTTP/1.%d %d", &subversion, &k->httpcode)) {
...
CURLcode
Curl_proxyCONNECT(struct connectdata* conn, int sockindex, const char* hostname, int remote_port)
{
    CURLcode result;
