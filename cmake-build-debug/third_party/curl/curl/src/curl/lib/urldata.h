...
typedef enum { GSS_AUTHNONE, GSS_AUTHRECV, GSS_AUTHSENT, GSS_AUTHDONE, GSS_AUTHSUCC } curlnegotiate;
...
struct Curl_handler {
    ...
    int (*proto_getsock)(struct connectdata* conn, curl_socket_t* socks);

    /* Called from the multi interface during the DOING phase, and it should
       then return a proper fd set */
    int (*doing_getsock)(struct connectdata* conn, curl_socket_t* socks);

    /* Called from the multi interface during the DO_MORE phase, and it should
       then return a proper fd set */
    int (*domore_getsock)(struct connectdata* conn, curl_socket_t* socks);

    /* Called from the multi interface during the DO_DONE, PERFORM and
       WAITPERFORM phases, and it should then return a proper fd set. Not setting
       this will make libcurl use the generic default one. */
    int (*perform_getsock)(const struct connectdata* conn, curl_socket_t* socks);
...
#define CONN_INUSE(c) ((c)->easyq.size)
    ...
    enum { TRNSPRT_TCP = 3, TRNSPRT_UDP = 4, TRNSPRT_QUIC = 5 } transport;
...
struct UserDefined {
    ...
    enum CURL_NETRC_OPTION use_netrc; /* defined in include/curl.h */
