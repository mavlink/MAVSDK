...
static struct altsvc* altsvc_createid(
    const char* srchost,
    const char* dsthost,
...
static struct altsvc* altsvc_create(
    char* srchost,
    char* dsthost,
    char* srcalpn,
    char* dstalpn,
 ...
 */
CURLcode Curl_altsvc_save(struct Curl_easy* data, struct altsvcinfo* altsvc, const char* file)
{
    struct curl_llist_element* e;
    struct curl_llist_element* n;
    CURLcode result = CURLE_OK;
...
/* altsvc_flush() removes all alternatives for this source origin from the
   list */
static void altsvc_flush(
    struct altsvcinfo* asi, enum alpnid srcalpnid, const char* srchost, unsigned short srcport)
{
    struct curl_llist_element* e;
    struct curl_llist_element* n;
    for (e = asi->list.head; e; e = n) {
        struct altsvc* as = e->ptr;
        n = e->next;
        if ((srcalpnid == as->src.alpnid) && (srcport == as->src.port) &&
    ...
    do {
        if (*p == '=') {
            ...
            if (*p == '\"') {
                ...
                if (dstalpnid) {
                    as = altsvc_createid(srchost, dsthost, srcalpnid, dstalpnid, srcport, dstport);
                    if (as) {
    ...
    const int versions) /* one or more bits */
{
    ...
    for (e = asi->list.head; e; e = n) {
        ...
        if ((as->src.alpnid == srcalpnid) && strcasecompare(as->src.host, srchost) &&
            (as->src.port == srcport) && (versions & as->dst.alpnid)) {
            /* match */
            *dstentry = as;
            return TRUE;
        }
