...
CURLcode Curl_SOCKS5_gssapi_negotiate(int sockindex, struct connectdata* conn)
{
    ...
    if (strchr(service, '/')) {
        ...
        service_name = malloc(service_length + strlen(conn->socks_proxy.host.name) + 2);
        if (!service_name)
            return CURLE_OUT_OF_MEMORY;
        msnprintf(
            service_name,
            service_length + strlen(conn->socks_proxy.host.name) + 2,
            "%s/%s",
            service,
    ...
    status = s_pSecFn->AcquireCredentialsHandle(
        NULL,
        (TCHAR*)TEXT("Kerberos"),
    ...
    for (;;) {
        ...
        status = s_pSecFn->InitializeSecurityContext(
            ...
            context_handle,
            sname,
            ISC_REQ_MUTUAL_AUTH | ISC_REQ_ALLOCATE_MEMORY | ISC_REQ_CONFIDENTIALITY |
        ...
        if (sspi_send_token.cbBuffer != 0) {
            ...
            code = Curl_write_plain(
                conn, sock, (char*)sspi_send_token.pvBuffer, sspi_send_token.cbBuffer, &written);
        ...
        result = Curl_blockread_all(
            conn, sock, (char*)sspi_recv_token.pvBuffer, sspi_recv_token.cbBuffer, &actualread);
    ...
    status = s_pSecFn->QueryCredentialsAttributes(&cred_handle, SECPKG_CRED_ATTR_NAMES, &names);
    s_pSecFn->FreeCredentialsHandle(&cred_handle);
    ...
    if (data->set.socks5_gssapi_nec) {
        ...
        status = s_pSecFn->QueryContextAttributes(&sspi_context, SECPKG_ATTR_SIZES, &sspi_sizes);
        ...
        status = s_pSecFn->EncryptMessage(&sspi_context, KERB_WRAP_NO_ENCRYPT, &wrap_desc, 0);
        ...
        sspi_send_token.cbBuffer =
            sspi_w_token[0].cbBuffer + sspi_w_token[1].cbBuffer + sspi_w_token[2].cbBuffer;
        ...
        memcpy(sspi_send_token.pvBuffer, sspi_w_token[0].pvBuffer, sspi_w_token[0].cbBuffer);
        memcpy(
            (PUCHAR)sspi_send_token.pvBuffer + (int)sspi_w_token[0].cbBuffer,
            sspi_w_token[1].pvBuffer,
            sspi_w_token[1].cbBuffer);
        memcpy(
            (PUCHAR)sspi_send_token.pvBuffer + sspi_w_token[0].cbBuffer + sspi_w_token[1].cbBuffer,
            sspi_w_token[2].pvBuffer,
    ...
    result = Curl_blockread_all(
        conn, sock, (char*)sspi_w_token[0].pvBuffer, sspi_w_token[0].cbBuffer, &actualread);
    ...
    if (!data->set.socks5_gssapi_nec) {
        ...
        status = s_pSecFn->DecryptMessage(&sspi_context, &wrap_desc, 0, &qop);
