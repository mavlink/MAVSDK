...
static void set_remote_option(struct connectdata* conn, int option, int newstate);
...
static CURLcode check_telnet_options(struct connectdata* conn)
{
    ...
    for (head = data->set.telnet_options; head; head = head->next) {
        if (sscanf(head->data, "%127[^= ]%*[ =]%255s", option_keyword, option_arg) == 2) {
            /* Terminal type */
...
static void suboption(struct connectdata* conn)
{
    ...
    switch (CURL_SB_GET(tn)) {
        ...
        case CURL_TELOPT_NEW_ENVIRON:
            ...
            msnprintf((char*)&temp[len], sizeof(temp) - len, "%c%c", CURL_IAC, CURL_SE);
            len += 2;
            bytes_written = swrite(conn->sock[FIRSTSOCKET], temp, len);
            if (bytes_written < 0) {
                err = SOCKERRNO;
...
static void sendsuboption(struct connectdata* conn, int option)
{
    ...
    switch (option) {
        case CURL_TELOPT_NAWS:
            ...
            printsub(data, '>', (unsigned char*)tn->subbuffer + 2, CURL_SB_LEN(tn) - 2);
...
static CURLcode telnet_do(struct connectdata* conn, bool* done)
{
    ...
    */
    ...
    create_event_func =
        CURLX_FUNCTION_CAST(WSOCK2_EVENT, (GetProcAddress(wsock2, "WSACreateEvent")));
    ...
    if (enum_netevents_func == NULL) {
        failf(data, "failed to find WSAEnumNetworkEvents function (%u)", GetLastError());
        FreeLibrary(wsock2);
        return CURLE_FAILED_INIT;
    }
    ...
    */
    ...
    while (keepon) {
        ...
        DWORD waitret = WaitForMultipleObjects(obj_count, objs, FALSE, wait_timeout);
        switch (waitret) {
            case WAIT_TIMEOUT: {
                for (;;) {
        ...
        switch (Curl_poll(pfd, poll_cnt, interval_ms)) {
            ...
            default: /* read! */
                ...
                if (poll_cnt == 2) {
                    ...
                    /* read from user-supplied method */
                    nread =
                        (int)data->state.fread_func(buf, 1, data->set.buffer_size, data->state.in);
