...
#if defined(USE_GSKIT) || defined(USE_NSS) || defined(USE_GNUTLS) || defined(USE_WOLFSSL) || \
...
static const char*
getASN1Element(curl_asn1Element* elem, const char* beg, const char* end) WARN_UNUSED_RESULT;

static const char* getASN1Element(curl_asn1Element* elem, const char* beg, const char* end)
{
    unsigned char b;
    unsigned long len;
 ...
 * string length.
 */
static ssize_t utf8asn1str(char** to, int type, const char* from, const char* end)
{
 ...
 * See RFC 5280.
 */
int Curl_parseX509(curl_X509certificate* cert, const char* beg, const char* end)
{
...
static const char* dumpAlgo(curl_asn1Element* param, const char* beg, const char* end)
{
    curl_asn1Element oid;
...
static void
do_pubkey_field(struct Curl_easy* data, int certnum, const char* label, curl_asn1Element* elem)
{
    const char* output;
...
CURLcode
Curl_extract_certinfo(struct connectdata* conn, int certnum, const char* beg, const char* end)
{
    ...
    result = Curl_base64_encode(
        data, cert.certificate.beg, cert.certificate.end - cert.certificate.beg, &cp1, &cl1);
    if (result)
        return result;
...
static const char* checkOID(const char* beg, const char* end, const char* oid)
{
    curl_asn1Element e;
    const char* ccp;
    const char* p;
...
CURLcode Curl_verifyhost(struct connectdata* conn, const char* beg, const char* end)
{
    struct Curl_easy* data = conn->data;
    curl_X509certificate cert;
    curl_asn1Element dn;
    ...
    for (p = cert.extensions.beg; p < cert.extensions.end && matched != 1;) {
        ...
        if (ext.beg) {
            ...
            for (q = elem.beg; matched != 1 && q < elem.end;) {
                ...
                switch (name.tag) {
                    case 2: /* DNS name. */
                        len = utf8asn1str(&dnsname, CURL_ASN1_IA5_STRING, name.beg, name.end);
