 ...
 */
CURLcode
Curl_sasl_start(struct SASL* sasl, struct connectdata* conn, bool force_ir, saslprogress* progress)
{
    CURLcode result = CURLE_OK;
    ...
    *progress = SASL_IDLE;
    ...
    if ((enabledmechs & SASL_MECH_EXTERNAL) && !conn->passwd[0]) {
        ...
        if ((enabledmechs & SASL_MECH_GSSAPI) && Curl_auth_is_gssapi_supported() &&
            ...
            if (force_ir || data->set.sasl_ir)
                result = Curl_auth_create_gssapi_user_message(
                    data,
                    conn->user,
                    conn->passwd,
                    service,
            ...
            if ((enabledmechs & SASL_MECH_DIGEST_MD5) && Curl_auth_is_digest_supported()) {
            ...
            if (force_ir || data->set.sasl_ir)
                result = Curl_auth_create_ntlm_type1_message(
                    data, conn->user, conn->passwd, service, hostname, &conn->ntlm, &resp, &len);
        } else
#endif
            ...
            if (force_ir || data->set.sasl_ir)
                result = Curl_auth_create_oauth_bearer_message(
                    data, conn->user, hostname, port, oauth_bearer, &resp, &len);
            ...
            if (force_ir || data->set.sasl_ir)
                result = Curl_auth_create_xoauth_bearer_message(
                    data, conn->user, oauth_bearer, &resp, &len);
            ...
            if (force_ir || data->set.sasl_ir)
                result = Curl_auth_create_plain_message(
                    data, conn->sasl_authzid, conn->user, conn->passwd, &resp, &len);
 ...
 */
CURLcode
Curl_sasl_continue(struct SASL* sasl, struct connectdata* conn, int code, saslprogress* progress)
{
    CURLcode result = CURLE_OK;
    struct Curl_easy* data = conn->data;
    ...
    *progress = SASL_INPROGRESS;
    ...
    switch (sasl->state) {
        ...
        case SASL_CRAMMD5:
            ...
            if (!result)
                result = Curl_auth_create_cram_md5_message(
                    data, chlg, conn->user, conn->passwd, &resp, &len);
            free(chlg);
            break;
        case SASL_DIGESTMD5:
            sasl->params->getmessage(data->state.buffer, &serverdata);
        ...
        case SASL_NTLM_TYPE2MSG:
            ...
            sasl->params->getmessage(data->state.buffer, &serverdata);
            result = Curl_auth_decode_ntlm_type2_message(data, serverdata, &conn->ntlm);
            if (!result)
                result = Curl_auth_create_ntlm_type3_message(
                    data, conn->user, conn->passwd, &conn->ntlm, &resp, &len);
            break;
#endif
        ...
        case SASL_GSSAPI_TOKEN:
            ...
            if (sasl->mutual_auth) {
                ...
                result = Curl_auth_create_gssapi_security_message(
                    data, serverdata, &conn->krb5, &resp, &len);
            break;
