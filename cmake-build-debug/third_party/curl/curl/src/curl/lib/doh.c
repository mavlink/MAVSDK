...
static size_t doh_write_cb(const void* contents, size_t size, size_t nmemb, void* userp)
{
    ...
    struct curl_slist* headers)
{
    ...
    DOHcode d = doh_encode(host, dnstype, p->dohbuffer, sizeof(p->dohbuffer), &p->dohlen);
    if (d) {
    ...
    if (data->set.doh_get) {
        char* b64;
        size_t b64len;
        result = Curl_base64url_encode(data, (char*)p->dohbuffer, p->dohlen, &b64, &b64len);
        if (result)
            goto error;
    ...
    if (!result) {
        ...
        if (data->set.str[STRING_SSL_CRLFILE_PROXY]) {
            ERROR_CHECK_SETOPT(CURLOPT_PROXY_CRLFILE, data->set.str[STRING_SSL_CRLFILE_PROXY]);
        }
        ...
        if (data->set.str[STRING_SSL_CAPATH_PROXY]) {
            ERROR_CHECK_SETOPT(CURLOPT_PROXY_CAPATH, data->set.str[STRING_SSL_CAPATH_PROXY]);
        }
#endif
        ...
        if (data->set.str[STRING_SSL_CAFILE_ORIG]) {
            ERROR_CHECK_SETOPT(CURLOPT_CAINFO, data->set.str[STRING_SSL_CAFILE_ORIG]);
        }
        if (data->set.str[STRING_SSL_CAPATH_ORIG]) {
            ERROR_CHECK_SETOPT(CURLOPT_CAPATH, data->set.str[STRING_SSL_CAPATH_ORIG]);
        }
        if (data->set.str[STRING_SSL_CRLFILE_ORIG]) {
            ERROR_CHECK_SETOPT(CURLOPT_CRLFILE, data->set.str[STRING_SSL_CRLFILE_ORIG]);
        }
        if (data->set.ssl.certinfo)
            ERROR_CHECK_SETOPT(CURLOPT_CERTINFO, 1L);
        if (data->set.str[STRING_SSL_RANDOM_FILE]) {
            ERROR_CHECK_SETOPT(CURLOPT_RANDOM_FILE, data->set.str[STRING_SSL_RANDOM_FILE]);
        }
...
Curl_addrinfo* Curl_doh(struct connectdata* conn, const char* hostname, int port, int* waitp)
{
    ...
    *waitp = TRUE; /* this never returns synchronously */
    ...
    data->req.doh.headers = curl_slist_append(NULL, "Content-Type: application/dns-message");
    ...
    if (conn->ip_version != CURL_IPRESOLVE_V6) {
        ...
        result = dohprobe(
            data,
            &data->req.doh.probe[DOH_PROBE_SLOT_IPADDR_V4],
            DNS_TYPE_A,
            hostname,
    ...
    if (conn->ip_version != CURL_IPRESOLVE_V4) {
        ...
        result = dohprobe(
            data,
            &data->req.doh.probe[DOH_PROBE_SLOT_IPADDR_V6],
            DNS_TYPE_AAAA,
            hostname,
            data->set.str[STRING_DOH],
            data->multi,
...
static DOHcode skipqname(const unsigned char* doh, size_t dohlen, unsigned int* indexp)
{
...
static DOHcode store_aaaa(const unsigned char* doh, int index, struct dohentry* d)
{
    /* silently ignore addresses over the limit */
    if (d->numaddr < DOH_MAX_ADDR) {
        struct dohaddr* a = &d->addr[d->numaddr];
...
static DOHcode cnameappend(struct cnamestore* c, const unsigned char* src, size_t len)
{
    if (!c->alloc) {
        c->allocsize = len + 1;
...
static DOHcode
store_cname(const unsigned char* doh, size_t dohlen, unsigned int index, struct dohentry* d)
{
    struct cnamestore* c;
...
static DOHcode rdata(
    const unsigned char* doh,
    size_t dohlen,
...
UNITTEST DOHcode
doh_decode(const unsigned char* doh, size_t dohlen, DNStype dnstype, struct dohentry* d)
{
    unsigned char rcode;
...
#ifndef CURL_DISABLE_VERBOSE_STRINGS
static void showdoh(struct Curl_easy* data, const struct dohentry* d)
{
    int i;
    ...
    for (i = 0; i < d->numaddr; i++) {
        ...
        if (a->type == DNS_TYPE_A) {
            infof(data, "DOH A: %u.%u.%u.%u\n", a->ip.v4[0], a->ip.v4[1], a->ip.v4[2], a->ip.v4[3]);
...
static Curl_addrinfo* doh2ai(const struct dohentry* de, const char* hostname, int port)
{
    Curl_addrinfo* ai;
...
CURLcode Curl_doh_is_resolved(struct connectdata* conn, struct Curl_dns_entry** dnsp)
{
    CURLcode result;
    struct Curl_easy* data = conn->data;
    *dnsp = NULL; /* defaults to no response */
    ...
    if (!data->req.doh.probe[DOH_PROBE_SLOT_IPADDR_V4].easy &&
        ...
        return conn->bits.proxy ? CURLE_COULDNT_RESOLVE_PROXY : CURLE_COULDNT_RESOLVE_HOST;
        ...
        for (slot = 0; slot < DOH_PROBE_SLOTS; slot++) {
            rc[slot] = doh_decode(
                data->req.doh.probe[slot].serverdoh.memory,
