...
static CURLcode singleipconnect(
...
static CURLcode
bindlocal(struct connectdata* conn, curl_socket_t sockfd, int af, unsigned int scope)
{
    ...
    if (dev && (strlen(dev) < 255)) {
        ...
        if (!is_host) {
             ...
             */
            if (setsockopt(
                    sockfd, SOL_SOCKET, SO_BINDTODEVICE, dev, (curl_socklen_t)strlen(dev) + 1) ==
                0) {
...
#endif
                /* IPv4 address */
                if ((af == AF_INET) && (Curl_inet_pton(AF_INET, myhost, &si4->sin_addr) > 0)) {
...
ainext(struct connectdata* conn, int tempindex, bool next) /* use current or next entry */
{
...
#ifdef HAVE_GETPEERNAME
            if (!Curl_addr2string(
                    (struct sockaddr*)&ssrem, plen, conn->primary_ip, &conn->primary_port)) {
                failf(
                    data,
...
#ifdef HAVE_GETSOCKNAME
            if (!Curl_addr2string(
                    (struct sockaddr*)&ssloc, slen, conn->local_ip, &conn->local_port)) {
                failf(
                    data,
...
static CURLcode connect_SOCKS(struct connectdata* conn, int sockindex, bool* done)
{
    ...
    if (conn->bits.socksproxy) {
        ...
        const int port = conn->bits.httpproxy ?
                             ...
                             conn->secondary_port :
                             conn->bits.conn_to_port ? conn->conn_to_port : conn->remote_port;
    ...
    for (i = 0; i < 2; i++) {
        ...
        if (rc == 0) { /* no connection yet */
            ...
            if (Curl_timediff(now, conn->connecttime) >= conn->timeoutms_per_addr) {
                infof(
                    data,
                    "After %" CURL_FORMAT_TIMEDIFF_T "ms connect time, move on!\n",
            ...
            if (i == 0 && !conn->parallel_connect &&
                (Curl_timediff(now, conn->connecttime) >= data->set.happy_eyeballs_timeout)) {
    ...
    error:
        ...
        if (error) {
            ...
            if (conn->tempaddr[i]) {
                ...
                conn->timeoutms_per_addr = conn->tempaddr[i]->ai_next == NULL ? allow : allow / 2;
                ainext(conn, i, TRUE);
...
void Curl_sndbufset(curl_socket_t sockfd)
{
    ...
    if (detectOsState == DETECT_OS_NONE) {
        if (Curl_verify_windows_version(6, 0, PLATFORM_WINNT, VERSION_GREATER_THAN_EQUAL))
 ...
 * having connected.
 */
static CURLcode singleipconnect(struct connectdata* conn, const Curl_addrinfo* ai, int tempindex)
{
    ...
    *sockp = CURL_SOCKET_BAD;
    ...
    if (addr.family == AF_INET
        ...
        result =
            bindlocal(conn, sockfd, addr.family, Curl_ipv6_scope((struct sockaddr*)&addr.sa_addr));
        if (result) {
    ...
    if (!isconnected && (conn->transport != TRNSPRT_UDP)) {
        if (conn->bits.tcp_fastopen) {
            ...
            if (setsockopt(
                    sockfd, IPPROTO_TCP, TCP_FASTOPEN_CONNECT, (void*)&optval, sizeof(optval)) < 0)
    ...
    const struct Curl_dns_entry* remotehost)
{
    ...
    conn->timeoutms_per_addr = conn->tempaddr[0]->ai_next == NULL ? timeout_ms : timeout_ms / 2;
...
curl_socket_t Curl_getconnectinfo(struct Curl_easy* data, struct connectdata** connp)
{
    ...
    if (data->state.lastconnect && (data->multi_easy || data->multi)) {
        ...
        Curl_conncache_foreach(
            data,
            data->multi_easy ? &data->multi_easy->conn_cache : &data->multi->conn_cache,
            &find,
            conn_is_conn);
    ...
    curl_socket_t* sockfd)
{
    ...
    if (data->set.fopensocket) {
        ...
        *sockfd = data->set.fopensocket(
            data->set.opensocket_client, CURLSOCKTYPE_IPCXN, (struct curl_sockaddr*)addr);
        Curl_set_in_callback(data, false);
    } else
