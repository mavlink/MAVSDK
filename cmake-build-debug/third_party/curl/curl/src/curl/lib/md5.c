...
static void MD5_Update(MD5_CTX* ctx, const unsigned char* data, unsigned int length)
{
#if !defined(HAS_MBEDTLS_RESULT_CODE_BASED_FUNCTIONS)
...
const HMAC_params Curl_HMAC_MD5[] = {{/* Hash initialization function. */
...
const MD5_params Curl_DIGEST_MD5[] = {{/* Digest initialization function */
 ...
 * @unittest: 1601
 */
void Curl_md5it(unsigned char* outbuffer, const unsigned char* input, const size_t len)
{
    MD5_CTX ctx;
...
CURLcode Curl_MD5_update(MD5_context* context, const unsigned char* data, unsigned int len)
{
