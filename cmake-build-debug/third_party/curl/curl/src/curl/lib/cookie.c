    ...
    if (list) {
        ...
        while (list) {
            struct CookieInfo* newcookies =
                Curl_cookie_init(data, list->data, data->cookies, data->set.cookiesession);
            if (!newcookies)
    ...
    bool secure) /* TRUE if connection is over secure origin */
{
    ...
    if (httpheader) {
        ...
        if (co->maxage) {
            ...
            offt = curlx_strtoofft(
                (*co->maxage == '\"') ? &co->maxage[1] : &co->maxage[0], NULL, 10, &co->expires);
    ...
    if (domain && co->domain && !isip(co->domain)) {
        ...
        if (!acceptable) {
            infof(
                data,
                "cookie '%s' dropped, domain '%s' must not "
                "set cookies for '%s'\n",
                co->name,
                domain,
                co->domain);
 ...
 ****************************************************************************/
struct CookieInfo*
Curl_cookie_init(struct Curl_easy* data, const char* file, struct CookieInfo* inc, bool newsession)
{
    struct CookieInfo* c;
    FILE* fp = NULL;
    bool fromfile = TRUE;
...
struct Cookie*
Curl_cookie_getlist(struct CookieInfo* c, const char* host, const char* path, bool secure)
{
    struct Cookie* newco;
    struct Cookie* co;
    ...
    while (co) {
        ...
        if (co->secure ? secure : TRUE) {
            ...
            if (!co->domain || (co->tailmatch && !is_ip && tailmatch(co->domain, host)) ||
 ...
 */
static int cookie_output(struct Curl_easy* data, struct CookieInfo* c, const char* filename)
{
    struct Cookie* co;
    FILE* out = NULL;
