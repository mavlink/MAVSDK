...
typedef union {
    enum { PL_UNIX_TOTALSIZE_INIT = 0, PL_UNIX_TOTALSIZE_READING } total_dirsize;

    enum { PL_UNIX_HLINKS_PRESPACE = 0, PL_UNIX_HLINKS_NUMBER } hlinks;

    enum { PL_UNIX_USER_PRESPACE = 0, PL_UNIX_USER_PARSING } user;

    enum { PL_UNIX_GROUP_PRESPACE = 0, PL_UNIX_GROUP_NAME } group;

    enum { PL_UNIX_SIZE_PRESPACE = 0, PL_UNIX_SIZE_NUMBER } size;
...
typedef union {
    enum { PL_WINNT_TIME_PRESPACE = 0, PL_WINNT_TIME_TIME } time;
    enum { PL_WINNT_DIRORSIZE_PRESPACE = 0, PL_WINNT_DIRORSIZE_CONTENT } dirorsize;
    enum {
...
struct ftp_parselist_data {
    enum { OS_TYPE_UNKNOWN = 0, OS_TYPE_UNIX, OS_TYPE_WIN_NT } os_type;
...
static CURLcode ftp_pl_insert_finfo(struct connectdata* conn, struct fileinfo* infop)
{
    ...
    finfo->strings.target =
        parser->offsets.symlink_target ? str + parser->offsets.symlink_target : NULL;
...
size_t Curl_ftp_parselist(char* buffer, size_t size, size_t nmemb, void* connptr)
{
    ...
    while (i < bufflen) { /* FSM */
        ...
        if (finfo->b_used >= finfo->b_size - 1) {
            ...
            char* tmp = realloc(finfo->b_data, finfo->b_size + FTP_BUFFER_ALLOCSIZE);
            if (tmp) {
                                ...
                                if (c == '\r') {
                                    ...
                                    if (strncmp("total ", finfo->b_data, 6) == 0) {
                                        ...
                                        /* here we can deal with directory size, pass the leading
                                           white spaces and then the digits */
                            ...
                            case PL_UNIX_SIZE_NUMBER:
                                ...
                                if (c == ' ') {
                                    ...
                                    finfo->b_data[parser->item_offset + parser->item_length - 1] =
                                        0;
                                    if (!curlx_strtoofft(
                                            finfo->b_data + parser->item_offset, &p, 10, &fsize)) {
                        ...
                        switch (parser->state.UNIX.sub.time) {
                            ...
                            case PL_UNIX_TIME_PART3:
                                ...
                                if (c == ' ') {
                                    ...
                                    /*
                                      if(ftp_pl_gettime(parser, finfo->b_data +
                                      parser->item_offset)) { parser->file_data->flags |=
                            ...
                            case PL_WINNT_DIRORSIZE_CONTENT:
                                ...
                                if (c == ' ') {
                                    ...
                                    if (strcmp("<DIR>", finfo->b_data + parser->item_offset) == 0) {
                                        ...
                                        char* endptr;
                                        if (curlx_strtoofft(
                                                finfo->b_data + parser->item_offset,
                                                &endptr,
                                                10,
                                                &finfo->size)) {
                                            parser->error = CURLE_FTP_BAD_FILE_LIST;
