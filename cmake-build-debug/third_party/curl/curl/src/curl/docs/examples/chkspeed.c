...
int main(int argc, char* argv[])
{
        ...
        for (argc--, argv++; *argv; argc--, argv++) {
            if (strncasecmp(*argv, "-", 1) == 0) {
                if (strncasecmp(*argv, "-H", 2) == 0) {
                    fprintf(
                        stderr, "\rUsage: %s [-m=1|2|5|10|20|50|100] [-t] [-x] [url]\n", appname);
                    exit(1);
    ...
    curl_easy_setopt(curl_handle, CURLOPT_USERAGENT, "libcurl-speedchecker/" CHKSPEED_VERSION);
    ...
    if (CURLE_OK == res) {
        ...
        if ((CURLE_OK == res) && (val > 0))
            printf(
                "Total download time: %" CURL_FORMAT_CURL_OFF_T ".%06ld sec.\n",
                (val / 1000000),
                (long)(val % 1000000));
        ...
        if (prtall) {
            ...
            if ((CURLE_OK == res) && (val > 0))
                printf(
                    "Name lookup time: %" CURL_FORMAT_CURL_OFF_T ".%06ld sec.\n",
                    (val / 1000000),
                    (long)(val % 1000000));
            ...
            if ((CURLE_OK == res) && (val > 0))
                printf(
                    "Connect time: %" CURL_FORMAT_CURL_OFF_T ".%06ld sec.\n",
                    (val / 1000000),
                    (long)(val % 1000000));
        }
    } else {
