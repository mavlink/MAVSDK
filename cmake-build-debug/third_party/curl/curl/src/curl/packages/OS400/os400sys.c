...
int Curl_gsk_attribute_set_buffer_a(
    gsk_handle my_gsk_handle, GSK_BUF_ID bufID, const char* buffer, int bufSize)
...
int Curl_gsk_attribute_set_enum(
    gsk_handle my_gsk_handle, GSK_ENUM_ID enumID, GSK_ENUM_VALUE enumValue)
...
int Curl_gsk_attribute_set_callback(
    gsk_handle my_gsk_handle, GSK_CALLBACK_ID callBackID, void* callBackAreaPtr)
...
static int
cachestring(struct Curl_gsk_descriptor* p, const char* ebcdicbuf, int bufsize, const char** buffer)
...
int Curl_gsk_attribute_get_buffer_a(
    gsk_handle my_gsk_handle, GSK_BUF_ID bufID, const char** buffer, int* bufSize)
...
int Curl_gsk_attribute_get_enum(
    gsk_handle my_gsk_handle, GSK_ENUM_ID enumID, GSK_ENUM_VALUE* enumValue)
...
int Curl_gsk_attribute_get_cert_info(
    gsk_handle my_gsk_handle,
    GSK_CERT_ID certID,
...

{
    ...
    return gsk_attribute_get_cert_info(p->h, certID, certDataElem, certDataElementCount);
}
...
int Curl_gsk_secure_soc_read(
    gsk_handle my_session_handle, char* readBuffer, int readBufSize, int* amtRead)
...
int Curl_gsk_secure_soc_write(
    gsk_handle my_session_handle, char* writeBuffer, int writeBufSize, int* amtWritten)
...
int Curl_gsk_secure_soc_startInit(
    gsk_handle my_session_handle, int IOCompletionPort, Qso_OverlappedIO_t* communicationsArea)
...
OM_uint32 Curl_gss_import_name_a(
    OM_uint32* minor_status, gss_buffer_t in_name, gss_OID in_name_type, gss_name_t* out_name)
...
OM_uint32 Curl_gss_display_status_a(
...

{
    ...
    rc = gss_display_status(
        minor_status, status_value, status_type, mech_type, message_context, status_string);

    if (rc != GSS_S_COMPLETE || !status_string || !status_string->length || !status_string->value)
        return rc;
...

{
    ...
    if (rc != GSS_S_COMPLETE || !output_token || !output_token->length || !output_token->value)
        return rc;
...
OM_uint32 Curl_gss_delete_sec_context_a(
    OM_uint32* minor_status, gss_ctx_id_t* context_handle, gss_buffer_t output_token)
...
int Curl_ldap_search_s_a(
    void* ld, char* base, int scope, char* filter, char** attrs, int attrsonly, LDAPMessage** res)
...

{
    ...
    if (attr) {
        ...
        cp = malloc(i + 1);
        if (!cp) {
            ldap_set_lderrno(ld, LDAP_NO_MEMORY, NULL, ldap_err2string(LDAP_NO_MEMORY));
...
static int
sockaddr2ebcdic(struct sockaddr_storage* dstaddr, const struct sockaddr* srcaddr, int srclen)
{
    ...
    if (!srcaddr || srclen < offsetof(struct sockaddr, sa_family) + sizeof(srcaddr->sa_family) ||
...
static int sockaddr2ascii(
    struct sockaddr* dstaddr, int dstlen, const struct sockaddr_storage* srcaddr, int srclen)
{
    const struct sockaddr_un* srcu;
    struct sockaddr_un* dstu;
    unsigned int dstsize;
    ...
    if (srclen >= offsetof(struct sockaddr_storage, ss_family) + sizeof(srcaddr->ss_family)) {
        switch (srcaddr->ss_family) {
            case AF_UNIX:
                ...
                if (dstsize > 0 && srclen > 0) {
                    srclen = QadrtConvertE2A(dstu->sun_path, srcu->sun_path, dstsize - 1, srclen);
...
int Curl_os400_sendto(
    int sd, char* buffer, int buflen, int flags, struct sockaddr* dstaddr, int addrlen)
{
    int i;
    struct sockaddr_storage laddr;
...
int Curl_os400_recvfrom(
    int sd, char* buffer, int buflen, int flags, struct sockaddr* fromaddr, int* addrlen)
{
    int rcvlen;
    struct sockaddr_storage laddr;
    ...
    rcvlen = recvfrom(sd, buffer, buflen, flags, (struct sockaddr*)&laddr, &laddrlen);
