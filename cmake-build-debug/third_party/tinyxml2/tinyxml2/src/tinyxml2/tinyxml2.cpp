...
XMLDocument::XMLDocument(bool processEntities, Whitespace whitespaceMode) :
    ...
    _commentPool()
{
    // avoid VC++ C4355 warning about 'this' in initializer list (C4355 is off by default in
    // VS2012+)
    _document = this;
}
...
// is useful and code with no check when a check is redundant depending on how size_t and unsigned
// long types sizes relate to each other.
            ...
            if (*q > 0 && *q < ENTITY_RANGE) {
                ...
                if (flag[static_cast<unsigned char>(*q)]) {
                    ...
                    if (!entityPatternPrinted) {
                        // TIXMLASSERT( entityPatternPrinted ) causes gcc -Wunused-but-set-variable
                        // in release
                        TIXMLASSERT(false);
                    }
                    ++p;
...
void XMLPrinter::PushHeader(bool writeBOM, bool writeDec)
{
    if (writeBOM) {
        static const unsigned char bom[] = {
            TIXML_UTF_LEAD_0, TIXML_UTF_LEAD_1, TIXML_UTF_LEAD_2, 0};
