    ...
    bool extraNL = false)
{
    return XMLTest(
        testString,
        XMLDocument::ErrorIDToName(expected),
        XMLDocument::ErrorIDToName(found),
        echo,
        extraNL);
}

bool XMLTest(
    const char* testString, bool expected, bool found, bool echo = true, bool extraNL = false)
{
    return XMLTest(
        testString, expected ? "true" : "false", found ? "true" : "false", echo, extraNL);
}
...
int main(int argc, const char** argv)
{
    ...
    }
    {
        ...
        XMLElement* sub[3] = {
            doc->NewElement("sub"), doc->NewElement("sub"), doc->NewElement("sub")};
        for (int i = 0; i < 3; ++i) {
        ...
        XMLTest(
            "Programmatic DOM",
            "comment",
            doc->FirstChildElement("element")->FirstChild()->Value());
        XMLTest(
            "Programmatic DOM",
            "0",
            doc->FirstChildElement("element")->FirstChildElement()->Attribute("attrib"));
        XMLTest(
            "Programmatic DOM",
            2,
            doc->FirstChildElement()->LastChildElement("sub")->IntAttribute("attrib"));
        XMLTest(
            "Programmatic DOM",
            "& Text!",
        ...
        XMLTest(
            "Programmatic DOM",
            true,
            doc->FirstChildElement()->FirstChildElement()->BoolAttribute("attrib"));
        ...
        int value2 = doc->FirstChildElement()->LastChildElement()->IntAttribute(
            "attrib", replacementIntValue);
        XMLError result =
            doc->FirstChildElement()->LastChildElement()->QueryIntAttribute("attrib", &value1);
        ...
        }
        {
            ...
            XMLTest(
                "Compact mode",
                "<element><sub attrib=\"true\"/><sub/></element>",
                streamer.CStr(),
                false);
        }
    ...
    }
    {
        ...
        XMLTest("Dream", "xml version=\"1.0\"", doc.FirstChild()->ToDeclaration()->Value());
        ...
        XMLTest("Dream-out", "xml version=\"1.0\"", doc2.FirstChild()->ToDeclaration()->Value());
    ...
    // ---------- Attributes ---------
    {
        ...
        }
        {
            ...
            XMLTest(
                "Attribute: unsigned", unsigned(100), element->UnsignedAttribute("attrib"), true);
        }
        {
...
}
{
    ...
    XMLTest(
        ...
        doc.ErrorID()); // is an  error to tinyxml (didn't use to be, but caused issues)
    doc.PrintError();
}
...

{
    ...
    for (XMLNode* ele = xml.FirstChildElement("Parent")->FirstChild(); ele;
         ele = ele->NextSibling()) {
        ++count;
    }
...

{
    ...
    static const char* xml =
        "<doc><element attribute='attribute'/><element attribute='attribute'/></doc>";
    XMLDocument doc;
    doc.Parse(xml);
...
// -------- Handles ------------
{
...

    {
        XMLHandle docH(doc);
        XMLElement* ele =
            docH.FirstChildElement("noSuchElement").FirstChildElement("element").ToElement();
        XMLTest("Handle, non-const, element not found", true, ele == 0);
    }

    {
        const XMLElement* ele =
            XMLConstHandle(doc).FirstChildElement("element").FirstChild().ToElement();
...

{
    ...
    const XMLElement* ElementBlank =
        ElementPlaylist->FirstChildElement("entry")->NextSiblingElement("blank");
...
// ----------- Whitespace ------------
{
    ...
    for (const XMLElement* parent = element->FirstChildElement(); parent;
...

{
    ...
    XMLElement* ele =
        doc.NewElement("unused"); // This will get cleaned up with the 'doc' going out of scope.
...

{
    ...
    XMLTest(
        "Loading an empty file and ErrorName as string",
...

{
    ...
    two = doc.RootElement()->FirstChildElement("two");
    XMLTest(
        "<two> is the last child at root level",
        true,
        two == doc.RootElement()->LastChildElement());
...

{
    ...
    const char* xml =
        "<?xml version=\"1.0\"?><root><sample><field0><1</field0><field1>2</field1></sample></root>";
    XMLDocument doc;
    doc.Parse(xml);
...
{
    // const char* xml = "<element attrA='123456789.123456789' attrB='1.001e9' attrC='1.0e-10'
    // attrD='1001000000.000000' attrE='0.1234567890123456789'/>";
    const char* xml = "<element/>";
    XMLDocument doc;
    doc.Parse(xml);
    ...
    XMLTest( "Float and double formatting.",
            "<element attrA-f64=\"123456789.12345679\" attrB-f64=\"1001000000\" attrC-f64=\"1e+20\"
    attrD-f64=\"0.123456789\" attrA-f32=\"1.2345679e+08\" attrB-f32=\"1.001e+09\"
    attrC-f32=\"1e+20\" attrD-f32=\"0.12345679\"/>\n", printer.CStr(), true );
    */
}
#endif
...

{
    ...
    doc.Parse(xml2);
    XMLTest(
        "Test that declaration after self-closed child is not allowed",
        XML_ERROR_PARSING_DECLARATION,
        doc.ErrorID());
    doc.Parse(xml3);
    XMLTest(
        "Test that declaration after a child is not allowed",
        XML_ERROR_PARSING_DECLARATION,
        doc.ErrorID());
    doc.Parse(xml4);
    XMLTest(
        "Test that declaration inside a child is not allowed",
        XML_ERROR_PARSING_DECLARATION,
        doc.ErrorID());
}
...
}
{
    ...
    const char* xml = "<playlist id='playlist1'>"
                      "<property name='track_name'>voice</property>"
    ...
    int nProperty = 0;
    for (const XMLElement* p = playlist->FirstChildElement("property"); p;
...
// ----------- Line Number Tracking --------------
{
    struct TestUtil : XMLVisitor {
        ...
        void TestParseError(
            const char* testString, const char* docStr, XMLError expected_error, int expectedLine)
        {
            XMLDocument doc;
